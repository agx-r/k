...
Window::~Window() { options().unregister_watcher(*this); }

void Window::set_client(Client* client) { m_client = client; }
...
static uint32_t compute_faces_hash(const FaceRegistry& faces) {
	...
	for (auto&& face :
		...
		hash = combine_hash(hash, face.base.empty() ? hash_value(face.face)
		                                            : hash_value(face.base));
	return hash;
}
...
Window::Setup Window::build_setup(const Context& context) const {
	return {m_position,
	        ...
	        context.selections() |
	            gather<Vector<BasicSelection, MemoryDomain::Display>>()};
}
...
bool Window::needs_redraw(const Context& context) const {
	...
	return m_position != m_last_setup.position or
	       ...
	       compute_faces_hash(context.faces(false)) !=
	           m_last_setup.faces_hash or
	       not std::equal(selections.begin(), selections.end(),
	                      m_last_setup.selections.begin(),
	                      m_last_setup.selections.end());
}
...
const DisplayBuffer& Window::update_display_buffer(const Context& context) {
	...
	if (m_display_buffer.timestamp() != -1) {
		for (auto&& change :
			...
			if (change.type == Buffer::Change::Insert and
			    change.begin.line < m_position.line)
				m_position.line += change.end.line - change.begin.line;
			if (change.type == Buffer::Change::Erase and
			    change.begin.line < m_position.line)
	...
	if (setup.line_count != m_last_display_setup.line_count or
		...
		// WinResize probably want to be notfied anyway
	...
	for (LineCount line = 0; line < setup.line_count; ++line) {
		...
		lines.emplace_back(AtomList{
		    {buffer(),
		     {buffer_line, {buffer_line, buffer()[buffer_line].length()}}}});
	}

	m_display_buffer.compute_range();
	const BufferRange range{{0, 0}, buffer().end_coord()};
	for (auto pass :
	     {HighlightPass::Replace, HighlightPass::Wrap, HighlightPass::Move})
	...
	if (context.ensure_cursor_visible) {
		...
		if (auto line_overflow = cursor_pos->line - m_dimensions.line +
		                         setup.scroll_offset.line + 1;
		    line_overflow > 0) {
			lines.erase(lines.begin(), lines.begin() + (size_t)line_overflow);
			setup.first_line = lines.begin()->range().begin.line;
		}

		auto max_first_column =
		    cursor_pos->column -
		    (setup.widget_columns + setup.scroll_offset.column);
		setup.first_column =
		    std::max(0_col, std::min(setup.first_column, max_first_column));

		auto min_first_column =
		    cursor_pos->column -
		    (m_dimensions.column - setup.scroll_offset.column) + 1;
		setup.first_column = std::max(setup.first_column, min_first_column);
	}

	for (auto& line : m_display_buffer.lines())
		line.trim_from(setup.widget_columns, setup.first_column,
		               m_dimensions.column);
	if (m_display_buffer.lines().size() > m_dimensions.line)
		m_display_buffer.lines().resize((size_t)m_dimensions.line);

	m_builtin_highlighters.highlight(
	    {context, setup, HighlightPass::Colorize, {}}, m_display_buffer, range);
...
static void check_display_setup(const DisplaySetup& setup,
                                const Window& window) {
	kak_assert(setup.first_line >= 0 and
	           setup.first_line < window.buffer().line_count());
...
DisplaySetup Window::compute_display_setup(const Context& context) const {
	...
	DisplaySetup setup{m_position.line, m_dimensions.line, m_position.column,
	                   0_col, offset};
	if (context.ensure_cursor_visible and
	    cursor.line - offset.line < setup.first_line)
		setup.first_line =
		    clamp(cursor.line - offset.line, 0_line, buffer().line_count() - 1);

	for (auto pass :
	     {HighlightPass::Move, HighlightPass::Wrap, HighlightPass::Replace})
	...
	if (context.ensure_cursor_visible and
		...
		setup.first_line =
		    std::min(cursor.line + offset.line - setup.line_count + 1,
		             buffer().line_count() - 1);
