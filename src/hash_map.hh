...
template <typename T> constexpr void constexpr_swap(T& lhs, T& rhs) {
	T tmp = std::move(lhs);
	lhs = std::move(rhs);
	rhs = std::move(tmp);
}
...
template <typename Key, typename Value> struct HashItem {
	Key key{};
	Value value{};
...
template <typename Key> struct HashItem<Key, void> {
	Key key;
...
struct HashMap {
	...
	template <typename KeyType>
	    requires IsHashCompatible<Key, KeyType>
	...
	template <typename KeyType>
	    requires IsHashCompatible<Key, std::remove_cvref_t<KeyType>>
	...
	constexpr void unordered_remove(const KeyType& key) {
		...
		if (index >= 0) {
			...
			if (index != m_items.size())
				m_index.unordered_fix_entries(
				    hash_value(item_key(m_items[index])), m_items.size(),
				    index);
		}
	}
	...
	constexpr bool
	operator==(const HashMap<Key, Value, otherDomain, Container>& other) const {
