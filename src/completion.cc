...
CandidateList complete_filename(StringView prefix, const Regex& ignored_regex,
	...
	Optional<
	    ThreadedRegexVM<const char*, RegexMode::Forward | RegexMode::AnyMatch |
	                                     RegexMode::NoSaves>>
	    vm;
	if (not ignored_regex.empty()) {
		vm.emplace(*ignored_regex.impl());
		if (vm->exec(fileprefix.begin(), fileprefix.end(), fileprefix.begin(),
		             fileprefix.end(), RegexExecFlags::None))
			vm.reset();
	}
	...
	list_files(parsed_dirname, [&](StringView filename, const struct stat& st) {
		if ((not vm or
		     not vm->exec(filename.begin(), filename.end(), filename.begin(),
		                  filename.end(), RegexExecFlags::None)) and
	...
	if (only_dirs and not dirname.empty() and dirname.back() == '/' and
	    fileprefix.empty() and /* exists on disk */ not files.empty()) {
...
CandidateList complete_command(StringView prefix, ByteCount cursor_pos) {
	...
	static HashMap<String, CommandCache, MemoryDomain::Completion>
	    command_cache;
	...
	for (auto dir : StringView{getenv("PATH")} | split<StringView>(':')) {
		...
		if (memcmp(&cache.mtim, &st.st_mtim, sizeof(TimeSpec)) != 0) {
			...
			list_files(dirname, [&](StringView filename,
...
Completions shell_complete(const Context& context, StringView prefix,
	...
	for (ByteCount pos = 0; pos < cursor_pos and pos < len;) {
		command =
		    (pos == 0 or prefix[pos - 1] == ';' or prefix[pos - 1] == '|' or
