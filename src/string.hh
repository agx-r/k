...
template <typename Type, typename CharType> class StringOps {
public:
	...
	[[gnu::always_inline]]
	CharType& operator[](ByteCount pos) {
...
public:
	...
	[[gnu::always_inline]]
	void append(const char* data, ByteCount count) {
	...
	//
	// The MSB of the last byte is used to flag if we are using the allocated
	// buffer (1) or in-situ storage, the small one (0). That means the
	// allocated capacity cannot use its most significant byte, so we
	// effectively limit capacity to 2^24 on 32bit arch, and 2^56 on 64bit.
	//
	// There is also a special NoCopy mode in which the data referred to is
	// un-owned. It is indicated by being in Long mode with capacity == 0.
	struct Data {
		using Alloc = Allocator<char, MemoryDomain::String>;

		Data() { set_empty(); }
		Data(NoCopy, const char* data, size_t size)
		    : u{Long{const_cast<char*>(data), size,
		             /*capacity=*/0,
		...
		size_t size() const {
			return is_long() ? u.l.size
			                 : (Short::capacity - u.s.remaining_size);
		}
		size_t capacity() const {
			return is_long() ? u.l.capacity : Short::capacity;
		}

		const char* data() const { return is_long() ? u.l.ptr : u.s.string; }
		char* data() { return is_long() ? u.l.ptr : u.s.string; }

		template <bool copy = true> void reserve(size_t new_capacity);
	...
	private:
		struct Long {
			static constexpr size_t capacity_bits =
			    CHAR_BIT * (sizeof(size_t) - 1);
			static constexpr size_t max_capacity =
			    ((size_t)1 << capacity_bits) - 1;
		...
		struct Short {
			...
			// When string is full remaining_size will be 0 and be the null
			// terminator. When string is empty remaining size will be 23
			// (0b00010111) and not collide with Long::active_mask.
		...
		void set_empty() {
			u.s.remaining_size = Short::capacity;
			u.s.string[0] = '\0';
		}
...
public:
	...
	[[gnu::always_inline]]
	constexpr ByteCount length() const {
		return m_length;
	}
...
inline String::String(NoCopy, StringView str)
    : m_data{NoCopy{}, str.begin(), (size_t)str.length()} {}
...
inline StringView StringOps<Type, CharType>::substr(ByteCount from,
	...
	return StringView{type().data() + (int)from,
	                  length >= 0 and length < max_length ? length
	                                                      : max_length};
}
...
inline StringView StringOps<Type, CharType>::substr(CharCount from,
	...
	return StringView{beg,
	                  length >= 0 ? utf8::advance(beg, end(), length) : end()};
}
...
inline StringView StringOps<Type, CharType>::substr(ColumnCount from,
	...
	auto beg = utf8::advance(begin(), end(), from);
	return StringView{beg, (length >= 0) ? utf8::advance(beg, end(), length)
	                                     : end()};
}
...
inline bool operator==(const StringView& lhs, const StringView& rhs) {
	return lhs.length() == rhs.length() and
	       (lhs.empty() or
	        std::memcmp(lhs.begin(), rhs.begin(), (size_t)lhs.length()) == 0);
}

inline auto operator<=>(const StringView& lhs, const StringView& rhs) {
	auto lit = lhs.begin(), lend = lhs.end(), rit = rhs.begin(),
	     rend = rhs.end();
	...
	return lit == lend ? std::strong_ordering::less
