...
template <typename Func> struct ViewFactory {
	Func func;
};
...
template <typename Range> struct DecayRangeImpl {
	using type = std::remove_cvref_t<Range>;
};

template <typename Range> struct DecayRangeImpl<Range&> {
	using type = Range&;
};
...
template <typename Range> struct RangeHolderImpl {
	using type = std::remove_cvref_t<Range>;
};

template <typename Range> struct RangeHolderImpl<Range&> {
	struct type {
		Range* range{};
...
template <typename Range> struct ReverseView {
...
template <typename Range> struct SkipView {
...
template <typename Range> struct DropView {
...
template <typename Range, typename Filter> struct FilterView {
...
template <typename Filter> constexpr auto filter(Filter f) {
	return ViewFactory{[f = std::move(f)](auto&& range) {
		using Range = decltype(range);
		return FilterView<DecayRange<Range>, Filter>{std::forward<Range>(range),
		                                             std::move(f)};
	}};
}

template <typename Range> struct EnumerateView {
	using RangeIt = IteratorOf<Range>;
...
template <typename Range, typename Transform> struct TransformView {
	using RangeIt = IteratorOf<Range>;
	using ResType =
...
template <typename Transform> constexpr auto transform(Transform t) {
	return ViewFactory{[t = std::move(t)](auto&& range) {
		using Range = decltype(range);
		return TransformView<DecayRange<Range>, Transform>{
		    std::forward<Range>(range), std::move(t)};
	}};
}

template <typename T, typename U> struct is_pointer_like : std::false_type {};

template <typename T, typename U>
    requires std::is_same_v<std::remove_cvref_t<decltype(*std::declval<U>())>,
                            std::remove_cvref_t<T>>
struct is_pointer_like<T, U> : std::true_type {};

template <typename M, typename T> constexpr auto transform(M T::* member) {
...
template <typename Range, bool escape, bool include_separator,
          typename Element = ValueOf<Range>, typename ValueTypeParam = void>
struct SplitView {
	using RangeIt = IteratorOf<Range>;
	using ValueType =
	...
	struct Iterator {
		...
		ValueType operator*() {
			return {pos, (not include_separator or sep == end) ? sep : sep + 1};
		}
...
auto split(Element separator) {
	return ViewFactory{[s = std::move(separator)](auto&& range) {
		using Range = decltype(range);
		return SplitView<DecayRange<Range>, false, false, Element, ValueType>{
...
auto split_after(Element separator) {
	return ViewFactory{[s = std::move(separator)](auto&& range) {
		using Range = decltype(range);
		return SplitView<DecayRange<Range>, false, true, Element, ValueType>{
		    std::forward<Range>(range), std::move(s), {}};
	}};
}
...
auto split(Element separator, Element escaper) {
	return ViewFactory{[s = std::move(separator),
	                    e = std::move(escaper)](auto&& range) {
		using Range = decltype(range);
		return SplitView<DecayRange<Range>, true, false, Element, ValueType>{
		    std::forward<Range>(range), std::move(s), std::move(e)};
	}};
}

template <typename Range> struct FlattenedView {
	...
	struct Iterator {
		...
		Iterator() = default;
		Iterator(OuterIt begin, OuterIt end)
		    : m_outer_it{begin}, m_outer_end{end} {
			find_next_inner();
		}
		...
		friend bool operator==(const Iterator& lhs, const Iterator& rhs) {
			return lhs.m_outer_it == rhs.m_outer_it and
			       lhs.m_inner_it == rhs.m_inner_it;
		}
...
template <typename Range1, typename Range2> struct ConcatView {
...
ConcatView<DecayRange<Range1>, DecayRange<Range2>>
concatenated(Range1&& range1, Range2&& range2) {
	return {range1, range2};
}

template <typename Range, typename T> auto find(Range&& range, const T& value) {
	using std::begin;
	using std::end;
	return std::find(begin(range), end(range), value);
}

template <typename Range, typename T> auto find_if(Range&& range, T&& op) {
	using std::begin;
	using std::end;
...
template <typename Range, typename T> bool all_of(Range&& range, T&& op) {
	using std::begin;
	using std::end;
	return std::all_of(begin(range), end(range), std::forward<T>(op));
}

template <typename Range, typename T> bool any_of(Range&& range, T&& op) {
	using std::begin;
	using std::end;
	return std::any_of(begin(range), end(range), std::forward<T>(op));
}

template <typename Range, typename T> auto remove_if(Range&& range, T&& op) {
	using std::begin;
	using std::end;
...
template <typename Container> auto gather() {
	return ViewFactory{[](auto&& range) {
		using std::begin;
		using std::end;
...
template <template <typename Element> class Container> auto gather() {
	return ViewFactory{[](auto&& range) {
		using std::begin;
		using std::end;
		using ValueType =
		    std::remove_cv_t<std::remove_reference_t<decltype(*begin(range))>>;
...
auto static_gather() {
	return []<size_t... Indexes>(std::index_sequence<Indexes...>) {
		return ViewFactory{[](auto&& range) {
			...
			// Note that initializer lists elements are guaranteed to be
			// sequenced
			Array<std::remove_cvref_t<decltype(*begin(range))>,
			      sizeof...(Indexes)>
			    res{{elem(Indexes)...}};
