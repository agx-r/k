...
BufferCoord update_insert(BufferCoord coord, BufferCoord begin,
                          BufferCoord end) {
	if (coord < begin)
		return coord;
...
Vector<Selection> compute_modified_ranges(const Buffer& buffer,
                                          size_t timestamp) {
	...
	auto touches = [&](const Selection& lhs, const Selection& rhs) {
		return lhs.max() == end_coord or
		       buffer.char_next(lhs.max()) >= rhs.min();
	};
	size_t dummy = 0;
	ranges.erase(
...
void update_selections(Vector<Selection>& selections, size_t& main,
                       const Buffer& buffer, size_t timestamp, bool merge) {
	...
	while (change_it != changes.end()) {
		...
		kak_assert(std::is_sorted(selections.begin(), selections.end(),
		                          compare_selections));
		if (merge)
			selections.erase(merge_overlapping(selections.begin(),
			                                   selections.end(), main,
			                                   overlaps),
			                 selections.end());
	}
...
void merge_overlapping_selections(Vector<Selection>& selections,
	...
	selections.erase(Kakoune::merge_overlapping(selections.begin(),
	                                            selections.end(), main_index,
	                                            overlaps),
	                 selections.end());
}
...
BufferRange insert(Buffer& buffer, Selection& sel, BufferCoord pos,
                   StringView content) {
	auto range = buffer.insert(pos, content);
	sel.anchor() =
	    buffer.clamp(update_insert(sel.anchor(), range.begin, range.end));
	sel.cursor() =
	    buffer.clamp(update_insert(sel.cursor(), range.begin, range.end));
	return range;
}
...
String selection_to_string(ColumnType column_type, const Buffer& buffer,
                           const Selection& selection, ColumnCount tabstop) {
...
String selection_list_to_string(ColumnType column_type,
...
Selection selection_from_string(ColumnType column_type, const Buffer& buffer,
                                StringView desc, ColumnCount tabstop) {
...
SelectionList selection_list_from_strings(Buffer& buffer,
                                          ColumnType column_type,
                                          ConstArrayView<String> descs,
                                          size_t timestamp, size_t main,
                                          ColumnCount tabstop) {
	if ((column_type != ColumnType::Byte and timestamp != buffer.timestamp()) or
