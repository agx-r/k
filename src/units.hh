...
public:
	...
	[[gnu::always_inline]]
	constexpr RealType operator-() const {
	...
	constexpr friend bool operator==(StronglyTypedNumber lhs,
	                                 StronglyTypedNumber rhs) = default;
	constexpr friend auto operator<=>(StronglyTypedNumber lhs,
	                                  StronglyTypedNumber rhs) = default;
...
template <typename Byte>
    requires(std::is_same_v<std::remove_cv_t<Byte>, char> or
             std::is_same_v<std::remove_cv_t<Byte>, void>)
Byte* operator+(Byte* ptr, ByteCount count) {
