...
String generate_switches_doc(const SwitchMap& switches) {
	...
	auto switch_len = [](auto& sw) {
		return sw.key.column_length() + (sw.value.arg_completer ? 5 : 0);
	};
	...
	for (auto& sw : switches) {
		res += format(
		    "-{} {}{}{}\n", sw.key, sw.value.arg_completer ? "<arg>" : "",
		    String{' ', maxlen - switch_len(sw) + 1}, sw.value.description);
	}
	return res;
}
...
ParametersParser::ParametersParser(ParameterList params,
	...
	for (size_t i = 0; i < params.size(); ++i) {
		if (not only_pos and not ignore_unknown_switches and
		    params[i] == "--") {
			m_state = State::Switch;
			only_pos = true;
		} else if (not only_pos and not params[i].empty() and
		           params[i][0_byte] == '-') {
			...
			m_switches[switch_name.str()] =
			    it->value.arg_completer ? params[i] : StringView{};
		} else // positional
		{
	...
	if (not ignore_errors and
	    (count > desc.max_positionals or count < desc.min_positionals))
