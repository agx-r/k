...
template <typename T, typename Policy = RefCountablePolicy> struct RefPtr {
	RefPtr() = default;
	explicit RefPtr(T* ptr) : m_ptr(ptr) { acquire(); }
	~RefPtr() noexcept { release(); }
	RefPtr(const RefPtr& other) : m_ptr(other.m_ptr) { acquire(); }
	RefPtr(RefPtr&& other) noexcept(noexcept(moved(nullptr)))
	    : m_ptr(other.m_ptr) {
		other.m_ptr = nullptr;
		moved(&other);
	}
	...
	friend bool operator==(const RefPtr& lhs, const T* rhs) {
...
private:
	...
	[[gnu::always_inline]]
	void moved(void* from) noexcept(noexcept(Policy::ptr_moved(nullptr, nullptr,
	                                                           nullptr))) {
		if (m_ptr)
