...
static int count_word_boundaries_match(StringView candidate, StringView query) {
	...
	for (Utf8It it{candidate.begin(), candidate}; it != candidate.end(); ++it) {
		...
		const bool is_word_boundary =
		    prev == 0 or (not is_word(prev, {}) and is_word(c, {})) or
...
static Optional<SubseqRes> subsequence_match_smart_case(StringView str,
                                                        StringView subseq) {
...
RankedMatch::RankedMatch(StringView candidate, StringView query,
	...
	if (auto it = find(candidate | reverse() | skip(1), '/').base();
	    it == candidate.begin() or
	    subsequence_match_smart_case({it, candidate.end()}, query)) {
		m_flags |= Flags::BaseName;
		if ((candidate.end() - it) >= query.length() and
		    std::equal(Utf8It{query.begin(), query}, Utf8It{query.end(), query},
		               Utf8It{it, candidate},
		               [](Codepoint query, Codepoint candidate) {
			               return smartcase_eq(candidate, query);
		               }))
			m_flags |= Flags::Prefix;
	}

	auto it = std::search(candidate.begin(), candidate.end(), query.begin(),
	                      query.end(), smartcase_eq);
	if (it != candidate.end()) {
		m_flags |= Flags::Contiguous;
		if (std::all_of(Utf8It{query.begin(), query},
		                Utf8It{query.end(), query},
		                [](auto cp) { return is_word(cp); }))
			m_flags |= Flags::SingleWord;
...
RankedMatch::RankedMatch(StringView candidate, UsedLetters candidate_letters,
    ...
    : RankedMatch{candidate, query, [&] {
	                  return matches(to_lower(query_letters),
	                                 to_lower(candidate_letters)) and
	                         matches(query_letters & upper_mask,
	                                 candidate_letters & upper_mask);
                  }} {}
...
UnitTest test_ranked_match{[] {
	...
	kak_assert(count_word_boundaries_match("countWordBoundariesMatch", "wm") ==
	           2);
	kak_assert(
	    count_word_boundaries_match("countWordBoundariesMatch", "cobm") == 3);
	kak_assert(
	    count_word_boundaries_match("countWordBoundariesMatch", "cWBM") == 4);
	...
	kak_assert(
	    preferred("foo_b", "foo/bar/foo_bar.baz", "test/test_foo_bar.baz"));
	kak_assert(
	    preferred("foo_b", "bar/bar_qux/foo_bar.baz", "foo/test_foo_bar.baz"));
	...
	kak_assert(
	    preferred("rm.cc", "src/ranked_match.cc", "test/README.asciidoc"));
	kak_assert(preferred("luaremote", "src/script/LuaRemote.cpp",
	                     "tests/TestLuaRemote.cpp"));
	kak_assert(preferred("lang/haystack/needle.c",
	                     "git.evilcorp.com/language/haystack/aaa/needle.c",
	                     "git.evilcorp.com/aaa/ng/wrong-haystack/needle.cpp"));
	kak_assert(preferred("evilcorp-lint/bar.go",
