...
public:
	...
	virtual void set(Context& context, ConstArrayView<String> values,
	                 bool restoring = false) = 0;
	virtual ConstArrayView<String> get(const Context& context) = 0;
	virtual const String& get_main(const Context& context,
	                               size_t main_index) = 0;
...
public:
	DynamicRegister(String name, Getter getter, Setter setter)
	    : StaticRegister(std::move(name)), m_getter(std::move(getter)),
...
public:
	...
	void set(Context& context, ConstArrayView<String> values,
	         bool restoring) override;
...
UniquePtr<Register> make_dyn_reg(String name, Func func) {
	...
	return make_unique_ptr<DynamicRegister<Func, decltype(setter)>>(
	    name, std::move(func), setter);
}

template <typename Getter, typename Setter>
UniquePtr<Register> make_dyn_reg(String name, Getter getter, Setter setter) {
	return make_unique_ptr<DynamicRegister<Getter, Setter>>(
...
public:
	...
	CandidateList complete_register_name(StringView prefix,
...
protected:
	HashMap<Codepoint, UniquePtr<Register>, MemoryDomain::Registers>
	    m_registers;
};
