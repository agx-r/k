...
Buffer* open_fifo(StringView name, StringView filename, Buffer::Flags flags,
	...
	return create_fifo_buffer(name.str(), fd, flags,
	                          scroll ? AutoScroll::Yes : AutoScroll::No);
}

template <typename... Completers> struct PerArgumentCommandCompleter;

template <> struct PerArgumentCommandCompleter<> {
	Completions operator()(const Context&, CommandParameters, size_t,
	                       ByteCount) const {
		return {};
	}
};
...
struct PerArgumentCommandCompleter<Completer, Rest...>
	...
	template <typename C, typename... R>
	    requires(not std::is_base_of_v<PerArgumentCommandCompleter<>,
	...
	Completions operator()(const Context& context, CommandParameters params,
		...
		return PerArgumentCommandCompleter<Rest...>::operator()(
		    context, params.subrange(1), token_to_complete - 1, pos_in_token);
	}
...
auto add_flags(Completer completer, Completions::Flags completions_flags) {
	return [completer = std::move(completer),
	        completions_flags](const Context& context, StringView prefix,
...
template <typename Completer> auto menu(Completer completer) {
...
template <bool menu>
auto filename_arg_completer = [](const Context& context, StringView prefix,
...
auto client_arg_completer = [](const Context& context, StringView prefix,
                               ByteCount cursor_pos) -> Completions {
	return {0_byte, cursor_pos,
...
auto arg_completer = [](auto candidates) -> PromptCompleter {
	return [=](const Context& context, StringView prefix,
	           ByteCount cursor_pos) -> Completions {
		return Completions{0_byte, cursor_pos,
		                   complete(prefix, cursor_pos, candidates),
		                   Completions::Flags::Menu};
	};
};
...
template <typename Func> auto make_single_word_completer(Func&& func) {
	return make_completer([func = std::move(func)](
	                          const Context& context, StringView prefix,
	                          ByteCount cursor_pos) -> Completions {
...
static Completions complete_scope(const Context&, StringView prefix,
                                  ByteCount cursor_pos) {
...
static Completions complete_scope_no_global(const Context&, StringView prefix,
                                            ByteCount cursor_pos) {
	static constexpr StringView scopes[] = {"buffer", "window", "local",
	                                        "current"};
	return {0_byte, cursor_pos, complete(prefix, cursor_pos, scopes)};
}
...
struct AsyncShellScript {
	...
	AsyncShellScript(const AsyncShellScript& other)
	    : m_shell_script{other.m_shell_script}, m_flags(other.m_flags) {}
	AsyncShellScript& operator=(const AsyncShellScript& other) {
		m_shell_script = other.m_shell_script;
		m_flags = other.m_flags;
		return *this;
	}

protected:
	void spawn_script(const Context& context, const ShellContext& shell_context,
	                  auto&& handle_line) {
		m_handle_line = handle_line;
		m_running_script.emplace(ShellManager::instance().spawn(
		    m_shell_script, context, false, shell_context));
		m_watcher.emplace((int)m_running_script->out, FdEvents::Read,
		                  EventMode::Urgent,
		                  [this, &input_handler = context.input_handler()](
		                      auto&&... args) { read_stdout(input_handler); });
	}
	...
	void read_stdout(InputHandler& input_handler) {
		...
		while (fd_readable(fd)) {
			...
			m_stdout_buffer.insert(m_stdout_buffer.end(), buffer,
			                       buffer + size);
		}
		auto end = closed ? m_stdout_buffer.end()
		                  : find(m_stdout_buffer | reverse(), '\n').base();
		for (auto c : ArrayView(m_stdout_buffer.begin(), end) |
		                  split<StringView>('\n') |
...
struct ShellScriptCompleter : AsyncShellScript {
	...
	Completions operator()(const Context& context, CommandParameters params,
		...
		if (m_last_token != token_to_complete or
		    pos_in_token != m_last_pos_in_token) {
			ShellContext shell_context{
			    params,
			    {{"token_to_complete", to_string(token_to_complete)},
			     {"pos_in_token", to_string(pos_in_token)}}};
			spawn_script(context, shell_context, [this](StringView line) {
				m_candidates.push_back(line.str());
			});

			candidates =
			    std::move(m_candidates); // avoid completion menu flicker by
...
struct ShellCandidatesCompleter : AsyncShellScript {
	...
	Completions operator()(const Context& context, CommandParameters params,
		...
		if (m_last_token != token_to_complete) {
			...
			spawn_script(context, shell_context, [this](StringView line) {
				m_candidates.emplace_back(line.str(), used_letters(line));
			});
			m_candidates.clear();
			m_last_token = token_to_complete;
		}
...
private:
	Completions rank_candidates(StringView query) {
		...
		for (auto&& [i, candidate] : m_candidates | enumerate()) {
			if (RankedMatch m{candidate.first, candidate.second, query,
			                  query_letters}) {
...
template <typename Completer> struct PromptCompleterAdapter {
	PromptCompleterAdapter(Completer completer)
	...
	operator PromptCompleter() && {
		...
		return [completer = std::move(m_completer)](const Context& context,
		                                            StringView prefix,
		                                            ByteCount cursor_pos) {
			return completer(context, {String{String::NoCopy{}, prefix}}, 0,
			                 cursor_pos);
		};
	}
...
void edit(const ParametersParser& parser, Context& context,
	...
	const auto flags =
	    (no_hooks ? Buffer::Flags::NoHooks : Buffer::Flags::None) |
	...
	const auto& name = parser.positional_count() > 0
	                       ? parser[0]
	                       : (scratch ? generate_buffer_name("*scratch-{}*")
	                                  : context.buffer().name());

	Buffer* buffer = buffer_manager.get_buffer_ifp(name);
	if (scratch) {
		if (parser.get_switch("readonly") or parser.get_switch("fifo") or
		    parser.get_switch("scroll"))
			throw runtime_error(
		...
		if (buffer == nullptr or force_reload) {
			...
			throw runtime_error(
			    format("buffer '{}' exists but is not a scratch buffer", name));
	} else if (force_reload and buffer and
	...
	buffer =
	    &context.buffer(); // change_buffer hooks might change the buffer again
...
ParameterDesc edit_params{
    {{"existing",
      {{}, "fail if the file does not exist, do not open a new file"}},
     {"scratch", {{}, "create a scratch buffer, not linked to a file"}},
     {"debug", {{}, "create buffer as debug output"}},
     {"fifo",
      {{filename_arg_completer<true>},
       "create a buffer reading its content from a named fifo"}},
...
const CommandDesc edit_cmd = {
    "edit",
    "e",
    "edit [<switches>] <filename> [<line> [<column>]]: open the given filename "
    "in a buffer",
    edit_params,
    CommandFlags::None,
    CommandHelper{},
...
const ParameterDesc write_params = {
    {{"sync",
     ...
     {"method",
      {{arg_completer(Array{"replace", "overwrite"})},
...
auto parse_write_method(StringView str) {
	...
	auto it = find_if(
	    desc, [str](const EnumDesc<WriteMethod>& d) { return d.name == str; });
	if (it == desc.end())
...
void do_write_buffer(Context& context, Optional<String> filename,
                     WriteFlags flags,
                     Optional<WriteMethod> write_method = {}) {
	...
	const bool is_readonly =
	    (bool)(context.buffer().flags() & Buffer::Flags::ReadOnly);
	...
	auto effective_filename =
	    filename ? parse_filename(*filename) : buffer.filename();
	...
	auto method = write_method.value_or_compute(
	    [&] { return context.options()["writemethod"].get<WriteMethod>(); });

	context.hooks().run_hook(Hook::BufWritePre, effective_filename, context);
	BusyIndicator busy_indicator{
	    context, [&](std::chrono::seconds elapsed) {
		    return DisplayLine{format("waiting while writing buffer '{}' ({}s)",
		                              buffer.filename(), elapsed.count()),
...
void write_buffer(const ParametersParser& parser, Context& context,
	...
	return do_write_buffer(
	    ...
	    (parser.get_switch("sync") ? WriteFlags::Sync : WriteFlags::None) |
	        (parser.get_switch("force") or force ? WriteFlags::Force
	                                             : WriteFlags::None),
	...
	for (auto& buffer : buffers) {
		if ((buffer->flags() & Buffer::Flags::File) and
		    ((buffer->flags() & Buffer::Flags::New) or
		     buffer->is_modified()) and
		    not(buffer->flags() & Buffer::Flags::ReadOnly)) {
			auto method = write_method.value_or_compute([&] {
				return context.options()["writemethod"].get<WriteMethod>();
			});
			...
			BusyIndicator busy_indicator{
			    context, [&](std::chrono::seconds elapsed) {
				    return DisplayLine{
				        format("waiting while writing buffer ({}s)",
				               elapsed.count()),
				        context.faces()["Information"]};
			    }};
...
const CommandDesc write_all_cmd = {
    "write-all",
    "wa",
    "write-all [<switches>]: write all changed buffers that are associated to "
    "a file",
    ParameterDesc{write_params_except_force.switches,
...
template <bool force>
void kill(const ParametersParser& parser, Context& context,
          const ShellContext&) {
...
const CommandDesc kill_cmd = {
    "kill",
    nullptr,
    "kill [<exit status>]: terminate the current session, the server and all "
    "clients connected. "
...
const CommandDesc force_kill_cmd = {
    "kill!",
    nullptr,
    "kill! [<exit status>]: force the termination of the current session, the "
    "server and all clients connected. "
    "An optional integer parameter can set the server and client processes "
    "exit status",
    {{}, ParameterDesc::Flags::SwitchesAsPositional, 0, 1},
    CommandFlags::None,
...
const CommandDesc daemonize_session_cmd = {
    ...
    [](const ParametersParser&, Context&, const ShellContext&) {
	    Server::instance().daemonize();
    }};

template <bool force>
void quit(const ParametersParser& parser, Context& context,
          const ShellContext&) {
	if (not force and ClientManager::instance().count() == 1 and
...
const CommandDesc quit_cmd = {
    "quit",
    "q",
    "quit [<exit status>]: quit current client, and the kakoune session if the "
    "client is the last "
...
void write_quit(const ParametersParser& parser, Context& context,
	...
	do_write_buffer(context, {},
	                parser.get_switch("sync") ? WriteFlags::Sync
	                                          : WriteFlags::None,
	                parser.get_switch("method").map(parse_write_method));
...
const CommandDesc write_quit_cmd = {
    "write-quit",
    "wq",
    "write-quit [<switches>] [<exit status>]: write current buffer and quit "
    "current client. "
    "An optional integer parameter can set the client exit status",
    write_params_except_force,
    CommandFlags::None,
...
const CommandDesc write_all_quit_cmd = {
    "write-all-quit",
    "waq",
    "write-all-quit [<switches>] [<exit status>]: write all buffers associated "
...
const CommandDesc buffer_cmd = {
    ...
    [](const ParametersParser& parser, Context& context, const ShellContext&) {
	    Buffer& buffer =
	        parser.get_switch("matching")
	            ? BufferManager::instance().get_buffer_matching(
...
void cycle_buffer(const ParametersParser& parser, Context& context,
	...
	auto it = find_if(
	    ...
	    [oldbuf](const UniquePtr<Buffer>& lhs) { return lhs.get() == oldbuf; });
...
void delete_buffer(const ParametersParser& parser, Context& context,
	...
	if (not force and (buffer.flags() & Buffer::Flags::File) and
	    buffer.is_modified())
		throw runtime_error(
		    format("buffer '{}' is modified", buffer.display_name()));
...
const CommandDesc rename_buffer_cmd = {
    ...
    [](const ParametersParser& parser, Context& context, const ShellContext&) {
	    ...
	    if (not buffer.set_name(is_file ? parse_filename(parser[0])
	                                    : parser[0]))
		    throw runtime_error(format("unable to change buffer name to '{}': "
		                               "a buffer with this name already exists",
		                               parser[0]));
    }};

static constexpr auto highlighter_scopes = {"global/", "buffer/", "window/",
                                            "shared/"};

template <bool add>
Completions
highlighter_cmd_completer(const Context& context, CommandParameters params,
                          size_t token_to_complete, ByteCount pos_in_token) {
	if (token_to_complete == 0) {
...
static std::pair<HighlighterGroup&, Highlighter&>
get_highlighter(const Context& context, StringView path) {
	...
	if (path.starts_with("buffer=")) {
		auto& buffer_manager = BufferManager::instance();
		while (sep_it != path.end() and
		       not buffer_manager.get_buffer_ifp({path.begin() + 7, sep_it}))
...
const CommandDesc arrange_buffers_cmd = {
    ...
    "    the named buffers will be moved to the front of the buffer list, in "
    "the order given\n"
    "    buffers that do not appear in the parameters will remain at the end "
    "of the list, keeping their current order",
    ParameterDesc{{
                      {"back",
                       {{},
                        "place the named buffers at the back, rather than the "
                        "front, of the buffer list"}},
                  },
                  ParameterDesc::Flags::None,
                  1},
    CommandFlags::None,
    CommandHelper{},
    [](const Context& context, CommandParameters params, size_t,
       ByteCount cursor_pos) {
	    return menu(complete_buffer_name<false>)(context, params.back(),
	                                             cursor_pos);
    },
...
const CommandDesc add_highlighter_cmd = {
    "add-highlighter",
    "addhl",
    "add-highlighter [-override] <path>/<name> <type> <type params>...: add a "
    "highlighter to the group identified by <path>\n"
    ...
    ParameterDesc{
        {
            {"override",
             {{}, "replace existing highlighter with same path if it exists"}},
        },
    ...
    [](const Context& context, CommandParameters params) -> String {
	    if (params.size() > 1) {
		    ...
		    if (it != registry.end()) {
			    ...
			    auto desc_params = generate_switches_doc(
			        it->value.description->params.switches);
    ...
    [](const ParametersParser& parser, Context& context, const ShellContext&) {
	    ...
	    parent.add_child(name.empty() ? auto_name(parser.positionals_from(1))
	                     ...
	                     (bool)parser.get_switch("override"));
...
static Completions complete_hooks(const Context&, StringView prefix,
                                  ByteCount cursor_pos) {
	return {0_byte, cursor_pos,
...
Completions complete_completer_type(const Context&, StringView prefix,
                                    ByteCount cursor_pos) {
	static constexpr StringView completers[] = {"file",
	                                            "client",
	                                            "buffer",
	                                            "shell-script",
...
CommandCompleter make_command_completer(StringView type, StringView param,
                                        Completions::Flags completions_flags) {
	if (type == "file") {
		return [=](const Context& context, CommandParameters params,
		           size_t token_to_complete, ByteCount pos_in_token) {
			const String& prefix = params[token_to_complete];
			const auto& ignored_files =
			    context.options()["ignored_files"].get<Regex>();
		...
		throw runtime_error(
		    format("invalid command completion type '{}'", type));
}

static CommandCompleter
parse_completion_switch(const ParametersParser& parser,
                        Completions::Flags completions_flags) {
	for (StringView completion_switch :
		...
		if (auto param = parser.get_switch(completion_switch)) {
			...
			if (completion_switch.ends_with(suffix))
				completion_switch = completion_switch.substr(
				    0, completion_switch.length() - suffix.length());
			return make_command_completer(completion_switch, *param,
			                              completions_flags);
		}
	}
	return {};
...
void define_command(const ParametersParser& parser, Context& context,
	...
	const Completions::Flags completions_flags =
	    menu ? Completions::Flags::Menu : Completions::Flags::None;
	...
	if (auto params = parser.get_switch("params")) {
		...
		desc =
		    ParameterDesc{{}, ParameterDesc::Flags::SwitchesAsPositional, 0, 0};
		cmd = [=](const ParametersParser& parser, Context& context,
		          const ShellContext& sc) {
			LocalScope local_scope{context};
	...
	CommandCompleter completer =
	    parse_completion_switch(parser, completions_flags);
	if (menu and not completer)
		throw runtime_error("menu switch requires a completion switch");
	auto docstring =
	    trim_indent(parser.get_switch("docstring").value_or(StringView{}));

	cm.register_command(cmd_name, cmd, docstring, desc, flags, CommandHelper{},
	                    std::move(completer));
}
...
const CommandDesc define_command_cmd = {
    ...
    ParameterDesc{
        {{"params",
         ...
         {"hidden",
          {{}, "do not display the command in completion candidates"}},
         {"docstring",
          {ArgCompleter{}, "define the documentation string for command"}},
         {"menu", {{}, "treat completions as the only valid inputs"}},
         {"file-completion",
          {{}, "complete parameters using filename completion"}},
         {"client-completion",
          {{}, "complete parameters using client name completion"}},
         {"buffer-completion",
          {{}, "complete parameters using buffer name completion"}},
         {"command-completion",
          {{}, "complete parameters using kakoune command completion"}},
         {"shell-completion",
          {{}, "complete parameters using shell command completion"}},
         {"shell-script-completion",
          {ArgCompleter{}, "complete parameters using the given shell-script"}},
         {"shell-script-candidates",
          {ArgCompleter{},
           "get the parameter candidates using the given shell-script"}}},
        ParameterDesc::Flags::None,
        2,
        2},
    CommandFlags::None,
...
static Completions complete_alias_name(const Context& context,
                                       StringView prefix,
                                       ByteCount cursor_pos) {
	return {0_byte, cursor_pos,
	        complete(prefix, cursor_pos,
...
const CommandDesc alias_cmd = {
    ...
    CommandFlags::None,
    CommandHelper{},
    make_completer(menu(complete_scope), complete_alias_name,
...
const CommandDesc complete_command_cmd = {
    ...
    [](const ParametersParser& parser, Context& context, const ShellContext&) {
	    const Completions::Flags flags = parser.get_switch("menu")
	                                         ? Completions::Flags::Menu
	                                         : Completions::Flags::None;
	    CommandCompleter completer = make_command_completer(
	        parser[1],
	        parser.positional_count() >= 3 ? parser[2] : StringView{}, flags);
	    CommandManager::instance().set_command_completer(parser[0],
	                                                     std::move(completer));
    }};
...
const CommandDesc echo_cmd = {
    ...
    ParameterDesc{
        {{"markup", {{}, "parse markup"}},
         {"quoting",
          {{arg_completer(Array{"raw", "kakoune", "shell"})},
           "quote each argument separately using the given style "
           "(raw|kakoune|shell)"}},
         {"end-of-line", {{}, "add trailing end-of-line"}},
         {"to-file",
          {{filename_arg_completer<false>}, "echo contents to given filename"}},
         {"to-shell-script",
          {ArgCompleter{}, "pipe contents to given shell script"}},
    ...
    [](const ParametersParser& parser, Context& context,
       const ShellContext& shell_context) {
	    ...
	    if (auto quoting = parser.get_switch("quoting"))
		    message = join(parser | transform(quoter(option_from_string(
		                                Meta::Type<Quoting>{}, *quoting))),
		                   ' ', false);
	    else
	    ...
	    if (auto filename = parser.get_switch("to-file")) {
		    BusyIndicator busy_indicator{
		        context, [&](std::chrono::seconds elapsed) {
			        return DisplayLine{
			            format("waiting while writing to '{}' ({}s)", *filename,
			                   elapsed.count()),
			            context.faces()["Information"]};
		        }};
		    write_to_file(*filename, message);
	    } else if (auto command = parser.get_switch("to-shell-script"))
		    ShellManager::instance().eval(*command, context, message,
...
KeymapMode parse_keymap_mode(StringView str,
	...
	if (prefix_match("combine", str))
		return KeymapMode::Combine;
...
static constexpr auto modes =
    make_array<StringView>({"normal", "insert", "menu", "prompt", "goto",
                            "view", "user", "object", "combine"});
...
const CommandDesc debug_cmd = {
    ...
    CommandFlags::None,
    CommandHelper{},
    make_completer([](const Context& context, StringView prefix,
                      ByteCount cursor_pos) -> Completions {
	    auto c = {"info",           "buffers",           "options", "memory",
	              "shared-strings", "profile-hash-maps", "faces",   "mappings",
	              "regex",          "registers"};
    ...
    [](const ParametersParser& parser, Context& context, const ShellContext&) {
	    if (parser[0] == "info") {
		    ...
		    write_to_debug_buffer(format("{:17} │{:17} │{:17} │{:17} ",
		                                 "domain", "bytes", "active allocs",
		                                 "total allocs"));
		    write_to_debug_buffer(format(
		        "{0}┼{0}┼{0}┼{0}", String(Codepoint{0x2500}, column_size + 1)));
    ...
    (__GLIBC__ > 2 || (__GLIBC__ == 2 && __GLIBC_MINOR__ >= 33))
		    write_to_debug_buffer(
		        format("  Malloced: {}", grouped(mallinfo2().uordblks)));
#elif defined(__GLIBC__) || defined(__CYGWIN__)
		    write_to_debug_buffer(
		        format("  Malloced: {}", grouped(mallinfo().uordblks)));
#endif
		    ...
		    for (auto mode : concatenated(modes, user_modes)) {
			    ...
			    for (auto& key : keymaps.get_mapped_keys(m)) {
				    ...
				    for (const auto& k : kl)
					    mapping += to_string(k);
				    write_to_debug_buffer(
				        format(" * {} {}: '{}' {}", mode, key, mapping,
				               keymaps.get_mapping_docstring(key, m)));
			    }
		    }
		    ...
		    write_to_debug_buffer(
		        format(" * {}:\n{}", parser[1],
		               dump_regex(compile_regex(parser[1],
...
const CommandDesc source_cmd = {
    ...
    [](const ParametersParser& parser, Context& context, const ShellContext&) {
	    ProfileScope profile{context.options()["debug"].get<DebugFlags>(),
...
const CommandDesc set_option_cmd = {
    ...
    [](const Context& context, CommandParameters params,
       size_t token_to_complete, ByteCount pos_in_token) -> Completions {
	    if (token_to_complete == 0)
		    return menu(complete_scope_including_current)(context, params[0],
		                                                  pos_in_token);
	    else if (token_to_complete == 1)
		    return {
		        0_byte, params[1].length(),
		        ...
		        Completions::Flags::Menu};
	             ...
	             GlobalScope::instance().option_registry().option_exists(
	                 params[1])) {
...
const CommandDesc declare_option_cmd = {
    "declare-option",
    "decl",
    "declare-option [<switches>] <type> <name> [value]: declare option <name> "
    "of type <type>.\n"
...
template <bool unmap>
static Completions
map_key_completer(const Context& context, CommandParameters params,
                  size_t token_to_complete, ByteCount pos_in_token) {
	if (token_to_complete == 0)
		return menu(complete_scope)(context, params[0], pos_in_token);
    ...
    [](const ParametersParser& parser, Context& context, const ShellContext&) {
	    ...
	    if (keymaps.is_mapped(key[0], keymap_mode) and
	        (parser.positional_count() < 4 or
	         keymaps.get_mapping_keys(key[0], keymap_mode) ==
	             parse_keys(parser[3])))
...
ParameterDesc make_context_wrap_params_impl(
	...
	return {
	    {{"client",
	      {{client_arg_completer},
	       "run in the client context for each client in the given comma "
	       "separatd list"}},
	     {"try-client",
	      {{client_arg_completer},
...
void context_wrap(const ParametersParser& parser, Context& context,
	...
	auto make_register_restorer = [&](char c) {
		...
		return OnScopeEnd([&, c, save = reg.save(context),
		                   d = ScopedSetBool{reg.modified_hook_disabled()}] {
			try {
		...
		if (*bufnames == "*") {
			for (auto&& buffer :
			     BufferManager::instance() |
			         ...
			         gather<Vector<SafePtr<Buffer>>>()) // gather as we might be
			                                            // mutating the buffer
			                                            // list in the loop.
	...
	auto context_wrap_for_context = [&parser, &func](Context& base_context) {
		...
		if (draft) {
			input_handler.emplace(base_context.selections(),
			                      Context::Flags::Draft, base_context.name());
			...
			// We do not want this draft context to commit undo groups if the
			// real one is going to commit the whole thing later
		...
		if (parser.get_switch("itersel")) {
			...
			for (auto& sel : sels) {
				...
				try {
					...
					if (not draft) {
						update_selections(new_sels, main, c.buffer(),
						                  timestamp);
						timestamp = c.buffer().timestamp();
						if (&sel == &sels.main())
							main =
							    new_sels.size() + c.selections().main_index();

						const auto middle = new_sels.insert(
						    new_sels.end(), c.selections().begin(),
						    c.selections().end());
						std::inplace_merge(new_sels.begin(), middle,
						                   new_sels.end(), compare_selections);
					}
			...
			const bool collapse_jumps =
			    not(c.flags() & Context::Flags::Draft) and c.has_buffer();
			auto& jump_list = c.jump_list();
			const size_t prev_index = jump_list.current_index();
			auto jump =
			    collapse_jumps ? c.selections() : Optional<SelectionList>{};

			func(parser, c);

			// If the jump list got mutated, collapse all jumps into a single
			// one from original selections
		...
		if (*client_names == "*") {
			for (auto&& client :
			     ClientManager::instance() |
			         ...
			         gather<Vector<SafePtr<Client>>>()) // gather as we might be
			                                            // mutating the client
			                                            // list in the loop.
				context_wrap_for_context(client->context());
		} else
			for (auto&& name : *client_names | split<StringView>(',', '\\') |
			                       transform(unescape<',', '\\'>))
				context_wrap_for_context(
				    ClientManager::instance().get_client(name).context());
...
const CommandDesc execute_keys_cmd = {
    ...
    make_context_wrap_params<3>(
        {{{"save-regs",
           {ArgCompleter{}, "restore all given registers after execution "
                            "(default: '/\"|^@:')"}},
          {"with-maps",
           {{}, "use user defined key mapping when executing keys"}},
...
const CommandDesc evaluate_commands_cmd = {
    ...
    make_context_wrap_params<3>(
        {{{"save-regs",
           {ArgCompleter{},
            "restore all given registers after execution (default: '')"}},
    ...
    [](const ParametersParser& parser, Context& context,
       const ShellContext& shell_context) {
	    context_wrap(
	        ...
	        [&](const ParametersParser& parser, Context& context) {
		        const bool no_hooks =
		            context.hooks_disabled() or parser.get_switch("no-hooks");
		        ...
		        if (parser.get_switch("verbatim"))
			        CommandManager::instance().execute_single_command(
			            parser | gather<Vector<String>>(), context,
			            shell_context);
		        else
			        CommandManager::instance().execute(join(parser, ' ', false),
...
const CommandDesc prompt_cmd = {
    ...
    ParameterDesc{
        {{"init", {ArgCompleter{}, "set initial prompt content"}},
         {"password",
          {{}, "Do not display entered text and clear reg after command"}},
         {"menu", {{}, "treat completions as the only valid inputs"}},
         ...
         {"shell-script-completion",
          {ArgCompleter{}, "use shell command completion for prompt"}},
         {"shell-script-candidates",
          {ArgCompleter{}, "use shell command completion for prompt"}},
         {"on-change",
          {ArgCompleter{}, "command to execute whenever the prompt changes"}},
         {"on-abort",
          {ArgCompleter{},
           "command to execute whenever the prompt is canceled"}}},
        ParameterDesc::Flags::None,
        2,
        2},
    CommandFlags::None,
    ...
    [](const ParametersParser& parser, Context& context,
       const ShellContext& shell_context) {
	    ...
	    const Completions::Flags completions_flags =
	        parser.get_switch("menu") ? Completions::Flags::Menu
	                                  : Completions::Flags::None;
	    PromptCompleterAdapter completer =
	        parse_completion_switch(parser, completions_flags);
	    ...
	    context.input_handler().prompt(
	        parser[0], initstr.str(), {}, context.faces()["Prompt"], flags, '_',
	        std::move(completer),
	        [command,
	         ...
	         sc = CapturedShellContext{shell_context}](
		        ...
		        auto remove_text =
		            OnScopeEnd([&] { sc.env_vars.erase("text"_sv); });
		        ...
		        try {
			        ...
			        context.print_status(
			            {error.what().str(), context.faces()["Error"]});
...
const CommandDesc on_key_cmd = {
    "on-key",
    nullptr,
    "on-key [<switches>] <command>: wait for next user key and then execute "
    "<command>, "
    "with key available in the `key` value",
    ParameterDesc{{{"mode-name", {ArgCompleter{}, "set mode name to use"}}},
                  ParameterDesc::Flags::None,
                  1,
                  1},
    CommandFlags::None,
    ...
    [](const ParametersParser& parser, Context& context,
       const ShellContext& shell_context) {
	    ...
	    context.input_handler().on_next_key(
	        parser.get_switch("mode-name").value_or("on-key"), KeymapMode::None,
	        [=](Key key, Context& context) mutable {
...
const CommandDesc info_cmd = {
    ...
    ParameterDesc{
        {{"anchor", {ArgCompleter{}, "set info anchoring <line>.<column>"}},
         {"style",
          {{arg_completer(Array{"above", "below", "menu", "modal"})},
           "set info style (above, below, menu, modal)"}},
         {"markup", {{}, "parse markup"}},
    ...
    [](const ParametersParser& parser, Context& context, const ShellContext&) {
	    ...
	    const InfoStyle style =
	        parser.get_switch("style")
	            .map([](StringView style) -> Optional<InfoStyle> {
		            if (style == "above")
			            return InfoStyle::InlineAbove;
		            if (style == "below")
			            return InfoStyle::InlineBelow;
		            if (style == "menu")
			            return InfoStyle::MenuDoc;
		            if (style == "modal")
			            return InfoStyle::Modal;
		            throw runtime_error(format("invalid style: '{}'", style));
	            })
	            .value_or(parser.get_switch("anchor") ? InfoStyle::Inline
	                                                  : InfoStyle::Prompt);
	    ...
	    const BufferCoord pos =
	        parser.get_switch("anchor")
	            .map([](StringView anchor) {
		            auto dot = find(anchor, '.');
		            if (dot == anchor.end())
			            throw runtime_error(
			                "expected <line>.<column> for anchor");
	    ...
	    if (parser.get_switch("markup"))
		    context.client().info_show(
		        parse_display_line(title, context.faces()),
		        parse_display_line_list(parser[0], context.faces()), pos,
		        style);
	    else
	    ...
	    for (size_t i = 0; i < parser.positional_count(); i += 2) {
		    if (i == 0 or i < parser.positional_count() - 1) {
			    try {
				    ...
				    shell_context_with_error->env_vars[StringView{"error"}] =
				        error.what().str();
			    }
		    } else
			    command_manager.execute(
...
static Completions complete_face(const Context& context, StringView prefix,
                                 ByteCount cursor_pos) {
	return {0_byte, cursor_pos,
	        complete(prefix, cursor_pos,
...
static String face_doc_helper(const Context& context,
                              CommandParameters params) {
	if (params.size() < 2)
		return {};
	try {
...
const CommandDesc set_face_cmd = {
    "set-face",
    "face",
    "set-face <scope> <name> <facespec>: set face <name> to <facespec> in "
    "<scope>\n"
    "\n"
    "facespec format is:\n"
    ...
    "if a base face is specified, colors and attributes are applied on top of "
    "it",
    ParameterDesc{{}, ParameterDesc::Flags::None, 3, 3},
    CommandFlags::None,
    face_doc_helper,
...
const CommandDesc set_register_cmd = {
    ...
    make_completer([](const Context& context, StringView prefix,
                      ByteCount cursor_pos) -> Completions {
	    return {0_byte, cursor_pos,
	            RegisterManager::instance().complete_register_name(prefix,
	                                                               cursor_pos)};
    }),
    [](const ParametersParser& parser, Context& context, const ShellContext&) {
...
const CommandDesc select_cmd = {
    ...
    ParameterDesc{
        {{"timestamp",
          {ArgCompleter{},
           "specify buffer timestamp at which those selections are valid"}},
         {"codepoint", {{}, "columns are specified in codepoints, not bytes"}},
         {"display-column",
          {{}, "columns are specified in display columns, not bytes"}}},
        ParameterDesc::Flags::SwitchesOnlyAtStart,
        1},
    CommandFlags::None,
    CommandHelper{},
    ...
    [](const ParametersParser& parser, Context& context, const ShellContext&) {
	    auto& buffer = context.buffer();
	    const size_t timestamp = parser.get_switch("timestamp")
	                                 .map(str_to_int_ifp)
	                                 .cast<size_t>()
	                                 .value_or(buffer.timestamp());
	    ...
	    context.selections_write_only() = selection_list_from_strings(
	        buffer, column_type, parser.positionals_from(0), timestamp, 0,
	        tabstop);
    }};
...
const CommandDesc change_directory_cmd = {
    "change-directory",
    "cd",
    "change-directory [<directory>]: change the server's working directory to "
    "<directory>, or the home directory if unspecified",
    single_optional_param,
    CommandFlags::None,
    CommandHelper{},
...
const CommandDesc rename_session_cmd = {
    ...
    [](const ParametersParser& parser, Context& ctx, const ShellContext&) {
	    ...
	    ctx.hooks().run_hook(
	        Hook::SessionRenamed,
	        format("{}:{}", old_name, Server::instance().session()), ctx);
    }};
...
void enter_user_mode(Context& context, NormalParams params, String mode_name,
                     KeymapMode mode, bool lock) {
	on_next_key_with_autoinfo(
	    context, format("user.{}", mode_name), KeymapMode::None,
	    [params, mode_name, mode, lock](Key key, Context& context) mutable {
		    ...
		    if (lock)
			    enter_user_mode(context, params, std::move(mode_name), mode,
			                    true);
	    },
...
const CommandDesc enter_user_mode_cmd = {
    "enter-user-mode",
    nullptr,
    "enter-user-mode [<switches>] <name> [<count>] [<register>]: enable <name> "
    "keymap mode for next key",
    ParameterDesc{
        {{"lock", {{}, "stay in mode until <esc> is pressed"}},
         {"count",
          {ArgCompleter{}, "count to forward to normal mode parameters"}},
         {"register",
          {ArgCompleter{}, "register to forward to normal mode parameters"}}},
        ParameterDesc::Flags::None,
        1,
        1},
    CommandFlags::None,
    ...
    [](const ParametersParser& parser, Context& context, const ShellContext&) {
	    ...
	    NormalParams params{
	        ...
	        parser.get_switch("register")
	            .map([](auto&& s) { return s.empty() ? '\0' : s[0]; })
	            .value_or('\0')};
...
const CommandDesc provide_module_cmd = {
    "provide-module",
    nullptr,
    "provide-module [<switches>] <name> <cmds>: declares a module <name> "
    "provided by <cmds>",
    ParameterDesc{{{"override", {{}, "allow overriding an existing module"}}},
                  ParameterDesc::Flags::None,
                  2,
                  2},
    CommandFlags::None,
    ...
    [](const ParametersParser& parser, Context& context, const ShellContext&) {
	    ...
	    if (cm.module_defined(module_name) and
	        not parser.get_switch("override"))
		    throw runtime_error(
		        format("module '{}' already defined", module_name));
...
const CommandDesc require_module_cmd = {
    ...
    CommandHelper{},
    make_completer(
        menu([](const Context&, StringView prefix, ByteCount cursor_pos) {
	        return CommandManager::instance().complete_module_name(
	            prefix.substr(0, cursor_pos));
        })),
    [](const ParametersParser& parser, Context& context, const ShellContext&) {
...
void register_commands() {
	...
	cm.register_command(
	    ...
	    "do nothing", {{}, ParameterDesc::Flags::IgnoreUnknownSwitches});
