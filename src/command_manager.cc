...
void CommandManager::register_command(String command_name, CommandFunc func,
                                      String docstring,
                                      ParameterDesc param_desc,
                                      CommandFlags flags, CommandHelper helper,
                                      CommandCompleter completer) {
	m_commands[command_name] = {std::move(func),       std::move(docstring),
	                            std::move(param_desc), flags,
	                            std::move(helper),     std::move(completer)};
}

void CommandManager::set_command_completer(StringView command_name,
                                           CommandCompleter completer) {
...
void CommandManager::register_module(String module_name, String commands) {
	...
	if (module != m_modules.end() and
	    module->value.state != Module::State::Registered)
...
void CommandManager::load_module(StringView module_name, Context& context) {
	...
	case Module::State::Loading:
		throw runtime_error(
		    format("module '{}' loaded recursively", module_name));
	case Module::State::Loaded:
		return;
	case Module::State::Registered:
	default:
		break;
	}
...
HashSet<String> CommandManager::loaded_modules() const {
	return m_modules | filter([](auto&& elem) {
		       return elem.value.state == Module::State::Loaded;
	       }) |
...
bool is_command_separator(char c) { return c == ';' or c == '\n'; }
...
ParseResult parse_quoted(ParseState& state, Delimiter delimiter) {
	static_assert(std::is_same_v<Delimiter, char> or
	              std::is_same_v<Delimiter, Codepoint>);
...
Token parse_percent_token(ParseState& state, bool throw_on_unterminated) {
	...
	const Codepoint opening_delimiter =
	    utf8::read_codepoint(state.pos, state.str.end());
	if (at_end or iswalpha(opening_delimiter)) {
		if (throw_on_unterminated)
			throw parse_error{
			    format("expected a string delimiter after '%{}'", type_name)};
		return {};
	}

	Token::Type type = token_type(type_name, throw_on_unterminated);

	constexpr struct CharPair {
		char opening;
		char closing;
		ParseResult (*parse_func)(ParseState&);
	} matching_pairs[] = {{'(', ')', parse_quoted_balanced<'(', ')'>},
	...
	if (auto it = find_if(matching_pairs,
	                      [=](const CharPair& cp) {
		...
		auto quoted = is_ascii ? parse_quoted(state, (char)opening_delimiter)
		                       : parse_quoted(state, opening_delimiter);
...
template <typename Target>
    requires(std::is_same_v<Target, Vector<String>> or
             std::is_same_v<Target, String>)
void expand_token(Token&& token, const Context& context,
                  const ShellContext& shell_context, Target& target) {
	constexpr bool single = std::is_same_v<Target, String>;
	auto set_target = [&](auto&& s) {
		if constexpr (single)
			target = std::move(s);
		else if constexpr (std::is_same_v<std::remove_cvref_t<decltype(s)>,
		                                  String>)
			target.push_back(std::move(s));
		else if constexpr (std::is_same_v<decltype(s), Vector<String>&&>)
			target.insert(target.end(), std::make_move_iterator(s.begin()),
	...
	case Token::Type::ShellExpand: {
		auto str = ShellManager::instance()
		               .eval(content, context, StringView{},
	...
	case Token::Type::RegisterExpand:
		if constexpr (single)
			...
			return set_target(
			    RegisterManager::instance()[content].get(context));
...
CommandParser::CommandParser(StringView command_line)
    : m_state{command_line, command_line.begin()} {}
...
Optional<Token> CommandParser::read_token(bool throw_on_unterminated) {
	...
	if (c == '"' or c == '\'') {
		...
		return Token{c == '"' ? Token::Type::Expand : Token::Type::RawQuoted,
		...
		return Token{
		    Token::Type::CommandSeparator, ++m_state.pos - line.begin(), {}};
	else {
...
String expand_impl(StringView str, const Context& context,
                   const ShellContext& shell_context, Postprocess postprocess) {
	...
	while (state) {
		if (*state.pos++ == '%') {
			if (state and *state.pos == '%') {
				...
				String token;
				expand_token(parse_percent_token(state, true), context,
				             shell_context, token);
...
void CommandManager::execute_single_command(CommandParameters params,
	...
	ProfileScope profile{
	    debug_flags, [&](std::chrono::microseconds duration) {
		    write_to_debug_buffer(
		        format("command {} took {} us", params[0], duration.count()));
	    }};

	command_it->value.func(
	    {{params.begin() + 1, params.end()}, command_it->value.param_desc},
	    context, shell_context);
}

void CommandManager::execute(StringView command_line, Context& context,
                             const ShellContext& shell_context) {
...
CommandManager::command_info(const Context& context,
	...
	if (tokens.empty() or (tokens.front().type != Token::Type::Raw and
...
Completions CommandManager::complete_command_name(const Context& context,
	...
	auto commands = m_commands | filter([](const CommandMap::Item& cmd) {
		                return not(cmd.value.flags & CommandFlags::Hidden);
	                }) |
...
Completions CommandManager::complete_module_name(StringView query) const {
	return {0, query.length(),
	        Kakoune::complete(query, query.length(),
	                          m_modules | filter([](auto&& item) {
		                          return item.value.state ==
		                                 Module::State::Registered;
	                          }) | transform(&ModuleMap::Item::key))};
}

static Completions complete_expansion(const Context& context, Token token,
                                      ByteCount start, ByteCount cursor_pos,
	...
	case Token::Type::ShellExpand:
		return offset_pos(shell_complete(context, token.content, pos_in_token),
		                  start);

	case Token::Type::ValExpand:
		return {start, cursor_pos,
		        ShellManager::instance().complete_env_var(token.content,
		                                                  pos_in_token)};
	...
	case Token::Type::FileExpand: {
		...
		return {start, cursor_pos,
		        complete_filename(token.content, ignored_files, pos_in_token,
...
static Completions complete_expand(const Context& context, StringView prefix,
                                   ByteCount start, ByteCount cursor_pos,
                                   ByteCount pos_in_token) {
	...
	while (state) {
		if (*state.pos++ == '%') {
			if (state and *state.pos == '%')
				...
				if (token.terminated)
					continue;
				if (token.type == Token::Type::Raw or
				    token.type == Token::Type::RawQuoted)
					return {};
				return complete_expansion(context, token, start + token.pos,
				                          cursor_pos, pos_in_token - token.pos);
			}
		}
	}
	return {};
	...
	if (token_type == Token::Type::Raw) {
		...
		for (auto& candidate : completions.candidates) {
			...
			if ((at_token_start and candidate.substr(0_byte, 1_byte) == "%") or
			    any_of(candidate,
			           [&](auto c) { return contains(to_escape, c); }))
				candidate = at_token_start ? quote(candidate)
				                           : escape(candidate, to_escape, '\\');
		}
...
Completions CommandManager::Completer::operator()(const Context& context,
	...
	if (tokens.size() == 1 and (token.type == Token::Type::Raw or
	                            token.type == Token::Type::RawQuoted)) {
		return offset_pos(
		    requote(CommandManager::instance().complete_command_name(context,
		                                                             prefix),
		            token.type),
		    start);
	}
	...
	case Token::Type::ValExpand:
	case Token::Type::FileExpand:
		return complete_expansion(context, token, start, cursor_pos,
		                          pos_in_token);
	...
	case Token::Type::RawQuoted: {
		...
		auto raw_params = tokens | skip(1) | transform(&Token::content) |
		                  gather<Vector<String>>();
		ParametersParser parser{raw_params, command.param_desc, true};

		switch (parser.state()) {
		case ParametersParser::State::Switch: {
			auto switches = Kakoune::complete(
			    token.content.substr(1_byte), pos_in_token,
			...
			return switches.empty()
			           ? Completions{}
			           : Completions{start + 1, cursor_pos, std::move(switches),
			                         Completions::Flags::Menu};
		}
		case ParametersParser::State::SwitchArgument: {
			const auto& switch_desc = command.param_desc.switches.get(
			    raw_params.at(raw_params.size() - 2).substr(1_byte));
			if (not *switch_desc.arg_completer)
				return Completions{};
			return offset_pos(
			    requote((*switch_desc.arg_completer)(context, raw_params.back(),
			                                         pos_in_token),
			            token.type),
			    start);
		}
		...
		return offset_pos(
		    requote(m_command_completer(context, params, index, pos_in_token),
		            token.type),
		    start);
	}
	case Token::Type::Expand:
		return complete_expand(context, token.content, start, cursor_pos,
		                       pos_in_token);
	default:
		break;
	}
...
Completions CommandManager::NestedCompleter::operator()(
    const Context& context, CommandParameters params, size_t token_to_complete,
    ByteCount pos_in_token) {
	StringView prefix = params[token_to_complete].substr(0, pos_in_token);
	if (token_to_complete == 0)
		return CommandManager::instance().complete_command_name(context,
		                                                        prefix);
	...
	return m_command_completer
	           ? m_command_completer(context, params.subrange(1),
	                                 token_to_complete - 1, pos_in_token)
	           : Completions{};
}

UnitTest test_command_parsing{[] {
	auto check_quoted = [](StringView str, bool terminated,
	                       StringView content) {
	...
	auto check_balanced = [](StringView str, bool terminated,
	                         StringView content) {
