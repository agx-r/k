...
Buffer::Buffer(String name, Flags flags, BufferLines lines, ByteOrderMark bom,
               EolFormat eolformat, FinalEol finaleol, FsStatus fs_status)
    : Scope{GlobalScope::instance()},
      m_filename{(flags & Flags::File) ? real_path(parse_filename(name))
                                       : String{}},
      m_display_name{(flags & Flags::File) ? compact_path(m_filename)
                                           : std::move(name)},
      m_flags{flags | Flags::NoUndo}, m_history{{HistoryId::Invalid}},
      m_history_id{HistoryId::First}, m_last_save_history_id{HistoryId::First},
...
void Buffer::on_registered() {
	...
	run_hook_in_own_context(
	    Hook::BufCreate, (m_flags & Flags::File) ? m_filename : m_display_name);

	if (m_flags & Flags::File) {
		if (m_flags & Buffer::Flags::New)
			run_hook_in_own_context(Hook::BufNewFile, (m_flags & Flags::File)
			                                              ? m_filename
			                                              : m_display_name);
		else {
			kak_assert(m_fs_status.timestamp != InvalidTime);
			run_hook_in_own_context(Hook::BufOpenFile, (m_flags & Flags::File)
			                                               ? m_filename
			                                               : m_display_name);
		}
	}
...
void Buffer::on_unregistered() {
	...
	run_hook_in_own_context(
	    Hook::BufClose, (m_flags & Flags::File) ? m_filename : m_display_name);
}
...
BufferCoordAndTarget Buffer::offset_coord(BufferCoordAndTarget coord,
	...
	return {line,
	        std::min(max_byte,
	                 get_byte_to_column(*this, tabstop, {line, final_column})),
	        column};
}
...
void Buffer::reload(BufferLines lines, ByteOrderMark bom, EolFormat eolformat,
                    FinalEol finaleol, FsStatus fs_status) {
	...
	if (not record_undo) {
		...
		Vector<Diff> diff;
		for_each_diff(
		    m_lines.begin(), m_lines.size(), lines.begin(), lines.size(),
		    [&diff](DiffOp op, int len) { diff.push_back({op, len}); },
		...
		for (auto [op, len] : diff) {
			...
			if (op == DiffOp::Keep) {
				...
				for (LineCount line = 0; line < len; ++line)
					m_current_undo_group.push_back({Modification::Insert,
					                                cur_line + line,
					                                *(new_it + (int)line)});
				...
				if (read_it != write_it) {
					auto count =
					    std::min(len, static_cast<int>(read_it - write_it));
				...
				for (LineCount line = len - 1; line >= 0; --line)
					m_current_undo_group.push_back({Modification::Erase,
					                                cur_line + line,
					                                *(read_it + (size_t)line)});
...
bool Buffer::undo(size_t count) {
	...
	while (count-- != 0 and
	       current_history_node().parent != HistoryId::Invalid) {
...
bool Buffer::redo(size_t count) {
	...
	while (count-- != 0 and
	       current_history_node().redo_child != HistoryId::Invalid) {
...
BufferRange Buffer::do_insert(BufferCoord pos, StringView content) {
	...
	for (ByteCount i = 0; i < content.length(); ++i) {
		if (content[i] == '\n') {
			...
			new_lines.push_back(start == 0 ? StringData::create(prefix, line)
			                               : StringData::create(line));
			start = i + 1;
		}
	}
	if (start == 0)
...
BufferCoord Buffer::do_erase(BufferCoord begin, BufferCoord end) {
	...
	auto new_line = (not prefix.empty() or not suffix.empty())
	                    ? StringData::create(prefix, suffix)
	                    : StringDataPtr{};
...
BufferRange Buffer::insert(BufferCoord pos, StringView content) {
	...
	if (not(m_flags & Flags::NoUndo))
		m_current_undo_group.push_back(
		    {Modification::Insert, pos, real_content});
...
BufferRange Buffer::replace(BufferCoord begin, BufferCoord end,
                            StringView content) {
	throw_if_read_only();
...
bool Buffer::is_modified() const {
	return m_flags & Flags::File and (m_history_id != m_last_save_history_id or
	                                  not m_current_undo_group.empty());
}
...
BufferCoord Buffer::advance(ArrayView<const StringDataPtr> lines,
                            BufferCoord coord, ByteCount count) {
	if (count > 0) {
...
BufferCoord Buffer::char_prev(BufferCoord coord) const {
	...
	auto line = m_lines[coord.line];
	auto column = (int)(utf8::character_start(line.begin() + coord.column - 1,
	                                          line.begin()) -
	                    line.begin());
	return {coord.line, column};
}
...
String Buffer::debug_description() const {
	...
	const size_t additional_size =
	    accumulate(m_history, 0,
	               [](size_t s, auto&& history) {
		               return sizeof(history) +
		                      history.undo_group.size() * sizeof(Modification) +
		                      s;
	               }) +
	...
	return format(
	    ...
	    is_modified() ? "Modified " : "", content_size, additional_size);
}

UnitTest test_buffer{[]() {
	auto make_lines = [](auto&&... lines) {
		return BufferLines{StringData::create(lines)...};
	};
