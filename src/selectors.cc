...
select_to_next_word<WordType::Word>(const Context&, const Selection&);
template Optional<Selection>
select_to_next_word<WordType::WORD>(const Context&, const Selection&);
...
template Optional<Selection>
select_to_next_word_end<WordType::Word>(const Context&, const Selection&);
template Optional<Selection>
select_to_next_word_end<WordType::WORD>(const Context&, const Selection&);
...
template Optional<Selection>
select_to_previous_word<WordType::Word>(const Context&, const Selection&);
template Optional<Selection>
select_to_previous_word<WordType::WORD>(const Context&, const Selection&);
...
template Optional<Selection>
select_word<WordType::Word>(const Context&, const Selection&, int, ObjectFlags);
template Optional<Selection>
select_word<WordType::WORD>(const Context&, const Selection&, int, ObjectFlags);
...
find_opening(Iterator pos, const Container& container, const Regex& opening,
             const Regex& closing, int level, bool nestable) {
	MatchResults<Iterator> res;
	// When on the token of a non-nestable block, we want to consider it opening
	if (nestable and
	    backward_regex_search(container.begin(), pos, container.begin(),
	                          container.end(), res, closing,
	                          RegexExecFlags::None,
	                          EventManager::handle_urgent_events) and
	    res[0].second == pos)
		pos = res[0].first;

	using RegexIt =
	    RegexIterator<Iterator, RegexMode::Backward, const Regex, void (*)()>;
	for (auto&& match : RegexIt{container.begin(), pos, container.begin(),
	                            container.end(), opening, RegexExecFlags::None,
	                            EventManager::handle_urgent_events}) {
		if (nestable) {
			for (auto m [[maybe_unused]] :
			     RegexIt{match[0].second, pos, container.begin(),
			             container.end(), closing, RegexExecFlags::None,
			             EventManager::handle_urgent_events})
				++level;
		}
...
template <typename Iterator, typename Container>
Optional<std::pair<Iterator, Iterator>>
find_closing(Iterator pos, const Container& container, const Regex& opening,
             const Regex& closing, int level, bool nestable) {
	for (auto match : RegexIterator{
	         pos, container.end(), container.begin(), container.end(), closing,
	         RegexExecFlags::None, EventManager::handle_urgent_events}) {
		if (nestable) {
			for (auto m [[maybe_unused]] :
			     RegexIterator{pos, match[0].first, container.begin(),
			                   container.end(), opening, RegexExecFlags::None,
			                   EventManager::handle_urgent_events})
				++level;
		}
...
Optional<std::pair<Iterator, Iterator>>
find_surrounding(const Container& container, Iterator pos, const Regex& opening,
                 const Regex& closing, ObjectFlags flags, int level) {
	auto empty = [](const std::pair<Iterator, Iterator>& m) {
	...
	if (flags & ObjectFlags::ToBegin) {
		if (auto res = find_opening(first + 1, container, opening, closing,
			...
			return {};
	           ...
	           regex_search(pos, container.end(), container.begin(),
	                        container.end(), res, opening, RegexExecFlags::None,
	                        EventManager::handle_urgent_events) and
	           res[0].first == pos) // Skip opening match if pos lies on it
		last = empty(res[0]) ? std::next(res[0].second) : res[0].second;

	if (flags & ObjectFlags::ToEnd) {
		if (auto res = find_closing(last, container, opening, closing, level,
		                            nestable))
			last = utf8::previous((flags & ObjectFlags::Inner) ? res->first
			                                                   : res->second,
			                      container.begin());
		else
			return {};
...
Optional<Selection> select_surrounding(const Context& context,
	...
	if (res and not(flags & ObjectFlags::Inner) and
	    (res->first.coord() == selection.min() or
	     not(flags & ObjectFlags::ToBegin)) and
	    (res->second.coord() == selection.max() or
	     not(flags & ObjectFlags::ToEnd)))
		...
		if (flags & ObjectFlags::ToBegin) {
			...
			while (first != buffer.begin()) {
				...
				if (not is_horizontal_blank(cur))
					saw_non_blank = true;
				if (is_eol(prev) and is_eol(cur) and
				    first + 1 != buffer.end()) {
					++first;
					break;
		...
		if (flags & ObjectFlags::ToEnd) {
			while (last != buffer.end()) {
				...
				if (is_end_of_sentence(cur) or
				    (is_eol(cur) and
				     (last + 1 == buffer.end() or is_eol(*(last + 1)))))
					break;
				++last;
			}
...
Optional<Selection> select_paragraph(const Context& context,
	...
	for (int i = 0; i <= count; ++i) {
		if (not(flags & ObjectFlags::ToEnd) and
		    first.coord() > BufferCoord{0, 1} and is_eol(*(first - 1)) and
		    first - 1 != buffer.begin() and is_eol(*(first - 2)))
			--first;
		else if ((flags & ObjectFlags::ToEnd) and first != buffer.begin() and
		         (first + 1) != buffer.end() and is_eol(*(first - 1)) and
		         is_eol(*first))
			++first;
		if (i == 0)
			last = first;
		...
		if (flags & ObjectFlags::ToEnd) {
			if (last != buffer.end() and is_eol(*last))
				++last;
			while (last != buffer.end()) {
				if (last != buffer.begin() and is_eol(*last) and
				    is_eol(*(last - 1))) {
					if (not(flags & ObjectFlags::Inner))
...
Optional<Selection> select_indent(const Context& context,
	...
	auto get_current_indent = [&](const Buffer& buffer, LineCount line,
	                              int tabstop) {
...
Vector<Selection> for_each_sel(const Context& context, auto&& callback) {
	...
	for (auto& sel : context.selections())
		callback(res, buffer.iterator_at(sel.min()),
		         buffer.iterator_at(buffer.char_next(sel.max())));
	if (res.empty())
...
template <WordType word_type>
Vector<Selection> select_nested_words(const Context& context, int count,
                                      ObjectFlags flags) {
	auto is_word = [extra_word_chars =
	                    get_extra_word_chars(context)](Codepoint c) {
		return Kakoune::is_word<word_type>(c, extra_word_chars);
	};
	auto is_not_word = [&](Codepoint c) { return not is_word(c); };

	return for_each_sel(context, [&](Vector<Selection>& res, BufferIterator beg,
	                                 BufferIterator end) {
		for (auto it = Utf8Iterator{beg, context.buffer()}; it != end;) {
			...
			res.push_back({start.base().coord(),
			               context.buffer().char_prev(it.base().coord())});
		}
	});
}
template Vector<Selection>
select_nested_words<Word>(const Context& context, int count, ObjectFlags flags);
template Vector<Selection>
select_nested_words<WORD>(const Context& context, int count, ObjectFlags flags);

Vector<Selection> select_nested_numbers(const Context& context, int count,
                                        ObjectFlags flags) {
	auto is_digit = [](char c) { return c >= '0' and c <= '9'; };
	auto is_number = [&](char c) {
		return is_digit(c) or (not(flags & ObjectFlags::Inner) and c == '.');
	};

	return for_each_sel(context, [&](Vector<Selection>& res, BufferIterator it,
	                                 BufferIterator end) {
		while (it != end) {
			if (not skip_while(it, end, [&](char c) {
				    return not(c == '-' or is_number(c));
			    }))
				break;
			...
			if (it == start)
				...
				res.push_back(
				    {start.coord(), context.buffer().char_prev(it.coord())});
		}
	});
}

Vector<Selection> select_nested_sentences(const Context& context, int count,
                                          ObjectFlags flags) {
	auto is_end_of_sentence = [](char c) {
		return c == '.' or c == ';' or c == '!' or c == '?';
	};
...
Vector<Selection> select_nested_paragraphs(const Context& context, int count,
                                           ObjectFlags flags) {
	...
	return for_each_sel(context, [&](Vector<Selection>& res, BufferIterator it,
		...
		while (it != end) {
			auto start =
			    std::find_if(it, end, [&](char c) { return not is_eol(c); });
...
Vector<Selection> select_nested_whitespaces(const Context& context, int count,
                                            ObjectFlags flags) {
	auto is_whitespace = [&](char c) {
		return c == ' ' or c == '\t' or
		       (not(flags & ObjectFlags::Inner) and c == '\n');
	...
	return for_each_sel(context, [&](Vector<Selection>& res, BufferIterator it,
	                                 BufferIterator end) {
		while (it != end) {
			if (not skip_while(it, end,
			                   [&](char c) { return not is_whitespace(c); }))
				break;
			auto start = it;
...
Vector<Selection> select_nested_indents(const Context& context, int count,
                                        ObjectFlags flags) {
	throw runtime_error("nested indents are not implemented");
}

Vector<Selection> select_nested_arguments(const Context& context, int count,
                                          ObjectFlags flags) {
	auto is_whitespace = [&](char c) {
		return c == ' ' or c == '\t' or c == '\n';
	};
		...
		while (it != end) {
			...
			case ';':
				if (level == 0) {
					res.push_back(
					    {start.coord(),
					     inner ? buffer.char_prev(it.coord()) : it.coord()});
					++it;
...
Vector<Selection> regex_select_nested(const Context& context,
                                      const Regex& opening,
                                      const Regex& closing, int count,
                                      ObjectFlags flags) {
	...
	return for_each_sel(context, [&](Vector<Selection>& res, BufferIterator beg,
	                                 BufferIterator end) {
		RegexIterator opening_matches{beg,
		                              end,
		                              buffer.begin(),
		                              buffer.end(),
		                              opening,
		                              RegexExecFlags::None,
		                              EventManager::handle_urgent_events};
		RegexIterator closing_matches{beg,
		                              end,
		                              buffer.begin(),
		                              buffer.end(),
		                              closing,
		                              RegexExecFlags::None,
		                              EventManager::handle_urgent_events};
		auto opening_it = opening_matches.begin(),
		     closing_it = closing_matches.begin();
		auto opening_end = opening_matches.end(),
		     closing_end = closing_matches.end();

		Optional<BufferIterator> start;
		int level = -count - 1;
		while (opening_it != opening_end and closing_it != closing_end) {
			while (opening_it != opening_end and
			       (closing_it == closing_end or
			        (*opening_it)[0].first < (*closing_it)[0].first)) {
				if (++level == 0)
					start = inner ? (*opening_it)[0].second
					              : (*opening_it)[0].first;
				++opening_it;
			}
			while (closing_it != closing_end and
			       (opening_it == opening_end or
			        (*closing_it)[0].first < (*opening_it)[0].first)) {
				if (level-- == 0 and start) {
					auto end_coord =
					    buffer.char_prev((inner ? (*closing_it)[0].first
					                            : (*closing_it)[0].second)
					                         .coord());
...
Vector<Selection> regex_select_nested(const Context& context,
	...
	return for_each_sel(context, [&](Vector<Selection>& res, BufferIterator beg,
		...
		for (auto m : RegexIterator{beg, end, buffer.begin(), buffer.end(),
		                            delimiter, RegexExecFlags::None,
		                            EventManager::handle_urgent_events}) {
			if (not start)
				start = inner ? m[0].second : m[0].first;
			else {
				auto end_coord = buffer.char_prev(
				    (inner ? m[0].first : m[0].second).coord());
				if (start->coord() <= end_coord)
					res.push_back({start->coord(), end_coord});
				start.reset();
			}
...
static bool find_next(const Buffer& buffer, const BufferIterator& pos,
	...
	if (pos != buffer.end() and
	    regex_search(pos, buffer.end(), buffer.begin(), buffer.end(), matches,
	                 ex, match_flags(buffer, pos, buffer.end()),
	                 EventManager::handle_urgent_events))
		return true;
	wrapped = true;
...
Vector<Selection> select_matches(const Buffer& buffer,
                                 ConstArrayView<Selection> selections,
                                 const Regex& regex, int capture_idx) {
	...
	ThreadedRegexVM<BufferIterator, RegexMode::Forward | RegexMode::Search> vm{
	    *regex.impl()};
	for (auto& sel : selections) {
		auto sel_beg = buffer.iterator_at(sel.min());
		auto sel_end = utf8::next(buffer.iterator_at(sel.max()), buffer.end());

		for (auto&& match : RegexIterator{sel_beg, sel_end, vm,
...
Vector<Selection> split_on_matches(const Buffer& buffer,
                                   ConstArrayView<Selection> selections,
                                   const Regex& regex, int capture_idx) {
	...
	for (auto& sel : selections) {
		...
		for (auto&& match : RegexIterator{
		         begin, sel_end, vm, match_flags(buffer, begin, sel_end)}) {
			auto capture = match[capture_idx];
