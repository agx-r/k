...
String left_pad(StringView str, ColumnCount size, Codepoint c) {
	return String(c, std::max(0_col, size - str.column_length())) +
	       str.substr(0, size);
}

String right_pad(StringView str, ColumnCount size, Codepoint c) {
	return str.substr(0, size) +
	       String(c, std::max(0_col, size - str.column_length()));
}
...
UnitTest test_string{[]() {
	...
	kak_assert(format("Youhou {1} {} '{0:4}' {2:04} \\{}", 10, "hehe", 5) ==
	           "Youhou hehe 5 '  10' 0005 {}");
