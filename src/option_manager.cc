...
struct option_not_found : public runtime_error {
	option_not_found(StringView name)
	    : runtime_error(format(
	          "option not found: '{}'. Use declare-option first", name)) {}
};
...
void OptionManager::unset_option(StringView name) {
	...
	if (it != m_options.end()) {
		...
		GlobalScope::instance().option_registry().move_to_trash(
		    std::move(it->value));
...
CandidateList
OptionsRegistry::complete_option_name(StringView prefix,
