...
public:
	...
	template <typename U> struct DecayOptionalImpl {
		using Type = U;
	};
	template <typename U> struct DecayOptionalImpl<Optional<U>> {
		using Type = typename DecayOptionalImpl<U>::Type;
	};
	template <typename U>
	using DecayOptional = typename DecayOptionalImpl<U>::Type;
	...
	template <typename U> auto cast() const -> Optional<U> {
		if (not m_valid)
			return {};
		return {(U)m_value};
	}
	...
	template <typename U> T value_or_compute(U&& compute_func) const {
		return m_valid ? m_value : compute_func();
	}
