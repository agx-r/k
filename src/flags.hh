...
template <typename Flags> constexpr bool with_bit_ops(Meta::Type<Flags>) {
	return false;
}
...
template <typename Flags> using UnderlyingType = std::underlying_type_t<Flags>;

template <WithBitOps Flags> constexpr Flags operator|(Flags lhs, Flags rhs) {
...
template <typename Flags> struct TestableFlags {
	Flags value;
	...
	constexpr bool
	operator==(const TestableFlags<Flags>& other) const = default;
};
...
template <WithBitOps Flags> constexpr Flags operator~(Flags lhs) {
