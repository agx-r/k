...
private:
	...
	template <typename T> void write_field(const T& val) {
	...
	void write_field(const String& str) { write_field(StringView{str}); }

	template <typename T> void write_field(ConstArrayView<T> view) {
	...
	template <typename T> void write_field(const Optional<T>& val) {
	...
	void write_field(const DisplayLine& line) { write_field(line.atoms()); }
...
private:
	...
	template <typename T>
	struct Reader<ArrayView<T>>
	    : Reader<Vector<std::remove_cv_t<T>, MemoryDomain::Undefined>> {};
	...
	template <typename T> struct Reader<Optional<T>> {
...
public:
	...
	template <typename T> auto read() { return Reader<T>::read(*this); }
	...
	~MsgReader() { m_ancillary_fd.map(close); }
...
private:
	void read_from_socket(int sock, size_t size) {
		...
		if (cmsghdr* cmsg = CMSG_FIRSTHDR(&msg);
		    cmsg and cmsg->cmsg_level == SOL_SOCKET and
		    cmsg->cmsg_type == SCM_RIGHTS and
...
template <> struct MsgReader::Reader<String> {
...
template <> struct MsgReader::Reader<Color> {
...
template <> struct MsgReader::Reader<DisplayAtom> {
...
template <> struct MsgReader::Reader<DisplayLine> {
	static DisplayLine read(MsgReader& reader) {
		return {Reader<Vector<DisplayAtom>>::read(reader)};
	}
};

template <> struct MsgReader::Reader<DisplayBuffer> {
...
RemoteUI::RemoteUI(int socket, DisplayCoord dimensions)
    : m_socket_watcher(
          socket, FdEvents::Read | FdEvents::Write, EventMode::Urgent,
          [this](FDWatcher& watcher, FdEvents events, EventMode) {
	          const int sock = watcher.fd();
	          try {
		          if (events & FdEvents::Write and
		              send_data(sock, m_send_buffer))
			          m_socket_watcher.events() &= ~FdEvents::Write;
...
void RemoteUI::menu_hide() { send_message(MessageType::MenuHide); }

void RemoteUI::info_show(const DisplayLine& title,
...
void RemoteUI::info_hide() { send_message(MessageType::InfoHide); }

void RemoteUI::draw(const DisplayBuffer& display_buffer,
                    DisplayCoord cursor_pos, const Face& default_face,
                    const Face& padding_face, ColumnCount widget_columns) {
	send_message(MessageType::Draw, display_buffer, cursor_pos, default_face,
	             padding_face, widget_columns);
}
...
void RemoteUI::draw_status(const DisplayLine& prompt,
                           ...
                           const Face& default_face, StatusStyle style) {
	send_message(MessageType::DrawStatus, prompt, content, cursor_pos,
	             mode_line, default_face, style);
}
...
void RemoteUI::exit(int status) { send_message(MessageType::Exit, status); }
...
const String& session_directory() {
	static String session_dir = [] {
		...
		if (not xdg_runtime_dir.empty()) {
			if (struct stat st; stat(xdg_runtime_dir.zstr(), &st) == 0 and
			                    st.st_uid == geteuid())
				return format("{}/kakoune", xdg_runtime_dir);
			else
				write_to_debug_buffer("XDG_RUNTIME_DIR does not exist or not "
				                      "owned by current user, using tmpdir");
		}
...
RemoteClient::RemoteClient(StringView session, StringView name,
                           UniquePtr<UserInterface>&& ui, int pid,
...

	{
		...
		msg.write(pid, name, init_command, init_coord, m_ui->dimensions(),
		          env_vars);
	}
	...
	m_socket_watcher.reset(new FDWatcher{
	    sock, FdEvents::Read | FdEvents::Write, EventMode::Urgent,
	    [this, reader = MsgReader{}](FDWatcher& watcher, FdEvents events,
		    ...
		    auto exec = [&]<typename... Args>(
		                    void (UserInterface::*method)(Args...)) {
			    struct Impl // Use a constructor to ensure left-to-right
			                // parameter evaluation
			    {
				    Impl(UserInterface& ui,
				         void (UserInterface::*method)(Args...), Args... args) {
					    (ui.*method)(std::forward<Args>(args)...);
				    }
			    };
			    Impl{*m_ui, method,
			         reader.read<std::remove_cvref_t<Args>>()...};
		    };

		    while (events & FdEvents::Read and not reader.ready() and
		           fd_readable(sock)) {
	...
	void handle_available_input(EventMode mode) {
		...
		try {
			...
			case MessageType::Connect: {
				...
				auto env_vars =
				    m_reader
				        .read<HashMap<String, String, MemoryDomain::EnvVars>>();

				if (auto stdin_fd = m_reader.ancillary_fd())
					create_fifo_buffer(generate_buffer_name("*stdin-{}*"),
...
Server::Server(String session_name, bool is_daemon)
	...
	m_listener.reset(new FDWatcher{listen_sock, FdEvents::Read,
	                               EventMode::Urgent, accepter});
}
