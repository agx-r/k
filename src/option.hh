...
template <typename P, typename T> struct PrefixedList {
	P prefix;
	Vector<T, MemoryDomain::Options> list;

	friend bool operator==(const PrefixedList& lhs,
	                       const PrefixedList& rhs) = default;
};

template <typename T> using TimestampedList = PrefixedList<size_t, T>;
