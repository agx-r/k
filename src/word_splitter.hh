	...
	struct Iterator {
		...
		Iterator& operator++() {
			...
			do {
				...
				while (it != end and
				       not is_word(utf8::read_codepoint(it, end), extra_chars))
					m_word_begin = it;

				m_word_end = it;
				while (it != end and
				       is_word(utf8::read_codepoint(it, end), extra_chars))
					m_word_end = it;
			} while (m_word_begin != end and
			         (m_word_end - m_word_begin) > max_word_len);
