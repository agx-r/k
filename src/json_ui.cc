...
String to_json(Color color) {
	if (color.color == Kakoune::Color::RGB) {
		char buffer[10];
		format_to(buffer, R"("#{:02}{:02}{:02}")", hex(color.r), hex(color.g),
		          hex(color.b));
		return buffer;
	}
...
String to_json(Face face) {
	return format(
	    R"(\{ "fg": {}, "bg": {}, "underline": {}, "attributes": {} })",
	    to_json(face.fg), to_json(face.bg), to_json(face.underline),
	    to_json(face.attributes));
}
...
String to_json(ColumnCount column) { return format("{}", column); }
...
String to_json(StatusStyle style) {
	switch (style) {
	case StatusStyle::Status:
		return R"("status")";
	case StatusStyle::Command:
		return R"("command")";
	case StatusStyle::Search:
		return R"("search")";
	case StatusStyle::Prompt:
		return R"("prompt")";
	}
	return "";
}
...
template <typename... Args> void rpc_call(StringView method, Args&&... args) {
	auto q =
...
void JsonUI::draw(const DisplayBuffer& display_buffer, DisplayCoord cursor_pos,
	...
	rpc_call("draw", display_buffer.lines(), cursor_pos, default_face,
	         padding_face, widget_columns);
}

void JsonUI::draw_status(const DisplayLine& prompt, const DisplayLine& content,
                         const ColumnCount cursor_pos,
                         const DisplayLine& mode_line, const Face& default_face,
                         StatusStyle style) {
	rpc_call("draw_status", prompt, content, cursor_pos, mode_line,
	         default_face, style);
}
...
void JsonUI::eval_json(const Value& json) {
	...
	if (json_it == object.end() or not json_it->value.is_a<String>() or
	...
	if (method == "keys") {
		...
		m_on_key({Key::Modifiers::MousePos,
		          encode_coord({params[0].as<int>(), params[1].as<int>()})});
