...
BufferIterator get_iterator(const Buffer& buffer, BufferCoord coord) {
	...
	if (not buffer.is_end(coord) and
...
StringView DisplayAtom::content() const {
	...
	case Range: {
		...
		else if (m_range.begin.line + 1 == m_range.end.line and
		         m_range.end.column == 0)
			return line.substr(m_range.begin.column);
		break;
	}
	case Text:
...
DisplayLine::iterator DisplayLine::split(iterator it, ColumnCount count) {
	...
	if (it->type() == DisplayAtom::Text or
	    it->type() == DisplayAtom::ReplacedRange) {
		DisplayAtom atom = *it;
	...
	auto pos = utf8::advance(get_iterator(it->buffer(), it->begin()),
	               ...
	               .coord();
	if (pos == it->begin()) // Can happen if we try to split in the middle of a
	                        // multi-column codepoint
		return m_atoms.insert(it, {it->buffer(), {pos, pos}, it->face});
	if (pos == it->end())
		return std::prev(m_atoms.insert(std::next(it),
		                                {it->buffer(), {pos, pos}, it->face}));
	return split(it, pos);
}
...
DisplayLine DisplayLine::extract(iterator beg, iterator end) {
	...
	DisplayLine extracted{
	    AtomList(std::move_iterator(beg), std::move_iterator(end))};
...
bool DisplayLine::trim_from(ColumnCount first_col, ColumnCount front,
...
void DisplayLine::compute_range(bool preserve_if_no_atoms) {
	auto first = find_if(
	    m_atoms, [](const auto& atom) { return atom.has_buffer_range(); });
	if (first == m_atoms.end())
		m_range = preserve_if_no_atoms and m_range != init_range
		              ? m_range
		              : BufferRange{{0, 0}, {0, 0}};
	else {
		auto last = std::find_if(
		    m_atoms.rbegin(), std::reverse_iterator(first + 1),
		    [](const auto& atom) { return atom.has_buffer_range(); });
...
DisplayLine parse_display_line(StringView line, Face& face,
	...
	for (auto it = line.begin(), end = line.end(); it != end; ++it) {
		...
		if (c == '\n' or
		    c == '\t') // line breaks and tabs are forbidden, replace with space
		{
...
DisplayLineList
parse_display_line_list(StringView content, const FaceRegistry& faces,
                        const HashMap<String, DisplayLine>& builtins) {
