...
public:
	...
	explicit Regex(StringView re,
	               RegexCompileFlags flags = RegexCompileFlags::None);
...
template <typename Iterator> struct MatchResults {
	struct SubMatch : std::pair<Iterator, Iterator> {
		...
		SubMatch(Iterator begin, Iterator end)
		    : std::pair<Iterator, Iterator>{begin, end},
		      matched{static_cast<bool>(begin)} {}
	...
	struct iterator {
		...
		using It =
		    typename Vector<Iterator, MemoryDomain::Regex>::const_iterator;
	...
	MatchResults(Vector<Iterator, MemoryDomain::Regex> values)
	...
	SubMatch operator[](size_t i) const {
		return i * 2 < m_values.size()
		           ? SubMatch{m_values[i * 2], m_values[i * 2 + 1]}
		           : SubMatch{};
	}

	friend bool operator==(const MatchResults& lhs,
	                       const MatchResults& rhs) = default;

	void swap(MatchResults& other) { m_values.swap(other.m_values); }
...
bool regex_match(It begin, It end, const Regex& re, IdleFunc&& idle_func = {}) {
	ThreadedRegexVM<It, RegexMode::Forward | RegexMode::AnyMatch |
	                        RegexMode::NoSaves>
	    vm{*re.impl()};
...
bool regex_match(It begin, It end, MatchResults<It>& res, const Regex& re,
	...
	if (vm.exec(begin, end, begin, end, RegexExecFlags::None, idle_func)) {
		std::copy(vm.captures().begin(), vm.captures().end(),
		          std::back_inserter(res.values()));
		return true;
	}
	return false;
}
...
bool regex_search(It begin, It end, It subject_begin, It subject_end,
	...
	if (vm.exec(begin, end, subject_begin, subject_end, flags, idle_func)) {
		std::move(vm.captures().begin(), vm.captures().end(),
		          std::back_inserter(res.values()));
		return true;
	}
	return false;
...
bool backward_regex_search(It begin, It end, It subject_begin, It subject_end,
                           ...
                           RegexExecFlags flags = RegexExecFlags::None,
                           IdleFunc&& idle_func = {}) {
	return regex_search<It, RegexMode::Backward>(
	    begin, end, subject_begin, subject_end, res, re, flags, idle_func);
}
...
struct RegexIterator {
	...
	RegexIterator(Iterator begin, Iterator end, Iterator subject_begin,
	              Iterator subject_end, VmArg& vm_arg,
	              RegexExecFlags flags = RegexExecFlags::None,
	              IdleFunc idle_func = {})
	    : m_vm{make_vm(vm_arg)}, m_next_pos{forward ? begin : end},
	      m_begin{std::move(begin)}, m_end{std::move(end)},
	      m_subject_begin{std::move(subject_begin)},
	      m_subject_end{std::move(subject_end)}, m_flags{flags},
	      m_idle_func{idle_func} {}

	RegexIterator(const Iterator& begin, const Iterator& end, VmArg& vm_arg,
	              RegexExecFlags flags = RegexExecFlags::None,
	              IdleFunc idle_func = {})
...
private:
	bool next() {
		...
		if (not m_vm.exec(forward ? m_next_pos : m_begin,
		                  forward ? m_end : m_next_pos, m_subject_begin,
		                  m_subject_end, m_flags | additional_flags,
		                  m_idle_func))
			return false;

		m_results.values().clear();
		std::move(m_vm.captures().begin(), m_vm.captures().end(),
		          std::back_inserter(m_results.values()));
