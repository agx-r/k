...
template <bool force_blocking = false> void write(int fd, StringView data);
...
enum class WriteMethod { Overwrite, Replace };
...
void list_files(StringView directory,
...
template <bool atomic, int buffer_size = 4096> struct BufferedWriter {
	...
	void write(StringView data) {
		while (not data.empty()) {
			...
			const ByteCount write_len =
			    clamp(length, ByteCount{0}, size - m_pos);
