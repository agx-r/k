...
using CommandFunc =
    Function<void(const ParametersParser& parser, Context& context,
                  const ShellContext& shell_context)>;

using CommandCompleter = Function<Completions(
    const Context& context, CommandParameters, size_t, ByteCount)>;

using CommandHelper =
    Function<String(const Context& context, CommandParameters)>;
...
public:
	...
	void set_command_completer(StringView command_name,
	...
	struct Completer {
		Completions operator()(const Context& context, StringView command_line,
...
private:
	...
	struct Module {
		enum class State { Registered, Loading, Loaded };
