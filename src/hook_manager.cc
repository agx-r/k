...
struct HookManager::HookData {
	...
	bool should_run(bool only_always, const Regex& disabled_hooks,
	                StringView param,
		...
		return (not only_always or (flags & HookFlags::Always)) and
		       (group.empty() or disabled_hooks.empty() or
		        not regex_match(group.begin(), group.end(), disabled_hooks)) and
		       regex_match(param.begin(), param.end(), captures, filter);
	}

	void exec(Hook hook, StringView param, Context& context,
	          const MatchResults<const char*>& captures) {
		...
		for (auto& c : filter.impl()->named_captures)
			env_vars.insert(
			    {format("hook_param_capture_{}", c.name),
...
void HookManager::add_hook(Hook hook, String group, HookFlags flags,
	...
	UniquePtr<HookData> hook_data{new HookData{
	    std::move(group), flags, std::move(filter), std::move(commands)}};
	if (hook == Hook::ModuleLoaded) {
		...
		for (auto&& name : CommandManager::instance().loaded_modules()) {
			...
			if (hook_data->should_run(only_always, disabled_hooks, name,
			                          captures)) {
...
void HookManager::remove_hooks(const Regex& regex) {
	for (auto& list : m_hooks) {
		list.erase(remove_if(list,
		                     [this, &regex](UniquePtr<HookData>& h) {
			                     if (not regex_match(h->group.begin(),
			                                         h->group.end(), regex))
				                     return false;
...
CandidateList HookManager::complete_hook_group(StringView prefix,
	...
	for (auto& list : m_hooks) {
		auto container =
		    list | transform([](const UniquePtr<HookData>& h) -> const String& {
			    return h->group;
		    });
...
void HookManager::run_hook(Hook hook, StringView param, Context& context) {
	...
	ProfileScope profile{context.options()["debug"].get<DebugFlags>(),
	                     [&](std::chrono::microseconds duration) {
		                     write_to_debug_buffer(
		                         format("hook '{}({})' took {} us", hook_name,
		                                param, (size_t)duration.count()));
	                     }};
	...
	for (auto& to_run : hooks_to_run) {
		try {
			...
			if (to_run.hook->flags & HookFlags::Once) {
				...
				if (auto it = find(hook_list, to_run.hook);
				    it != hook_list.end()) {
