...
public:
	...
	template <typename It>
	    requires std::contiguous_iterator<It> and
	...
	template <typename Container>
	    requires(sizeof(decltype(*std::declval<Container>().data())) ==
	             sizeof(T))
...
ArrayView(Container& c)
    -> ArrayView<std::remove_reference_t<decltype(*c.data())>>;

template <typename T> using ConstArrayView = ArrayView<const T>;

template <typename T> bool operator==(ArrayView<T> lhs, ArrayView<T> rhs) {
