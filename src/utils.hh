...
template <typename T> Singleton<T>* Singleton<T>::ms_instance = nullptr;
...
// a wrapping class
template <typename T> class [[nodiscard]] OnScopeEnd {
public:
	[[gnu::always_inline]]
	OnScopeEnd(T func)
	    : m_valid{true}, m_func{std::move(func)} {}
	...
	[[gnu::always_inline]]
	~OnScopeEnd() noexcept(noexcept(m_func())) {
		if (m_valid)
			m_func();
	}
...
template <typename E> auto to_underlying(E value) {
...
template <typename Res, typename... Args> class FunctionRef<Res(Args...)> {
public:
	FunctionRef()
	    : m_target{nullptr}, m_invoker{[](void* target, Args... args) {
	...
	template <typename Target>
	    requires requires(Target t, Args... a) {
		    requires not std::is_same_v<FunctionRef,
		                                std::remove_cvref_t<Target>>;
		    { t(a...) } -> ConvertibleTo<Res>;
	    }
	FunctionRef(Target&& target)
	    : m_target{&target}, m_invoker{[](void* target, Args... args) {
...
template <typename... Funcs> struct Overload : Funcs... {
	using Funcs::operator()...;
};

template <typename... Funcs> auto overload(Funcs&&... funcs) {
	return Overload<std::remove_cvref_t<Funcs>...>{
