...
Buffer* BufferManager::create_buffer(String name, Buffer::Flags flags,
                                     BufferLines lines, ByteOrderMark bom,
	...
	m_buffers.push_back(
	    make_unique_ptr<Buffer>(std::move(name), flags, std::move(lines), bom,
...
void BufferManager::delete_buffer(Buffer& buffer) {
	...
	if (it ==
	    m_buffers.end()) // we might be trying to recursively delete this buffer
		return;
...
Buffer* BufferManager::get_buffer_ifp(StringView name) {
	...
	for (auto& buf : m_buffers) {
		if (buf->name() == name or (buf->flags() & Buffer::Flags::File and
...
Buffer* BufferManager::get_buffer_matching_ifp(
    const FunctionRef<bool(Buffer&)>& filter) {
...
Buffer&
BufferManager::get_buffer_matching(const FunctionRef<bool(Buffer&)>& filter) {
...
void BufferManager::backup_modified_buffers() {
	for (auto& buf : m_buffers) {
		if ((buf->flags() & Buffer::Flags::File) and buf->is_modified() and
		    not(buf->flags() & Buffer::Flags::ReadOnly))
			write_buffer_to_backup_file(*buf);
	}
}

void BufferManager::clear_buffer_trash() { m_buffer_trash.clear(); }

void BufferManager::arrange_buffers(ConstArrayView<String> buffers,
                                    bool to_back) {
	Vector<size_t> indices;
	for (const auto& name : buffers) {
		auto filename = real_path(parse_filename(name));
		auto it = find_if(m_buffers, [&](auto& buf) {
			return buf->display_name() == name or
		...
		if (contains(indices, index))
			throw runtime_error{
			    format("buffer '{}' appears more than once", name)};
