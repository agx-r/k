...
[[gnu::always_inline]]
inline char read(Iterator& it) noexcept {
	char c = *it;
	++it;
	return c;
}
...
[[gnu::noinline]]
Codepoint
read_codepoint_multibyte(Iterator& it, const Sentinel& end,
                         char byte) noexcept(noexcept(InvalidPolicy{}(0))) {
	if (it == end)
...
template <typename InvalidPolicy = utf8::InvalidPolicy::Pass, typename Iterator,
          typename Sentinel>
Codepoint
read_codepoint(Iterator& it,
               const Sentinel& end) noexcept(noexcept(InvalidPolicy{}(0))) {
	if (it == end)
...
template <typename InvalidPolicy = utf8::InvalidPolicy::Pass>
ByteCount codepoint_size(Codepoint cp) noexcept(noexcept(InvalidPolicy{}(0))) {
	if (cp <= 0x7F)
		return 1;
	else if (cp <= 0x7FF)
		return 2;
