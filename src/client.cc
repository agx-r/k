...
Client::Client(UniquePtr<UserInterface>&& ui, UniquePtr<Window>&& window,
               SelectionList selections, int pid, EnvVarMap env_vars,
               String name, OnExitCallback on_exit)
    : m_ui{std::move(ui)}, m_window{std::move(window)}, m_pid{pid},
      m_on_exit{std::move(on_exit)}, m_env_vars(std::move(env_vars)),
	...
	m_ui->set_ui_options(
	    m_window->options()["ui_options"].get<UserInterface::Options>());
	m_ui->set_on_key([this](Key key) {
		...
		if (key == ctrl('c')) {
			...
			print_status({},
			             {"operation cancelled", context().faces()["Error"]},
	...
	m_window->hooks().run_hook(Hook::WinDisplay, m_window->buffer().name(),
	                           context());
...
bool Client::is_ui_ok() const { return m_ui->is_ok(); }

bool Client::process_pending_inputs() {
	const bool debug_keys =
	    (bool)(context().options()["debug"].get<DebugFlags>() &
	           DebugFlags::Keys);
		...
		try {
			if (debug_keys)
				write_to_debug_buffer(
				    format("Client '{}' got key '{}'", context().name(), key));

			if (key == Key::FocusIn)
				context().hooks().run_hook(Hook::FocusIn, context().name(),
				                           context());
			else if (key == Key::FocusOut)
				context().hooks().run_hook(Hook::FocusOut, context().name(),
				                           context());
			else {
			...
			context().print_status(
			    {}, {error.what().str(), context().faces()["Error"]}, -1,
			    StatusStyle::Status);
...
void Client::print_status(DisplayLine prompt, DisplayLine content,
                          ColumnCount cursor_pos, StatusStyle style) {
...
DisplayCoord Client::dimensions() const { return m_ui->dimensions(); }
...
String generate_context_info(const Context& context) {
	...
	if (context.client().input_handler().is_recording())
		s += format("[recording ({})]",
		            context.client().input_handler().recording_reg());
	if (context.hooks_disabled())
		s += "[no-hooks]";
	if (not(context.buffer().flags() &
	        (Buffer::Flags::File | Buffer::Flags::Debug)))
		s += "[scratch]";
...
DisplayLine Client::generate_mode_line() const {
	...
	try {
		auto [mode_info_line, normal_params] =
		    context().client().input_handler().mode_info();
		const String& modelinefmt =
		    context().options()["modelinefmt"].get<String>();
		HashMap<String, DisplayLine> atoms{
		    {"mode_info", mode_info_line},
		    {"context_info",
		     {generate_context_info(context()),
		      context().faces()["Information"]}}};
		ShellContext shell_context{
		    {},
		    {
		        {"register",
		         normal_params ? StringView{normal_params->reg}.str() : ""},
		        {"count",
		         normal_params ? String{to_string(normal_params->count)} : ""},
		    }};
		...
		write_to_debug_buffer(
		    format("Error while parsing modelinefmt: {}", err.what()));
		modeline.push_back({"modelinefmt error, see *debug* buffer",
		                    context().faces()["Error"]});
	}
...
void Client::change_buffer(Buffer& buffer,
                           Optional<FunctionRef<void()>> set_selections) {
	...
	m_window->options().unregister_watcher(*this);
	m_window->set_client(nullptr);
	client_manager.add_free_window(std::move(m_window), context().selections());
...
static bool is_inline(InfoStyle style) {
	return style == InfoStyle::Inline or style == InfoStyle::InlineAbove or
...
void Client::redraw_ifn() {
	...
	if (m_ui_pending & Draw) {
		...
		auto cursor_pos =
		    window.display_coord(context().selections().main().cursor())
		        .value_or(DisplayCoord{});
		m_ui->draw(display_buffer, cursor_pos, faces["Default"],
		           faces["BufferPadding"],
		           window.last_display_setup().widget_columns);
	}

	const bool update_menu_anchor =
	    (m_ui_pending & Draw) and not(m_ui_pending & MenuHide) and
	...
	if ((m_ui_pending & MenuShow) or update_menu_anchor) {
		auto anchor = m_menu.style == MenuStyle::Inline
		                  ? window.display_coord(m_menu.anchor)
		                  : DisplayCoord{};
	...
	const bool update_info_anchor =
	    (m_ui_pending & Draw) and not(m_ui_pending & InfoHide) and
	...
	if ((m_ui_pending & InfoShow) or update_info_anchor) {
		auto anchor = is_inline(m_info.style)
		                  ? window.display_coord(m_info.anchor)
		                  : DisplayCoord{};
	...
	if (m_ui_pending & InfoShow and m_info.ui_anchor)
		m_ui->info_show(m_info.title, m_info.content, *m_info.ui_anchor,
		                faces[(is_inline(m_info.style) or
		                       m_info.style == InfoStyle::MenuDoc)
		                          ? "InlineInformation"
		                          : "Information"],
		                m_info.style);
	...
	// This needs to be done *after* update_display_buffer as ithe mode line may
	// rely on it to compute whether selections are visible.
	...
	if (m_ui_pending & StatusLine)
		m_ui->draw_status(m_status_prompt, m_status_content,
		                  m_status_cursor_pos, m_mode_line, faces["StatusLine"],
		                  m_status_style);
...
void Client::reload_buffer() {
	...
	try {
		...
		context().print_status(
		    {format("error while reloading buffer: '{}'", error.what()),
		     context().faces()["Error"]});
...
void Client::on_buffer_reload_key(Key key) {
	...
	auto set_autoreload = [this](Autoreload autoreload) {
		...
		if (&option->manager() == &GlobalScope::instance().options())
			option =
			    &context().buffer().options().get_local_option("autoreload");
	...
	if (key == 'y' or key == 'Y' or key == Key::Return) {
		...
		buffer.set_fs_status(get_fs_status(buffer.filename()));
		print_status({},
		             {format("'{}' kept", buffer.display_name()),
		              context().faces()["Information"]},
		             -1, StatusStyle::Status);
		if (key == 'N')
		...
		print_status({},
		             {format("'{}' is not a valid choice", key),
		              context().faces()["Error"]},
		             -1, StatusStyle::Status);
		m_input_handler.on_next_key(
		    "buffer-reload", KeymapMode::None,
		    [this](Key key, Context&) { on_buffer_reload_key(key); });
		return;
	}
...
void Client::check_if_buffer_needs_reloading() {
	...
	try {
		...
		if (MappedFile fd{filename};
		    fd.st.st_size == status.file_size and
		    murmur3(fd.data, fd.st.st_size) == status.hash)
			return;
		...
		write_to_debug_buffer(
		    format("Error while checking if buffer {} changed: {}",
		           buffer.filename(), error.what()));
	}
}
...
void Client::on_option_changed(const Option& option) {
	...
	m_ui_pending |=
	    Draw; // a highlighter might depend on the option, so we need to redraw
}

void Client::menu_show(Vector<DisplayLine> choices, BufferCoord anchor,
                       MenuStyle style) {
...
void Client::info_show(DisplayLine title, DisplayLineList content,
                       BufferCoord anchor, InfoStyle style) {
	if (m_info.style == InfoStyle::Modal) // We already have a modal info
	                                      // opened, do not touch it.
		return;
...
void Client::info_show(StringView title, StringView content, BufferCoord anchor,
                       InfoStyle style) {
	...
	info_show(title.empty() ? DisplayLine{} : DisplayLine{title.str(), Face{}},
	          content | split<StringView>('\n') | transform([](StringView s) {
		          return DisplayLine{replace(s, '\t', ' '), Face{}};
              ...
              [this, status_message = std::move(status_message),
               wait_time](Timer& timer) {
	              ...
	              if (not m_previous_status)
		              m_previous_status.emplace(
		                  client.m_status_prompt, client.m_status_content,
		                  client.m_status_cursor_pos, client.m_status_style);

	              client.print_status(
	                  {},
	                  status_message(duration_cast<seconds>(now - wait_time)),
	                  -1, StatusStyle::Status);
	              client.redraw_ifn();
              },
              EventMode::Urgent} {}
...
BusyIndicator::~BusyIndicator() {
	if (m_previous_status and
	    std::uncaught_exceptions() == 0) // restore the status line
	{
		auto& [prompt, content, cursor_pos, style] = *m_previous_status;
		m_context.print_status(std::move(prompt), std::move(content),
		                       std::move(cursor_pos), style);
