...
UniquePtr<Highlighter>
make_highlighter(Func func, HighlightPass pass = HighlightPass::Colorize) {
...
void highlight_range(DisplayBuffer& display_buffer, BufferCoord begin,
	...
	if (begin >= end or end <= display_buffer.range().begin or
	    begin >= display_buffer.range().end)
		return;
	...
	for (auto& line : display_buffer.lines()) {
		...
		for (auto atom_it = line.begin(); atom_it != line.end(); ++atom_it) {
			...
			if (not atom_it->has_buffer_range() or
			    (skip_replaced and is_replaced) or end <= atom_it->begin() or
			    begin >= atom_it->end())
				continue;
...
void replace_range(DisplayBuffer& display_buffer, BufferCoord begin,
	...
	auto first_it =
	    std::lower_bound(lines.begin(), lines.end(), begin,
	                     [](const DisplayLine& l, const BufferCoord& c) {
		                     return l.range().end < c;
	                     });
	if (first_it == lines.end())
		return;

	auto first_atom_it = std::find_if(
	    first_it->begin(), first_it->end(), [&begin](const DisplayAtom& a) {
		    return a.has_buffer_range() and a.end() > begin;
	    });
	first_atom_it = first_it->split(begin);

	auto last_it =
	    std::lower_bound(first_it, lines.end(), end,
	                     [](const DisplayLine& l, const BufferCoord& c) {
		                     return l.range().end < c;
	                     });
	...
	if (first_it == last_it) {
		...
		first_atom_it =
		    first_it->erase(first_it->begin() + first_atom_idx, end_atom_it);
	} else {
		...
		if (last_it != lines.end()) {
			...
			first_atom_it =
			    first_it->insert(first_atom_it, end_atom_it, last_it->end());
			++last_it;
		}
...
static UniquePtr<Highlighter>
create_fill_highlighter(HighlighterParameters params, Highlighter*) {
	if (params.size() != 1)
		throw runtime_error("wrong parameter count");

	return make_highlighter(
	    [spec = parse_face(params[0])](HighlightContext context,
	                                   DisplayBuffer& display_buffer,
	                                   BufferRange range) {
...
template <typename T> struct BufferSideCache {
...
public:
	RegexHighlighter(Regex regex, FacesSpec faces)
	    : Highlighter{HighlightPass::Colorize}, m_regex{std::move(regex)},
	...
	static UniquePtr<Highlighter> create(HighlighterParameters params,
		...
		for (auto& spec : params.subrange(1)) {
			...
			const int capture =
			    str_to_int_ifp(capture_name).value_or_compute([&] {
				    return re.named_capture_index(capture_name);
			    });
			if (capture < 0)
				throw runtime_error(
				    format("capture name {} is neither a capture index, nor an "
				           "existing capture name",
				           capture_name));
			faces.emplace_back(capture, parse_face({colon + 1, spec.end()}));
		}

		return make_unique_ptr<RegexHighlighter>(std::move(re),
		                                         std::move(faces));
	}
...
private:
	...
	struct Cache {
		...
		using RangeAndMatchesList =
		    Vector<RangeAndMatches, MemoryDomain::Highlight>;
		HashMap<BufferRange, RangeAndMatchesList, MemoryDomain::Highlight>
		    m_matches;
	};
	...
	void add_matches(const Buffer& buffer, MatchList& matches,
	                 BufferRange range) {
		kak_assert(matches.size() % m_faces.size() == 0);
		for (auto&& match : RegexIterator{
		         get_iterator(buffer, range.begin),
		         get_iterator(buffer, range.end), buffer.begin(), buffer.end(),
		         m_regex,
	...
	const MatchList& get_matches(const Buffer& buffer,
		...
		if (cache.m_regex_version != m_regex_version or
		    ...
		    accumulate(cache.m_matches, (size_t)0, [](size_t c, auto&& m) {
			    return c + m.value.size();
		    }) > 1000) {
...
const HighlighterDesc dynamic_regex_desc = {
    ...
    {}};
UniquePtr<Highlighter>
create_dynamic_regex_highlighter(HighlighterParameters params, Highlighter*) {
	if (params.size() < 2)
		throw runtime_error("wrong parameter count");
	...
	for (auto& spec : params.subrange(1)) {
		...
		faces.emplace_back(String{spec.begin(), colon},
		                   parse_face({colon + 1, spec.end()}));
	}

	auto make_hl = [](auto& get_regex, auto& resolve_faces) {
		return make_unique_ptr<DynamicRegexHighlighter<
		    std::remove_cvref_t<decltype(get_regex)>,
	...
	auto resolve_faces = [faces = std::move(faces)](const Regex& regex) {
		...
		for (auto& face : faces) {
			const int capture =
			    str_to_int_ifp(face.first).value_or_compute([&] {
				    return regex.named_capture_index(face.first);
			    });
			if (capture < 0) {
				write_to_debug_buffer(format(
				    "Error while evaluating dynamic regex expression faces,"
	...
	if (token and parser.done() and token->type == Token::Type::OptionExpand and
		...
		auto get_regex = [option_name =
		                      token->content](const Context& context) {
...
const HighlighterDesc line_desc = {
    ...
    {}};
UniquePtr<Highlighter> create_line_highlighter(HighlighterParameters params,
                                               Highlighter*) {
	if (params.size() != 2)
		throw runtime_error("wrong parameter count");
...
const HighlighterDesc column_desc = {
    ...
    {{{"ruler",
       {ArgCompleter{},
        "replace empty or whitespace cells with the given character. When "
        "provided, <face> is not applied to non-empty cells"}}},
     ParameterDesc::Flags::None,
     2,
     2},
};
UniquePtr<Highlighter> create_column_highlighter(HighlighterParameters params,
                                                 Highlighter*) {
	...
	auto func = [col_expr = positionals[0],
	             facespec = parse_face(positionals[1]), highlight_non_blank,
	             col_char](HighlightContext context,
		...
		if (column < context.setup.first_column or
		    column >= context.setup.first_column +
		                  context.context.window().dimensions().column)
			return;
		...
		for (auto& line : display_buffer.lines()) {
			...
			bool found = false;
			for (auto atom_it = line.begin(); atom_it != line.end();
			     ++atom_it) {
...
const HighlighterDesc wrap_desc = {
    ...
    {{
         ...
         {"width",
          {ArgCompleter{},
           "wrap at the given column instead of the window's width"}},
         {"marker",
          {ArgCompleter{},
           "insert the given text at the beginning of the wrapped line"}},
     },
     ParameterDesc::Flags::None,
     0,
     0}};
struct WrapHighlighter : Highlighter {
	...
	void do_highlight(HighlightContext context, DisplayBuffer& display_buffer,
		...
		const ColumnCount wrap_column =
		    std::min(m_max_width, context.context.window().dimensions().column -
		                              context.setup.widget_columns);
		if (wrap_column <= 0)
			return;

		const Buffer& buffer = context.context.buffer();
		const int tabstop = context.context.options()["tabstop"].get<int>();
		const ColumnCount marker_len =
		    zero_if_greater(m_marker.column_length(), wrap_column);
		...
		for (auto it = display_buffer.lines().begin();
			...
			const ColumnCount indent =
			    m_preserve_indent
			        ? zero_if_greater(
			              line_indent(buffer, tabstop, it->range().begin.line),
			              wrap_column)
			        : 0_col;
			...
			while (next_split_pos(pos, it->end(), wrap_column, prefix_len)) {
				...
				if (pos.byte > 0)
					pos.atom_it =
					    ++line.split(pos.atom_it, pos.atom_it->begin() +
					                                  BufferCoord{0, pos.byte});

				auto coord = pos.atom_it->begin();
				DisplayLine new_line{
				    AtomList{std::make_move_iterator(pos.atom_it),
				             std::make_move_iterator(line.end())}};
				line.erase(pos.atom_it, line.end());
				auto next_atom_it = new_line.begin();

				if (marker_len != 0)
					next_atom_it = ++new_line.insert(new_line.begin(),
					                                 {m_marker, face_marker});
				if (indent > marker_len) {
					next_atom_it =
					    new_line.insert(next_atom_it, {buffer, {coord, coord}});
	...
	bool next_split_pos(SplitPos& pos, DisplayLine::iterator line_end,
		...
		while (pos.atom_it != line_end and pos.column < wrap_column) {
			...
			if (pos.column + width >
			    wrap_column) // the target column was in the char
			{
			...
			if (it == content.end()) {
				...
				if (pos.atom_it != line_end and
				    pos.atom_it->type() != DisplayAtom::Range and
				    pos.column + pos.atom_it->length() >= wrap_column)
					return true;

				// Thanks to the pass ordering, atom boundary should always be
				// reasonable word split points
				last_word_boundary = pos;
				last_WORD_boundary = pos;
			}
		}
		...
		if (m_word_wrap and pos.byte < content.length()) {
			auto find_split_pos = [&](SplitPos start_pos, auto is_word) {
				...
				if (not is_word(utf8::codepoint(it, content.end()), {'_'}))
					return true;
				// split at last word boundary if the word is shorter than our
				// wrapping width
				...
				while (it != content.end() and word_length <= max_word_length) {
					const Codepoint cp =
					    utf8::read_codepoint(it, content.end());
					if (not is_word(cp, {'_'}))
						break;
	...
	static ColumnCount line_indent(const Buffer& buffer, int tabstop,
	                               LineCount line) {
	...
	static UniquePtr<Highlighter> create(HighlighterParameters params,
		...
		ColumnCount max_width = parser.get_switch("width")
		                            .map(str_to_int)
		                            .value_or(std::numeric_limits<int>::max());

		return make_unique_ptr<WrapHighlighter>(
		    max_width, (bool)parser.get_switch("word"),
		    (bool)parser.get_switch("indent"),
		    parser.get_switch("marker").value_or("").str());
	}

	static ColumnCount zero_if_greater(ColumnCount val, ColumnCount max) {
...
struct TabulationHighlighter : Highlighter {
	...
	void do_highlight(HighlightContext context, DisplayBuffer& display_buffer,
		...
		for (auto& line : display_buffer.lines()) {
			...
			for (auto atom_it = line.begin(); atom_it != line.end();
				...
				if (auto* atom_line_data = buffer[begin.line].data();
				    atom_line_data != line_data) {
					pos = line_data = atom_line_data;
					column = 0;
				}

				kak_assert(pos != nullptr and pos <= line_data + begin.column);
				for (auto end = line_data + atom_it->end().column; pos != end;
				     ++pos) {
					...
					while (pos != next_tab)
						column += codepoint_width(
						    utf8::read_codepoint(pos, next_tab));
					...
					if (pos >= line_data + atom_it->begin().column) {
						if (pos != line_data + atom_it->begin().column)
							atom_it = ++line.split(
							    atom_it,
							    {begin.line, ByteCount(pos - line_data)});
						if (pos + 1 != end)
							atom_it = line.split(
							    atom_it,
							    {begin.line, ByteCount(pos + 1 - line_data)});
	...
	void do_compute_display_setup(HighlightContext context,
		...
		const ColumnCount win_end =
		    setup.first_column + context.context.window().dimensions().column -
		    setup.widget_columns;
...
const HighlighterDesc show_whitespace_desc = {
    ...
    "[-spc <separator>] [-nbsp <separator>] [-indent <separator>]\n"
    "Display whitespaces using symbols",
    {{{"tab", {ArgCompleter{}, "replace tabulations with the given character"}},
      {"tabpad",
       {ArgCompleter{}, "append as many of the given character as is necessary "
                        "to honor `tabstop`"}},
      {"spc", {ArgCompleter{}, "replace spaces with the given character"}},
      {"lf", {ArgCompleter{}, "replace line feeds with the given character"}},
      {"nbsp",
       {ArgCompleter{},
        "replace non-breakable spaces with the given character"}},
      {"indent",
       {ArgCompleter{}, "replace first space of every indent with the given "
                        "character according to `indentwidth`"}},
...
struct ShowWhitespacesHighlighter : Highlighter {
	ShowWhitespacesHighlighter(String tab, String tabpad, String spc, String lf,
	                           String nbsp, String indent, bool only_trailing)
	    : Highlighter{HighlightPass::Replace}, m_tab{std::move(tab)},
	      m_tabpad{std::move(tabpad)}, m_spc{std::move(spc)},
	      m_lf{std::move(lf)}, m_nbsp{std::move(nbsp)},
	      m_indent{std::move(indent)},
	      m_only_trailing{std::move(only_trailing)} {}
	...
	static UniquePtr<Highlighter> create(HighlighterParameters params,
		...
		auto get_param = [&](StringView param, StringView fallback) {
			...
			if (value.char_length() > 1)
				throw runtime_error{
				    format("-{} expects a single character or empty parameter",
				           param)};
			return value.str();
		};
		...
		return make_unique_ptr<ShowWhitespacesHighlighter>(
		    ...
		    get_param("spc", "·"), get_param("lf", "¬"), get_param("nbsp", "⍽"),
		    get_param("indent", "│"), only_trailing);
	}
...
private:
	void do_highlight(HighlightContext context, DisplayBuffer& display_buffer,
		...
		const int indentwidth =
		    context.context.options()["indentwidth"].get<int>();
				...
				for (BufferIterator it = begin; it != end;) {
					...
					if (is_whitespace(cp)) {
						...
						if (cp == '\t' and not m_tab.empty() and
							...
							atom_it->replace(
							    m_tab + String(m_tabpad[(CharCount)0],
							                   count - m_tab.column_length()));
						} else if (cp == ' ' and is_indentation and
						           indentwidth > 0 and not m_indent.empty()) {
							const ColumnCount column =
							    get_column(buffer, tabstop, coord);
						...
						else if ((cp == 0xA0 or cp == 0x202F) and
						         not m_nbsp.empty())
...
const HighlighterDesc line_numbers_desc = {
    "Parameters: [-relative] [-hlcursor] [-separators "
    "<separator|separator:cursor|cursor:up:down>] [-min-digits <cols>]\n"
    "Display line numbers",
    {{{"relative", {{}, "show line numbers relative to the main cursor line"}},
      {"full-relative",
       {{},
        "show line numbers relative to the main cursor line and the main "
        "cursor line as 0"}},
      {"separator",
       {ArgCompleter{}, "string to separate the line numbers column from the "
                        "rest of the buffer (default '|')"}},
      {"cursor-separator",
       {ArgCompleter{},
        "identical to -separator but applies only to the line of the cursor "
        "(default is the same value passed to -separator)"}},
      {"min-digits",
       {ArgCompleter{}, "use at least the given number of columns to display "
                        "line numbers (default 2)"}},
      {"hlcursor", {{}, "highlight the cursor line with a separate face"}}},
     ParameterDesc::Flags::None,
     0,
     0}};
struct LineNumbersHighlighter : Highlighter {
	LineNumbersHighlighter(bool relative, bool zero_cursor_line,
	                       bool hl_cursor_line, String separator,
	                       String cursor_separator, int min_digits)
	    : Highlighter{HighlightPass::Move}, m_relative{relative},
	      m_zero_cursor_line{zero_cursor_line},
	      m_hl_cursor_line{hl_cursor_line}, m_separator{std::move(separator)},
	...
	static UniquePtr<Highlighter> create(HighlighterParameters params,
		...
		if (cursor_separator.column_length() != separator.column_length())
			throw runtime_error("separator for active line should have the "
			                    "same length as 'separator'");

		int min_digits =
		    parser.get_switch("min-digits").map(str_to_int).value_or(2);
		if (min_digits < 0)
		...
		return make_unique_ptr<LineNumbersHighlighter>(
		    relative or full_relative, full_relative,
		    (bool)parser.get_switch("hlcursor"), separator.str(),
		    cursor_separator.str(), min_digits);
	}
...
private:
	...
	void do_compute_display_setup(HighlightContext context,
		...
		ColumnCount width =
		    compute_digit_count(context) + m_separator.column_length();
		...
		LineCount line_count =
		    (m_relative and m_zero_cursor_line)
		        ? std::max(abs(context.setup.first_line - cursor_line),
		                   abs(context.setup.first_line +
		                       context.setup.line_count - cursor_line))
	...
	for (auto& sel : context.context.selections()) {
		...
		if (matching_prev)
			highlight_range(display_buffer, it.base().coord(),
			                (it + 1).base().coord(), false, apply_face(face));
...
UniquePtr<Highlighter>
create_matching_char_highlighter(HighlighterParameters params, Highlighter*) {
	ParametersParser parser{params, show_matching_desc.params};
	return make_highlighter(parser.get_switch("previous")
	                            ? show_matching_char<true>
	                            : show_matching_char<false>);
}
...
void expand_unprintable(HighlightContext context, DisplayBuffer& display_buffer,
                        BufferRange) {
	auto is_printable =
	    [](Codepoint cp) { // follow musl iswprint set of printable characters,
		                   // but with straightforward impl
		    if (cp < 0xff)
			    return cp == '\n' or cp >= ' ';
		    else if (cp == 0x2028 or cp == 0x2029 or
		             (cp >= 0xFFF9 and cp <= 0xFFFB))
			    return false;
		    return true;
	    };
	const auto& buffer = context.context.buffer();
	...
	for (auto& line : display_buffer.lines()) {
		for (auto atom_it = line.begin(); atom_it != line.end(); ++atom_it) {
			...
			for (auto it = line_data + begin.column,
			          end = line_data + atom_it->end().column;
			     it < end;) {
				auto next = it;
				...
				if (not is_printable(cp)) {
					...
					if (ByteCount pos(next - line_data);
					    pos < atom_it->end().column)
						atom_it = line.split(atom_it, {begin.line, pos});
...
static void update_line_specs_ifn(const Buffer& buffer,
	...
	for (auto it = lines.begin(); it != lines.end(); ++it) {
		auto& line =
		    std::get<0>(*it); // that line is 1 based as it comes from user side
		auto modif_it =
		    std::upper_bound(modifs.begin(), modifs.end(), line - 1,
...
struct FlagLinesHighlighter : Highlighter {
	FlagLinesHighlighter(String option_name, String default_face, bool after)
	    : Highlighter{HighlightPass::Move},
	      ...
	      m_default_face{std::move(default_face)}, m_after(after) {}
	...
	static UniquePtr<Highlighter> create(HighlighterParameters params,
		...
		return make_unique_ptr<FlagLinesHighlighter>(
		    option_name, default_face, (bool)parser.get_switch("after"));
	}
...
template <typename OptionType, typename DerivedType,
          HighlightPass pass = HighlightPass::Colorize>
struct OptionBasedHighlighter : Highlighter {
	OptionBasedHighlighter(String option_name)
	    : Highlighter{pass}, m_option_name{std::move(option_name)} {}
	...
	OptionType& get_option(const HighlightContext& context) const {
		return context.context.options()[m_option_name]
		    .template get_mutable<OptionType>();
	}
...
bool option_element_compare(RangeAndString const& lhs,
                            RangeAndString const& rhs) {
...
bool option_add_from_strings(Vector<RangeAndString, MemoryDomain::Options>& opt,
                             ConstArrayView<String> strs) {
	auto vec = option_from_strings(
	    Meta::Type<Vector<RangeAndString, MemoryDomain::Options>>{}, strs);
	if (vec.empty())
		return false;
	auto middle = opt.insert(opt.end(), std::make_move_iterator(vec.begin()),
...
const HighlighterDesc ranges_desc = {
    ...
    {}};
struct RangesHighlighter
    : OptionBasedHighlighter<RangeAndStringList, RangesHighlighter> {
...
private:
	void do_highlight(HighlightContext context, DisplayBuffer& display_buffer,
		...
		for (auto& [range, face] : range_and_faces.list) {
			try {
				if (buffer.is_valid(range.first) and
				    (buffer.is_valid(range.last) and
				     not buffer.is_end(range.last)))
					highlight_range(display_buffer, range.first,
					                buffer.char_next(range.last), false,
...
const HighlighterDesc replace_ranges_desc = {
    "Parameters: <option name>\n"
    "Use the range-specs option given as parameter to highlight buffer\n"
    "each spec is interpreted as a display line to display in place of the "
    "range",
    {}};
struct ReplaceRangesHighlighter
    : OptionBasedHighlighter<RangeAndStringList, ReplaceRangesHighlighter,
                             HighlightPass::Replace> {
...
private:
	static bool is_valid(Buffer& buffer, BufferCoord c) {
		return c.line >= 0 and c.column >= 0 and
		       c.line < buffer.line_count() and
		       c.column <= buffer[c.line].length();
	}

	static bool is_fully_selected(const SelectionList& sels,
	                              const InclusiveBufferRange& range) {
		auto it =
		    std::lower_bound(sels.begin(), sels.end(), range.first,
		                     [](const Selection& s, const BufferCoord& c) {
			                     return s.max() < c;
		                     });
		if (it == sels.end())
			return true;
		return it->min() > range.last or
		       (it->min() <= range.first and it->max() >= range.last);
	}
		...
		for (auto& [range, spec] : range_and_faces.list) {
			try {
				if (not is_valid(buffer, range.first) or
				    (not is_empty(range) and
				     not is_valid(buffer, range.last)) or
				    not is_fully_selected(sels, range))
					continue;
				auto end = is_empty(range) ? range.first
				                           : buffer.char_next(range.last);
				replace_range(
				    display_buffer, range.first, end,
				    [&, range = BufferRange{range.first, end}](
					    ...
					    while (true) {
						    for (auto& atom : parse_display_line(
						             {it, eol}, context.context.faces())) {
							    atom.replace(range);
	...
	void do_compute_display_setup(HighlightContext context,
		...
		for (auto& [range, spec] : range_and_faces.list) {
			if (not is_valid(buffer, range.first) or
			    (not is_empty(range) and not is_valid(buffer, range.last)) or
			    not is_fully_selected(sels, range))
				continue;

			auto last =
			    is_empty(range) ? range.first : buffer.char_next(range.last);
			if (range.first.line < setup.first_line and
			    last.line >= setup.first_line)
...
const HighlighterDesc higlighter_group_desc = {
    ...
    {{{"passes",
       {ArgCompleter{}, "flags(colorize|move|wrap) "
                        "kind of highlighters can be put in the group "
                        "(default colorize)"}}},
     ParameterDesc::Flags::SwitchesOnlyAtStart,
     0,
     0}};
UniquePtr<Highlighter> create_highlighter_group(HighlighterParameters params,
                                                Highlighter*) {
...
private:
	void do_highlight(HighlightContext context, DisplayBuffer& display_buffer,
		...
		try {
			SharedHighlighters::instance().get_child(m_name).highlight(
...
struct ForwardHighlighterApplier {
	...
	void operator()(BufferCoord begin, BufferCoord end,
	                Highlighter& highlighter) {
		if (begin == end)
			return;

		auto first_line = std::find_if(cur_line, end_line, [&](auto&& line) {
			return line.range().end > begin;
		});
		...
		while (cur_line != end_line and cur_line->range().begin < end) {
			auto& line = *cur_line;
			auto first = std::find_if(cur_atom, line.end(), [&](auto&& atom) {
				return atom.has_buffer_range() and atom.end() > begin;
			});
			if (first != line.end() and first->type() == DisplayAtom::Range and
			    first->begin() < begin)
				first = ++line.split(first, begin);
			auto idx = first - line.begin();

			auto last = std::find_if(first, line.end(), [&](auto&& atom) {
				return atom.has_buffer_range() and atom.end() > end;
			});
			if (last != line.end() and last->type() == DisplayAtom::Range and
			    last->begin() < end)
		...
		for (size_t i = 0; i < insert_pos.size(); ++i) {
			...
			auto it = line_it->insert(line_it->begin() + idx,
...
const HighlighterDesc region_desc = {
    "Parameters:  [-match-capture] [-recurse <recurse>] <opening> <closing> "
    "<type> <params>...\n"
    ...
    {{{"match-capture",
       {{},
        "only consider region ending/recurse delimiters whose first capture "
        "group match the region beginning delimiter"}},
      {"recurse",
       {ArgCompleter{}, "make the region end on the first ending delimiter "
                        "that does not close the given parameter"}}},
     ParameterDesc::Flags::SwitchesOnlyAtStart |
...
public:
	...
	void do_highlight(HighlightContext context, DisplayBuffer& display_buffer,
		...
		auto last_begin =
		    (begin == regions.begin()) ? range.begin : (begin - 1)->end;
		...
		for (; begin != end; ++begin) {
			if (apply_default and last_begin < begin->begin)
				apply_highlighter(correct(last_begin), correct(begin->begin),
				                  *default_region_it->value);

			apply_highlighter(correct(begin->begin), correct(begin->end),
			                  *begin->highlighter);
			last_begin = begin->end;
		}
		if (apply_default and last_begin < display_range.end)
			apply_highlighter(correct(last_begin), range.end,
			                  *default_region_it->value);
	...
	void add_child(String name, UniquePtr<Highlighter>&& hl,
	               bool override) override {
		if (not dynamic_cast<RegionHighlighter*>(hl.get()))
			throw runtime_error{"only region highlighter can be added as child "
			                    "of a regions highlighter"};
		auto it = m_regions.find(name);
		if (not override and it != m_regions.end())
			throw runtime_error{format("duplicate id: '{}'", name)};

		UniquePtr<RegionHighlighter> region_hl{
	...
	Completions complete_child(StringView path, ByteCount cursor_pos,
		...
		auto completions_flags =
		    group ? Completions::Flags::None : Completions::Flags::Menu;
	...
	static UniquePtr<Highlighter> create_region(HighlighterParameters params,
	                                            Highlighter* parent) {
		...
		return make_unique_ptr<RegionHighlighter>(
		    std::move(delegate), parser[0], parser[1],
		    parser.get_switch("recurse").value_or("").str(), match_capture);
	}

	static UniquePtr<Highlighter>
	create_default_region(HighlighterParameters params, Highlighter* parent) {
		if (not is_regions(parent))
			throw runtime_error{"default-region highlighter can only be added "
			                    "to a regions parent"};
...
private:
	struct RegionHighlighter : public Highlighter {
		RegionHighlighter(UniquePtr<Highlighter>&& delegate, String begin,
		                  String end, String recurse, bool match_capture)
		    : Highlighter{delegate->passes()}, m_delegate{std::move(delegate)},
		      m_begin{std::move(begin)}, m_end{std::move(end)},
		      m_recurse{std::move(recurse)}, m_match_capture{match_capture} {}
	...
	struct RegexKey {
		...
		friend size_t hash_value(const RegexKey& key) {
			return hash_values(key.regex, key.match_captures);
		}
	...
	RegexMatchList::const_iterator
	find_matching_end(const Buffer& buffer, BufferCoord beg_pos,
	                  const RegexMatchList& end_matches,
	                  const RegexMatchList& recurse_matches,
	...
	Optional<RegionAndMatch> find_next_begin(const Cache& cache,
		...
		for (size_t i = 0; i < m_regions.size(); ++i) {
			...
			const auto& matches = cache.matches.get(
			    RegexKey{region->m_begin, region->match_capture()});
			auto it = std::lower_bound(matches.begin(), matches.end(), pos,
			                           compare_to_begin);
			if (it != matches.end() and
			    (not res or it->begin_coord() < res->second->begin_coord()))
	...
	class MatchAdder {
	public:
		MatchAdder(RegionsHighlighter& region, const Buffer& buffer,
		           Cache& cache)
		    : m_buffer(buffer) {
		...
		~MatchAdder() {
			...
			for (auto& [matches, regex, pivot, vm] : m_matchers)
				std::inplace_merge(matches.begin(), matches.begin() + pivot,
				                   matches.end(),
		...
		void add(LineRange range) {
			for (auto line = range.begin; line < range.end; ++line) {
				...
				const auto flags =
				    RegexExecFlags::NotEndOfLine; // buffer line already ends
				                                  // with \n

				for (auto& [matches, regex, pivot, vm] : m_matchers) {
					auto extra_flags = RegexExecFlags::None;
					auto pos = l.begin();
					while (vm.exec(pos, l.end(), l.begin(), l.end(),
					               flags | extra_flags)) {
						ConstArrayView<const char*> captures = vm.captures();
						const bool with_capture =
						    regex.mark_count() > 0 and
						    captures[2] != nullptr and
						    captures[1] - captures[0] <
						        std::numeric_limits<uint16_t>::max();
						matches.push_back(
						    {line, (int)(captures[0] - l.begin()),
						     (int)(captures[1] - l.begin()),
						     (uint16_t)(with_capture ? captures[2] - captures[0]
						                             : 0),
						     (uint16_t)(with_capture ? captures[3] - captures[2]
						                             : 0)});
						pos = captures[1];

						extra_flags = (captures[0] == captures[1])
	...
	private:
		struct Matcher {
			...
			const Regex& regex;
			size_t pivot = matches.size();
			ThreadedRegexVM<const char*, RegexMode::Forward | RegexMode::Search>
			    vm{*regex.impl()};
		};
	...
	void update_changed_lines(const Buffer& buffer,
	...
	bool update_matches(Cache& cache, const Buffer& buffer, LineRange range) {
		...
		if (cache.buffer_timestamp == 0 or
			...
			if (cache.buffer_timestamp != buffer_timestamp) {
				auto modifs =
				    compute_line_modifications(buffer, cache.buffer_timestamp);
	...
	const RegionList& get_regions_for_range(const Buffer& buffer,
		...
		for (auto begin = find_next_begin(cache, range.begin); begin;) {
			...
			auto& end_matches = cache.matches.get(
			    RegexKey{region.m_end, region.match_capture()});
			auto& recurse_matches =
			    region.m_recurse.empty()
			        ? empty_matches
			        : cache.matches.get(
			              RegexKey{region.m_recurse, region.match_capture()});

			auto end_it = find_matching_end(
			    buffer, beg_it->end_coord(), end_matches, recurse_matches,
			    region.match_capture() ? beg_it->capture(buffer)
			                           : Optional<StringView>{});

			if (end_it == end_matches.end() or
			    end_it->end_coord() >=
			        range.end) // region continue past range end
			{
	...
	HashMap<String, UniquePtr<RegionHighlighter>, MemoryDomain::Highlight>
	    m_regions;
...
void setup_builtin_highlighters(HighlighterGroup& group) {
	group.add_child("tabulations"_str,
	                make_unique_ptr<TabulationHighlighter>());
...
void register_highlighters() {
	...
	registry.insert({"column", {create_column_highlighter, &column_desc}});
	registry.insert(
	    {"default-region",
	     {RegionsHighlighter::create_default_region, &default_region_desc}});
	registry.insert(
	    {"dynregex", {create_dynamic_regex_highlighter, &dynamic_regex_desc}});
	registry.insert({"fill", {create_fill_highlighter, &fill_desc}});
	registry.insert(
	    {"flag-lines", {FlagLinesHighlighter::create, &flag_lines_desc}});
	registry.insert(
	    {"group", {create_highlighter_group, &higlighter_group_desc}});
	registry.insert({"line", {create_line_highlighter, &line_desc}});
	registry.insert(
	    {"number-lines", {LineNumbersHighlighter::create, &line_numbers_desc}});
	registry.insert({"ranges", {RangesHighlighter::create, &ranges_desc}});
	registry.insert({"ref", {ReferenceHighlighter::create, &ref_desc}});
	registry.insert({"regex", {RegexHighlighter::create, &regex_desc}});
	registry.insert(
	    {"region", {RegionsHighlighter::create_region, &region_desc}});
	registry.insert({"regions", {RegionsHighlighter::create, &regions_desc}});
	registry.insert({"replace-ranges",
	...
	registry.insert({"wrap", {WrapHighlighter::create, &wrap_desc}});
}
