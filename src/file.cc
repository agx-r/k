...
file_access_error::file_access_error(StringView filename, StringView error_desc)
...
MappedFile::MappedFile(StringView filename) : data{nullptr} {
...
bool regular_file_exists(StringView filename) {
	struct stat st;
	return stat(filename.zstr(), &st) == 0 and (st.st_mode & S_IFMT) == S_IFREG;
}

template <bool atomic> void write(int fd, StringView data) {
	...
	while (count) {
		if (ssize_t written = ::write(fd, ptr, count); written != -1) {
			ptr += written;
			count -= written;
		} else if (errno == EAGAIN and not atomic and
		           EventManager::has_instance())
			EventManager::instance().handle_next_events(
			    EventMode::Urgent, nullptr, std::chrono::nanoseconds{});
		else
			throw file_access_error(fd, strerror(errno));
	}
}
...
int create_file(const char* filename) {
	...
	const int flags = O_CREAT | O_WRONLY | O_TRUNC |
	                  (EventManager::has_instance() ? O_NONBLOCK : 0);
	while ((fd = open(filename, flags, 0644)) == -1) {
		if (errno == ENXIO and
		    EventManager::has_instance()) // trying to open a FIFO with no
		                                  // readers yet
			EventManager::instance().handle_next_events(
			    EventMode::Urgent, nullptr,
...
String find_file(StringView filename, StringView buf_dir,
	...
	for (auto candidate : paths | transform([&](StringView s) {
		                      return parse_filename(s, buf_dir);
	                      })) {
...
void list_files(StringView dirname,
                FunctionRef<void(StringView, const struct stat&)> callback) {
	char buffer[PATH_MAX + 1];
