...
Scope::Scope(Scope& parent)
    : m_data(make_unique_ptr<Data>(parent.options(), parent.hooks(),
                                   parent.keymaps(), parent.aliases(),
                                   parent.faces(), parent.highlighters())) {}
...
struct GlobalScope::GlobalData final : public OptionWatcher {
	GlobalData(GlobalScope& parent)
	    : m_parent(parent), m_option_registry(parent.options()) {
		m_parent.options().register_watcher(*this);
	}

	~GlobalData() { m_parent.options().unregister_watcher(*this); }

	void on_option_changed(const Option& option) override {
		Context empty_context{Context::EmptyContextFlag{}};
		m_parent.hooks().run_hook(
...
OptionsRegistry& GlobalScope::option_registry() {
	return m_global_data->m_option_registry;
}
const OptionsRegistry& GlobalScope::option_registry() const {
	return m_global_data->m_option_registry;
}
