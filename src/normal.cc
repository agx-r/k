...
template <typename T> void select(Context& context, SelectMode mode, T func) {
...
String build_autoinfo_for_mapping(const Context& context, KeymapMode mode,
	...
	for (auto& key : keymaps.get_mapped_keys(mode)) {
		...
		if (keymaps.get_mapping_keys(key, mode).empty() and docstring.empty())
			continue;
		if (auto it = find_if(
		        descs, [&](auto& elem) { return elem.second == docstring; });
		    it != descs.end())
	...
	for (auto& [keys, docstring] : descs)
		res +=
		    format("{}:{}{}\n", keys,
		           String{' ', max_len - keys.column_length() + 1}, docstring);
	return res;
}
		    ...
		    [params](Key key, Context& context) {
			    ...
			    case 'd': {
				    ...
				    select(
				        context, mode,
				        [&](Context& context,
				            Selection& sel) -> Optional<Selection> {
					        if (context.has_window()) {
						        auto& cursor = sel.cursor();
						        auto& window = context.window();
						        if (auto display_coord =
						                window.display_coord(cursor)) {
							        if (cursor.display_target == -1)
								        cursor.display_target =
								            display_coord->column;
							        display_coord->column =
							            cursor.display_target;
							        if (auto buffer_coord = window.buffer_coord(
							                *display_coord + offset))
								        return Selection{BufferCoordAndTarget{
								            *buffer_coord, -1,
								            cursor.display_target}};
						        }
					        }
					        ...
					        return Selection{context.buffer().offset_coord(
					            sel.cursor(), offset, tabstop)};
				        });
				    break;
			    }
...
template <bool lock> void view_commands(Context& context, NormalParams params) {
	const int count = params.count;
	on_next_key_with_autoinfo(
	    context, "view", KeymapMode::View,
	    [count](Key key, Context& context) {
		    ...
		    const DisplayCoord scrolloff =
		        context.options()["scrolloff"].get<DisplayCoord>();
		    const LineCount line_offset{
		        std::min((window.dimensions().line - 1) / 2, scrolloff.line)};
		    ...
		    case 't':
			    ...
			    break;
		    case 'b':
			    window.display_line_at(cursor.line, window.dimensions().line -
			                                            1 - line_offset);
			    break;
		    case '<':
			    window.display_column_at(
			        context.buffer()[cursor.line].column_count_to(
			            cursor.column),
			        column_offset);
			    break;
...
void replace_with_char(Context& context, NormalParams) {
	on_next_key_with_autoinfo(
	    context, "replace-char", KeymapMode::None,
...
void command(const Context& context, EnvVarMap env_vars, char reg = 0) {
	...
	String default_command =
	    context.main_sel_register_value(reg ? reg : ':').str();

	context.input_handler().prompt(
	    ":", {}, default_command, context.faces()["Prompt"],
	    PromptFlags::DropHistoryEntriesWithBlankPrefix | PromptFlags::Command,
	    ':',
	    [completer = CommandManager::Completer{}](const Context& context,
	                                              StringView cmd_line,
	                                              ByteCount pos) mutable {
		    return completer(context, cmd_line, pos);
	    },
	    [env_vars = std::move(env_vars), default_command](
		    ...
		    if (context.has_client()) {
			    ...
			    if (event == PromptEvent::Change) {
				    ...
				    context.input_handler().set_prompt_face(
				        context.faces()[info or cmdline.empty() ? "Prompt"
				                                                : "Error"]);

				    auto autoinfo =
				        context.options()["autoinfo"].get<AutoInfo>();
				    if (autoinfo & AutoInfo::Command) {
					    if (cmdline.length() == 1 and
					        is_horizontal_blank(cmdline[0_byte]))
...
BufferRange apply_diff(Buffer& buffer, BufferCoord pos,
                       ArrayView<StringView> lines_before, StringView after) {
	BufferCoord first = pos;
	const auto lines_after =
	    after | split_after<StringView>('\n') | gather<Vector<StringView>>();

	auto byte_count = [](auto&& lines, int first, int count) {
		return std::accumulate(
		    lines.begin() + first, lines.begin() + first + count, 0_byte,
		    [](ByteCount l, StringView s) { return l + s.length(); });
	};

	bool tried_to_erase_final_newline = false;
	for_each_diff(
	    lines_before.begin(), (int)lines_before.size(), lines_after.begin(),
	    (int)lines_after.size(),
	    [&, posA = 0, posB = 0](DiffOp op, int len) mutable {
		    ...
		    case DiffOp::Add:
			    ...
			    pos = buffer
			              .insert(pos, {lines_after[posB].begin(),
		    ...
		    case DiffOp::Remove: {
			    kak_assert(not tried_to_erase_final_newline);
			    BufferCoord end =
			        buffer.advance(pos, byte_count(lines_before, posA, len));
...
template <bool replace> void pipe(Context& context, NormalParams params) {
	...
	String default_command =
	    context.main_sel_register_value(params.reg ? params.reg : '|').str();
		    ...
		    if (replace) {
			    ...
			    for (auto& sel : selections) {
				    ...
				    for (auto line = first.line; line <= last.line; ++line) {
					    ...
					    if (line == last.line)
						    content = content.substr(
						        0, last.column + utf8::codepoint_size(
						                             content[last.column]));
					    if (line == first.line)
				    ...
				    context.selections_write_only().set(
				        {keep_direction(Selection{first, last}, sel)}, 0);
...
template <bool yank> void change(Context& context, NormalParams params) {
	if (yank) {
		const char reg = params.reg ? params.reg : '"';
		RegisterManager::instance()[reg].set(context,
		                                     context.selections_content());
...
BufferCoord paste_pos(Buffer& buffer, BufferCoord min, BufferCoord max,
                      PasteMode mode, bool linewise) {
	switch (mode) {
	case PasteMode::Append:
		if (buffer.is_end(max))
			return max;
		return linewise ? std::min(buffer.line_count(), max.line + 1)
		                : buffer.char_next(max);
...
template <PasteMode mode> void paste(Context& context, NormalParams params) {
	const char reg = params.reg ? params.reg : '"';
	...
	ScopedSelectionEdition selection_edition{context};
	context.selections().for_each(
	    [&, last = BufferCoord{}](size_t index, Selection& sel) mutable {
		    ...
		    BufferRange range =
		        (mode == PasteMode::Replace)
		            ...
		            : buffer.insert(paste_pos(buffer, min, std::max(max, last),
		                                      mode, linewise),
		                            str);
		    min = range.begin;
...
void paste_all(Context& context, NormalParams params) {
	...
	for (auto& str : strings) {
		...
		ByteCount cursor_offset =
		    utf8::previous(str.end(), str.begin()) - str.begin();
	...
	auto& selections = context.selections();
	{
		ScopedEdition edition(context);
		selections.for_each(
		    [&](size_t, const Selection& sel) {
			    auto range =
			        (mode == PasteMode::Replace)
			            ? buffer.replace(sel.min(), buffer.char_next(sel.max()),
			                             all)
			            : buffer.insert(paste_pos(buffer, sel.min(), sel.max(),
			                                      mode, linewise),
			                            all);
...
void insert_output(Context& context, NormalParams params) {
	const char* prompt =
	    mode == PasteMode::Insert ? "insert-output:" : "append-output:";
	String default_command =
	    context.main_sel_register_value(params.reg ? params.reg : '|').str();
	    ...
	    [default_command, selection_edition = ScopedSelectionEdition(context)](
		    ...
		    selections.for_each(
		        [&](size_t index, Selection& sel) {
			        ...
			        auto range =
			            insert(buffer, sel,
			                   paste_pos(buffer, min, max, mode, false), out);
			        min = range.begin;
			        max = range.end > range.begin ? buffer.char_prev(range.end)
			                                      : range.begin;
		        },
...
template <typename Func>
void regex_prompt(Context& context, String prompt, char reg, RegexMode mode,
                  Func func) {
	kak_assert(is_direction(mode));
	DisplayCoord position =
	    context.has_window() ? context.window().position() : DisplayCoord{};
	SelectionList selections = context.selections();
	auto default_regex = RegisterManager::instance()[reg].get_main(
	    context, context.selections().main_index());
	context.input_handler().prompt(
	    std::move(prompt), {}, default_regex, context.faces()["Prompt"],
	    PromptFlags::Search, reg,
	    [](const Context& context, StringView regex,
	    ...
	    [=, func = std::move(func),
	     saved_reg = RegisterManager::instance()[reg].save(context),
	     selection_edition = ScopedSelectionEdition(context)](
		    ...
		    try {
			    ...
			    if (incsearch) {
				    ...
				    RegisterManager::instance()[reg].restore(context,
				                                             saved_reg);
			    }
...
void select_next_matches(Context& context, const Regex& regex, RegexMode mode,
                         int count) {
	auto& selections = context.selections();
	do {
		bool wrapped = false;
		for (auto& sel : selections)
			sel = keep_direction(
			    find_next_match(context, sel, regex, mode, wrapped), sel);
		selections.sort_and_merge_overlapping();
	} while (--count > 0);
}

void extend_to_next_matches(Context& context, const Regex& regex,
                            RegexMode mode, int count) {
...
void search(Context& context, NormalParams params, SelectMode mode,
            RegexMode regex_mode) {
	kak_assert(is_direction(regex_mode));
	const StringView prompt =
	    mode == SelectMode::Extend
	        ? (regex_mode & RegexMode::Forward ? "search (extend):"
	...
	regex_prompt(context, prompt.str(), reg, regex_mode,
	             [=](const Regex& regex, PromptEvent event, Context& context) {
		             ...
		             if (mode == SelectMode::Extend)
			             extend_to_next_matches(context, regex, regex_mode,
			                                    count);
		             else
...
void search_next(Context& context, NormalParams params, SelectMode mode,
                 RegexMode regex_mode) {
	const char reg = to_lower(params.reg ? params.reg : '/');
	...
	if (not str.empty()) {
		...
		do {
			...
			if (mode == SelectMode::Replace) {
				auto& sel = selections.main();
				sel = keep_direction(
				    find_next_match(context, sel, regex, regex_mode, wrapped),
				    sel);
			} else if (mode == SelectMode::Append) {
				auto sel =
				    keep_direction(find_next_match(context, selections.main(),
				                                   regex, regex_mode, wrapped),
				                   selections.main());
...
void use_selection_as_search_pattern(Context& context, NormalParams params) {
	...
	auto patterns =
	    context.selections() | transform([&](auto&& sel) {
	...
	context.print_status({format("register '{}' set to '{}'", reg, pattern),
	                      context.faces()["Information"]});
	             ...
	             [capture](Regex ex, PromptEvent event, Context& context) {
		             ...
		             if (not ex.empty() and not ex.str().empty())
			             selections = SelectionList{
			                 buffer,
			                 select_matches(buffer, selections, ex, capture)};
	             });
}
...
void split_regex(Context& context, NormalParams params) {
	...
	const int capture = params.count;
	auto prompt =
	    capture ? format("split (on capture {}):", (int)capture) : "split:"_str;
	...
	regex_prompt(context, std::move(prompt), reg, RegexMode::Forward,
	             [capture](Regex ex, PromptEvent event, Context& context) {
		             ...
		             if (not ex.empty() and not ex.str().empty())
			             selections = SelectionList{
			                 buffer,
			                 split_on_matches(buffer, selections, ex, capture)};
	             });
}
...
void join_lines_select_spaces(Context& context, NormalParams) {
	...
	for (auto& sel : context.selections()) {
		...
		for (LineCount line = min_line; line < end_line; ++line) {
			...
			auto end =
			    std::find_if_not(begin + 1, buffer.end(), is_horizontal_blank);
...
template <bool matching> void keep(Context& context, NormalParams params) {
	...
	regex_prompt(context, prompt.str(), reg, RegexMode::Forward,
	             [selection_edition = ScopedSelectionEdition(context)](
		             ...
		             for (auto& sel : context.selections()) {
			             ...
			             auto end = utf8::next(buffer.iterator_at(sel.max()),
			                                   buffer.end());
			             // We do not consider if end is on an eol, as it seems
			             // to give more intuitive behaviours in keep use cases.
			             const auto flags =
			                 match_flags(is_bol(begin.coord()), false,
...
void select_object(Context& context, NormalParams params, ObjectFlags flags,
	...
	on_next_key_with_autoinfo(
	    context, "text-object", KeymapMode::Object,
	    [=](Key key, Context& context) {
		    if (key == Key::Escape)
			    return;
		    ...
		    auto select_nested_and_set_last = [count, flags](Context& context,
		                                                     auto&& func) {
			    context.set_last_select([=](Context& context) {
				    ScopedSelectionEdition selection_edition{context};
		    ...
		    static constexpr struct ObjectType {
			    ...
			    Optional<Selection> (*func)(const Context&, const Selection&,
			                                int, ObjectFlags);
			    Vector<Selection> (*nested_func)(const Context&, int,
			                                     ObjectFlags);
		    } selectors[] = {
		    ...
		    if (obj_it != std::end(selectors)) {
			    if (nested)
				    return select_nested_and_set_last(context,
				                                      obj_it->nested_func);
			    else
				    return select_and_set_last(
				        context, mode, [=](Context& context, Selection& sel) {
					        return obj_it->func(context, sel, count, flags);
				        });
		    }

		    static constexpr auto regex_selector =
		        [=](StringView open, StringView close, int count,
		            ObjectFlags flags) {
			        return [open = Regex{open, RegexCompileFlags::Backward},
				        ...
				        return select_surrounding(context, sel, open, close,
				                                  count, flags);
			        };
		        };
		    ...
		    if (key == 'c') {
			    ...
			    context.input_handler().prompt(
			        ...
			        [count, info, mode, nested, flags,
			         select_nested_and_set_last](StringView cmdline,
			                                     PromptEvent event,
			                                     Context& context) {
				        if (event != PromptEvent::Change)
				        ...
				        struct error : runtime_error {
					        error(size_t)
					            : runtime_error{"desc parsing failed, expected "
					                            "<open>,<close>"} {}
				        };
				        ...
				        if (nested)
					        return select_nested_and_set_last(
					            context, [open = Regex{params[0]},
					                      close = Regex{params[1]}](
					        ...
					        select_and_set_last(context, mode,
					                            regex_selector(params[0],
					                                           params[1], count,
					                                           flags));
			        });
			    return;
		    ...
		    if (auto it = find_if(surrounding_pairs,
		                          [key](auto s) {
			                          return key == s.open or key == s.close or
			                                 key == s.name;
		                          });
			    ...
			    if (nested)
				    return select_nested_and_set_last(
				        context,
				        [open = Regex{format("\\Q{}", it->open)},
				         close = Regex{format("\\Q{}", it->close)}](
				            Context& context, int count, ObjectFlags flags) {
					        return open == close
					                   ? regex_select_nested(context, open,
					                                         flags)
					                   : regex_select_nested(context, open,
					                                         close, count,
					                                         flags);
				    ...
				    return select_and_set_last(
				        context, mode,
				        regex_selector(format("\\Q{}", it->open),
				                       format("\\Q{}", it->close), count,
				                       flags));
		    }
		    ...
		    if (auto cp = key.codepoint(); cp and is_punctuation(*cp, {})) {
			    ...
			    if (nested)
				    return select_nested_and_set_last(
				        context,
				        [re](Context& context, int count, ObjectFlags flags) {
					        return regex_select_nested(context, Regex{re},
					                                   flags);
				        });
			    else
				    return select_and_set_last(
				        context, mode, regex_selector(re, re, count, flags));
		    }
	    },
	    get_title(),
	    build_autoinfo_for_mapping(
...
void select_object(Context& context, NormalParams params) {
	static_assert((not(flags & ObjectFlags::Nested)) or
	                  (flags & (ObjectFlags::ToBegin | ObjectFlags::ToEnd)),
...
void scroll(Context& context, NormalParams params) {
	...
	scroll_window(context, offset * direction,
	              OnHiddenCursor::MoveCursorAndAnchor);
}
	...
	for (auto& sel : selections) {
		...
		for (size_t i = 0, nb_sels = 0; nb_sels < max_lines; ++i) {
			...
			if (anchor_line < 0 or cursor_line < 0 or
			    anchor_line >= buffer.line_count() or
			    cursor_line >= buffer.line_count())
				break;

			const ByteCount anchor_byte =
			    get_byte_to_column(buffer, tabstop, {anchor_line, anchor_col});
			const ByteCount cursor_byte =
			    get_byte_to_column(buffer, tabstop, {cursor_line, cursor_col});
			...
			if (anchor_byte != buffer[anchor_line].length() and
				...
				result.emplace_back(BufferCoord{anchor_line, anchor_byte},
				                    BufferCoordAndTarget{cursor_line,
				                                         cursor_byte,
				                                         cursor.target});
...
void rotate_selections_content(Context& context, NormalParams params) {
	...
	for (auto it = strings.begin(); it != strings.end();) {
		...
		if (it <= main and main < end)
			main =
			    main < new_beg ? end - (new_beg - main) : it + (main - new_beg);
		it = end;
	}
...
enum class SelectFlags { None = 0, Reverse = 1, Inclusive = 2, Extend = 4 };
...
void select_to_next_char(Context& context, NormalParams params,
	...
	on_next_key_with_autoinfo(
	    context, "to-char", KeymapMode::None,
	    [params, flags](Key key, Context& context) {
		    ...
		    auto new_flags = flags & SelectFlags::Extend ? SelectMode::Extend
		                                                 : SelectMode::Replace;
		    select_and_set_last(context, new_flags,
		                        [cp = *cp, count = params.count,
		                         flags](auto& context, auto& sel) {
			                        auto& func = flags & SelectFlags::Reverse
			                                         ? select_to_reverse
			                                         : select_to;
			                        return func(context, sel, cp, count,
	...
	for (auto& sel : context.selections()) {
		for (auto it = buffer.iterator_at(sel.min()),
			...
			if (*it == ' ') {
				...
				while (spaces_end != end and *spaces_end == ' ' and
				       (col % tabstop) != 0) {
					++spaces_end;
					++col;
				}
...
SelectionList read_selections_from_register(char reg, const Context& context) {
	...
	auto end_content =
	    content[0] | reverse() | split('@') | transform([](auto bounds) {
	...
	const auto buffer_name =
	    StringView{content[0].begin(), end_content[1].begin() - 1};
	Buffer& buffer = BufferManager::instance().get_buffer(buffer_name);

	return selection_list_from_strings(buffer, ColumnType::Byte,
	                                   content.subrange(1), timestamp, main);
}
...
void combine_selections(Context& context, SelectionList list, Func func,
	...
	on_next_key_with_autoinfo(
	    context, "combine-selections", KeymapMode::Combine,
...
void save_selections(Context& context, NormalParams params) {
	...
	auto save_to_reg = [reg](Context& context, const SelectionList& sels) {
		auto& buffer = context.buffer();
		auto to_string = [&](const Selection& sel) {
	...
	if (combine and not empty) {
		...
		combine_selections(context, read_selections_from_register(reg, context),
		                   save_to_reg, "combine selections to register");
	} else
...
void restore_selections(Context& context, NormalParams params) {
	...
	if (not combine) {
		...
		combine_selections(context, std::move(selections), set_selections,
		                   "combine selections from register");
}
...
void exec_user_mappings(Context& context, NormalParams params) {
	on_next_key_with_autoinfo(
	    context, "user-mapping", KeymapMode::None,
	    [params](Key key, Context& context) mutable {
		    ...
		    ScopedSelectionEdition selection_edition{context};
		    for (auto& key :
		         context.keymaps().get_mapping_keys(key, KeymapMode::User))
			    context.input_handler().handle_key(key);
	    },
	    "user mapping",
...
void add_empty_line(Context& context, NormalParams params) {
	...
	for (int i = 0; i < sels.size(); ++i) {
		auto line =
		    (above ? sels[i].min().line : sels[i].max().line + 1) + (i * count);
...
void move_cursor(Context& context, NormalParams params, Direction direction,
                 SelectMode mode) {
...
void select_whole_buffer(Context& context, NormalParams) {
	...
	context.selections_write_only() =
	    SelectionList{buffer, {{0, 0}, {buffer.back_coord(), max_column}}};
}
...
static constexpr HashMap<Key, NormalCmd, MemoryDomain::Undefined, KeymapBackend>
    keymap = {
        ...
        {{'p'},
         {"paste after selected text", repeated<paste<PasteMode::Append>>}},
        {{'P'},
         {"paste before selected text", repeated<paste<PasteMode::Insert>>}},
        {{alt('p')},
         {"paste every yanked selection after selected text",
          paste_all<PasteMode::Append>}},
        {{alt('P')},
        ...
        {{alt('!')},
         {"append command output", insert_output<PasteMode::Append>}},
        ...
        {{'/'},
         {"select next given regex match",
          [](Context& c, NormalParams p) {
	          return search(c, p, SelectMode::Replace, RegexMode::Forward);
          }}},
        {{'?'},
         {"extend with next given regex match",
          [](Context& c, NormalParams p) {
	          return search(c, p, SelectMode::Extend, RegexMode::Forward);
          }}},
        {{alt('/')},
         {"select previous given regex match",
          [](Context& c, NormalParams p) {
	          return search(c, p, SelectMode::Replace, RegexMode::Backward);
          }}},
        {{alt('?')},
        ...
        {{alt('I')},
         {"select nested objects",
          select_object<ObjectFlags::Nested | ObjectFlags::Inner |
                        ObjectFlags::ToBegin | ObjectFlags::ToEnd>}},
        {{alt('A')},
         {"select nested objects",
          select_object<ObjectFlags::Nested | ObjectFlags::ToBegin |
                        ObjectFlags::ToEnd>}},
