...
void StaticRegister::set(Context& context, ConstArrayView<String> values,
                         bool) {
...
const String& StaticRegister::get_main(const Context& context,
                                       size_t main_index) {
	auto content = get(context);
	return content[std::min(main_index, content.size() - 1)];
}

void HistoryRegister::set(Context& context, ConstArrayView<String> values,
                          bool restoring) {
	...
	if (restoring) {
		...
		m_content.erase(
		    remove_if(m_content, [](auto&& s) { return s.empty(); }),
		    m_content.end());
		return;
	}
	...
	if (m_content.size() > size_limit)
		m_content.erase(m_content.end() - (m_content.size() - size_limit),
		                m_content.end());
...
CandidateList
RegisterManager::complete_register_name(StringView prefix,
