...
void replace(Buffer& buffer, ArrayView<BufferRange> ranges,
             ConstArrayView<String> strings) {
	...
	for (size_t index = 0; index < ranges.size(); ++index) {
		...
		range = buffer.replace(
		    ...
		    strings.empty() ? StringView{}
		                    : strings[std::min(index, strings.size() - 1)]);
		kak_assert(buffer.is_valid(range.begin) and buffer.is_valid(range.end));
...
ColumnCount column_length(const Buffer& buffer, ColumnCount tabstop,
                          LineCount line) {
...
static BufferLines parse_lines(const char* pos, const char* end,
                               EolFormat eolformat,
	...
	while (pos < end) {
		...
		lines.emplace_back(StringData::create(
		    StringView{
		        pos,
		        eol - (eolformat == EolFormat::Crlf and eol != end ? 1 : 0)},
		    "\n"));
		pos = eol + 1;
	}
...
Buffer* create_buffer_from_string(String name, Buffer::Flags flags,
                                  StringView data) {
...
decltype(auto) parse_file(StringView filename, Func&& func) {
	...
	auto eolformat =
	    (has_crlf and not has_lf) ? EolFormat::Crlf : EolFormat::Lf;
	auto finaleol = pos == end           ? FinalEol::IfNotEmpty
	                : *(end - 1) == '\n' ? FinalEol::Present
	                                     : FinalEol::Missing;
	FsStatus fs_status{file.st.st_mtim, file.st.st_size,
	                   murmur3(file.data, file.st.st_size)};
	return func(parse_lines(pos, end, eolformat, line_count), bom, eolformat,
	            finaleol, fs_status);
}

Buffer* open_file_buffer(StringView filename, Buffer::Flags flags) {
	return parse_file(filename, [&](BufferLines&& lines, ByteOrderMark bom,
	                                EolFormat eolformat, FinalEol finaleol,
	                                FsStatus fs_status) {
		return BufferManager::instance().create_buffer(
		    filename.str(), Buffer::Flags::File | flags, std::move(lines), bom,
		    eolformat, finaleol, fs_status);
	});
}
...
Buffer* open_or_create_file_buffer(StringView filename, Buffer::Flags flags) {
	...
	return create_buffer_from_string(
	    filename.str(), Buffer::Flags::File | Buffer::Flags::New, StringView{});
}
...
void write_buffer_to_fd(Buffer& buffer, int fd) {
	...
	bool write_eol_at_eof = finaleol == FinalEol::Present or
	                        (finaleol == FinalEol::IfNotEmpty and
	                         (buffer.line_count() != 1 or buffer[0] != "\n"));
...
void write_buffer_to_file(Buffer& buffer, StringView filename,
	...
	if ((replace or force) and
	    (::stat(zfilename, &st) != 0 or (st.st_mode & S_IFMT) != S_IFREG)) {
		force = false;
		replace = false;
	}
	...
	if (force and ::chmod(zfilename, st.st_mode | S_IWUSR) < 0)
		throw runtime_error(
		    format("unable to change file permissions: {}", strerror(errno)));
	...
	if (replace and geteuid() == 0 and
	    ::chown(temp_filename, st.st_uid, st.st_gid) < 0)
		throw runtime_error(format(
		    "unable to set replacement file ownership: {}", strerror(errno)));
	if (replace and ::chmod(temp_filename, st.st_mode) < 0)
		throw runtime_error(format(
		    "unable to set replacement file permissions: {}", strerror(errno)));
	if (force and not replace and ::chmod(zfilename, st.st_mode) < 0)
		throw runtime_error(
		    format("unable to restore file permissions: {}", strerror(errno)));
...
Buffer* create_fifo_buffer(String name, int fd, Buffer::Flags flags,
	...
	if (buffer) {
		...
		buffer->reload({StringData::create("\n")}, ByteOrderMark::None,
		               EolFormat::Lf, FinalEol::Present, {InvalidTime, {}, {}});
	...
	struct FifoWatcher : FDWatcher {
		...
		~FifoWatcher() {
			...
			if (not m_buffer.values().contains(fifo_watcher_id))
				m_buffer.flags() &=
				    ~(Buffer::Flags::Fifo | Buffer::Flags::NoUndo);
		}
		...
		void read_fifo() {
			...
			// iteration number allows us to go back go back to the event loop
			// and handle other events sources (such as input)
			...
			{
				auto restore_flags =
				    OnScopeEnd([this, flags = m_buffer.flags()] {
				...
				do {
					...
					if ((m_scroll == AutoScroll::No and
					     (is_first or m_had_trailing_newline)) or
					    (m_scroll == AutoScroll::NotInitially and is_first))
			...
			if (insert_begin) {
				auto insert_back =
				    (m_had_trailing_newline and m_scroll == AutoScroll::No)
				        ? m_buffer.back_coord()
				        : m_buffer.prev(m_buffer.back_coord());
				m_buffer.run_hook_in_own_context(
				    Hook::BufReadFifo,
				    selection_to_string(ColumnType::Byte, m_buffer,
				                        {*insert_begin, insert_back}));
			}
	...
	buffer->values()[fifo_watcher_id] =
	    Value(Meta::Type<FifoWatcher>{}, fd, *buffer, scroll);
...
Vector<String> history_as_strings(ConstArrayView<Buffer::HistoryNode> history) {
	...
	for (auto& node : history) {
		auto seconds = std::chrono::duration_cast<std::chrono::seconds>(
		    node.committed.time_since_epoch());
