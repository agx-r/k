...
UnitTest test_diff{[]() {
	...
	auto check_diff = [](StringView a, StringView b,
	                     std::initializer_list<Diff> diffs) {
		size_t count = 0;
	...
	check_diff("a?", "!",
	           {{DiffOp::Remove, 1}, {DiffOp::Add, 1}, {DiffOp::Remove, 1}});
	check_diff("abcde", "cd",
	           {{DiffOp::Remove, 2}, {DiffOp::Keep, 2}, {DiffOp::Remove, 1}});
	check_diff("abcd", "cdef",
	           {{DiffOp::Remove, 2}, {DiffOp::Keep, 2}, {DiffOp::Add, 2}});
