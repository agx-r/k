...
Snake find_end_snake_of_further_reaching_dpath(IteratorA a, int N, IteratorB b,
...
Snake find_middle_snake(IteratorA a, int N, IteratorB b, int M, int* V1,
                        int* V2, int cost_limit, Equal&& eq) {
	const int delta = N - M;
	V1[1] = 0;
	V2[1] = 0;
	...
	for (int D = 0; D < max_D; ++D) {
		for (int k1 = -D; k1 <= D; k1 += 2) {
			auto p = find_end_snake_of_further_reaching_dpath<true>(
			    a, N, b, M, V1, D, k1, eq);
			V1[k1] = p.u;

			const int k2 = -(k1 - delta);
			if ((delta % 2 != 0) and -(D - 1) <= k2 and k2 <= (D - 1) and
			    V1[k1] + V2[k2] >= N)
		...
		for (int k2 = -D; k2 <= D; k2 += 2) {
			auto p = find_end_snake_of_further_reaching_dpath<false>(
			    a, N, b, M, V2, D, k2, eq);
			V2[k2] = p.u;

			const int k1 = -(k2 - delta);
			if ((delta % 2 == 0) and -D <= k1 and k1 <= D and
			    V1[k1] + V2[k2] >= N)
				return {
				    N - p.u, M - p.v, N - p.x, M - p.y,
				    (Snake::Op)(p.op +
				                Snake::RevAdd)}; // return last snake on reverse
				                                 // path, len = 2 * D
		}
	}
	...
	for (int k1 = -max_D; k1 <= max_D; k1 += 2) {
		auto p = find_end_snake_of_further_reaching_dpath<true>(a, N, b, M, V1,
		                                                        max_D, k1, eq);
		V1[k1] = p.u;
		if ((delta % 2 != 0) and p.u <= N and p.v <= M and
		    score(p) >= score(best))
			best = p;
	}
	for (int k2 = -max_D; k2 <= max_D; k2 += 2) {
		auto p = find_end_snake_of_further_reaching_dpath<false>(a, N, b, M, V2,
		                                                         max_D, k2, eq);
		V2[k2] = p.u;
		if ((delta % 2 == 0) and p.u <= N and p.v <= M and
		    score(p) >= score(best))
...
enum class DiffOp { Keep, Add, Remove };

template <typename IteratorA, typename IteratorB, typename Equal,
          typename OnDiff>
void find_diff_rec(IteratorA a, int begA, int endA, IteratorB b, int begB,
...
template <typename IteratorA, typename IteratorB, typename OnDiff,
          typename Equal = std::equal_to<>>
void for_each_diff(IteratorA a, int N, IteratorB b, int M, OnDiff&& on_diff,
                   Equal&& eq = Equal{}) {
	...
	find_diff_rec(a, 0, N, b, 0, M, &data[N + M], &data[max + N + M],
	              cost_limit, eq, [&last, &on_diff](DiffOp op, int len) {
		              if (last.op == op)
			              last.len += len;
		              else {
			              if (last.len != 0)
