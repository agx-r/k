...
enum class PromptEvent { Change, Abort, Validate };
using PromptCallback =
    MoveOnlyFunction<void(StringView, PromptEvent, Context&)>;
...
using PromptCompleter =
    Function<Completions(const Context&, StringView, ByteCount)>;
...
public:
	...
	void on_next_key(StringView mode_name, KeymapMode mode,
...
constexpr auto enum_desc(Meta::Type<AutoInfo>) {
	return make_array<EnumDesc<AutoInfo>>({{AutoInfo::Command, "command"},
	                                       {AutoInfo::OnKey, "onkey"},
	                                       {AutoInfo::Normal, "normal"}});
}

enum class AutoComplete { None = 0, Insert = 0b01, Prompt = 0b10 };
...
void on_next_key_with_autoinfo(const Context& context, StringView mode_name,
                               KeymapMode keymap_mode, Cmd cmd, String title,
                               String info) {
	context.input_handler().on_next_key(
	    mode_name, keymap_mode,
	    [cmd](Key key, Context& context) mutable {
