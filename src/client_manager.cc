...
ClientManager::~ClientManager() { clear(true); }
...
Client* ClientManager::create_client(UniquePtr<UserInterface>&& ui, int pid,
                                     String name, EnvVarMap env_vars,
	...
	if (init_coord) {
		context.selections_write_only() =
		    SelectionList(*buffer, context.buffer().clamp(*init_coord));
...
WindowAndSelections ClientManager::get_free_window(Buffer& buffer) {
	...
	auto it =
	    find_if(m_free_windows | reverse(), [&](const WindowAndSelections& ws) {
...
void ClientManager::add_free_window(UniquePtr<Window>&& window,
...
void ClientManager::ensure_no_client_uses_buffer(Buffer& buffer) {
	...
	m_free_windows.erase(
	    remove_if(m_free_windows,
	...
	for (auto&& removed_window : removed_windows) {
		removed_window->run_hook_in_own_context(
		    Hook::WinClose, removed_window->buffer().name());
...
CandidateList ClientManager::complete_client_name(StringView prefix,
	...
	auto c =
	    m_clients | transform([](const UniquePtr<Client>& c) -> const String& {
