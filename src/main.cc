...
static_assert(sizeof(version_notes) / sizeof(version_notes[0]) <=
              ...
              "Maximum 3 versions should be displayed in version notes, not "
...
void show_startup_info(Client* local_client, int last_version) {
	...
	for (auto [version, notes] : version_notes) {
		...
		for (auto&& line : notes | split<StringView>('\n'))
			info.push_back(
			    parse_display_line(line, GlobalScope::instance().faces()));
	}
	if (not info.empty())
		local_client->info_show(
		    {{{format("Kakoune {}", version), version_face},
		      {", more info at ", {}},
...
String config_directory() {
	if (StringView kak_cfg_dir = getenv("KAKOUNE_CONFIG_DIR");
	    not kak_cfg_dir.empty())
		return kak_cfg_dir.str();
	if (StringView xdg_cfg_home = getenv("XDG_CONFIG_HOME");
	    not xdg_cfg_home.empty())
...
const EnvVarDesc builtin_env_vars[] = {
    ...
    {"buflist", false,
     [](StringView name, const Context& context) -> Vector<String> {
	     return BufferManager::instance() | transform(&Buffer::display_name) |
    ...
    {"selections_char_desc", false,
     [](StringView name, const Context& context) -> Vector<String> {
	     return main_sel_first(context.selections()) |
	            transform([&buffer = context.buffer()](const Selection& sel) {
		            return selection_to_string(ColumnType::Codepoint, buffer,
		                                       sel);
	            }) |
	            gather<Vector<String>>();
     }},
    {"selections_display_column_desc", false,
     [](StringView name, const Context& context) -> Vector<String> {
	     return main_sel_first(context.selections()) |
	            transform([&buffer = context.buffer(),
	                       tabstop = context.options()["tabstop"].get<int>()](
	                          const Selection& sel) {
		            return selection_to_string(ColumnType::DisplayColumn,
		                                       buffer, sel, tabstop);
	            }) |
	            gather<Vector<String>>();
     }},
    ...
    {"window_range", false,
     [](StringView name, const Context& context) -> Vector<String> {
	     ...
	     const auto width =
	         context.window().dimensions().column - setup.widget_columns;
...
void register_registers() {
	...
	for (Codepoint c : StringView{"abcdefghijklmnopqrstuvwxyz\"^@"})
		register_manager.add_register(
		    c, make_unique_ptr<StaticRegister>(String{c}));

	for (Codepoint c : StringView{"/|:\\"})
		register_manager.add_register(
		    c, make_unique_ptr<HistoryRegister>(String{c}));
...
void register_options() {
	...
	reg.declare_option("finaleol", "write and end of line at end of file",
	                   FinalEol::Present);
	...
	reg.declare_option(
	    ...
	    Autoreload::Ask);
	reg.declare_option("writemethod", "how to write buffer to files",
...
UniquePtr<UserInterface> make_ui(UIType ui_type) {
	struct DummyUI : UserInterface {
		...
		void info_show(const DisplayLine&, const DisplayLineList&, DisplayCoord,
		               Face, InfoStyle) override {}
		void info_hide() override {}

		void draw(const DisplayBuffer&, DisplayCoord, const Face&, const Face&,
		          ColumnCount) override {}
		void draw_status(const DisplayLine&, const DisplayLine&,
		                 const ColumnCount, const DisplayLine&, const Face&,
		                 StatusStyle) override {}
	...
	case UIType::Terminal:
		return make_unique_ptr<TerminalUI>();
	case UIType::Json:
		return make_unique_ptr<JsonUI>();
	case UIType::Dummy:
		return make_unique_ptr<DummyUI>();
	}
	throw logic_error{};
}
...
int run_client(StringView session, StringView name, StringView client_init,
	...
	try {
		...
		RemoteClient client{
		    ...
		    get_env_vars(), client_init, std::move(init_coord), stdin_fd};
		stdin_fd.map(close);
...
int run_server(StringView session, StringView server_init,
               StringView client_init, StringView init_buffer,
               Optional<BufferCoord> init_coord, ServerFlags flags,
               UIType ui_type, DebugFlags debug_flags,
		...
		ProfileScope profile{
		    debug_flags, [&](std::chrono::microseconds duration) {
			    write_to_debug_buffer(format(
			        "running the unit tests took {} ms", duration.count()));
		    }};
...

	{
		...
		global_scope.hooks().run_hook(Hook::EnterDirectory, real_path("."),
		                              empty_context);
	...
	try {
		...
		while (not terminate and (not client_manager.empty() or
		                          server.negotiating() or server.is_daemon())) {
			...
			try {
				while (event_manager.handle_next_events(EventMode::Normal,
				                                        nullptr, timeout)) {
			...
			if (local_client and not contains(client_manager, local_client)) {
				...
				if ((not client_manager.empty() or server.is_daemon()) and
				    fork_server_to_background())
					exit(exit_status); // We do not want to run destructors and
					                   // hooks here
				...
				String selections = selection_list_to_string(
				    ColumnType::Byte, local_context.selections());
...
int main(int argc, char* argv[]) {
	...
	const ParameterDesc param_desc{SwitchMap{
	    {"c", {ArgCompleter{}, "connect to given session"}},
	    {"C",
	     {ArgCompleter{},
	      "connect to given session, create if it does not exist"}},
	    ...
	    {"p",
	     {ArgCompleter{}, "just send stdin as commands to the given session"}},
	    {"f",
	     {ArgCompleter{}, "filter: for each file, select the entire buffer and "
	                      "execute the given keys"}},
	    {"i",
	     {ArgCompleter{}, "backup the files on which a filter is applied using "
	                      "the given suffix"}},
	    {"q", {{}, "in filter mode, be quiet about errors applying keys"}},
	    {"ui",
	     {ArgCompleter{},
	      "set the type of user interface to use (terminal, dummy, or json)"}},
	...
	try {
		...
		const UIType ui_type =
		    parse_ui_type(parser.get_switch("ui").value_or("terminal"));
		...
		if ((bool)parser.get_switch("c") + (bool)parser.get_switch("C") +
		        (bool)parser.get_switch("s") >
		    1) {
			write_stderr("error: -s, -c and -C are mutually exclusive\n");
			return -1;
		}
		if (parser.get_switch("c") or
		    (parser.get_switch("C") and
		     check_session(*parser.get_switch("C")))) {
			for (auto opt : {"n", "d", "E", "ro"}) {
				if (parser.get_switch(opt)) {
					write_stderr(format("error: -{} incompatible with "
					                    "connecting to an existing session\n",
					                    opt));
					return -1;
				}
			}
			StringView session = parser.get_switch("c")
			                         ? *parser.get_switch("c")
			                         : *parser.get_switch("C");
			String new_files;
			for (auto name : files) {
			...
			return run_client(session, {}, new_files + client_init, init_coord,
			                  ui_type, false);
		} else {
			StringView session = parser.get_switch("C").value_or(
			    parser.get_switch("s").value_or(StringView{}));
			try {
				auto ignore_kakrc = (bool)parser.get_switch("n");
				auto flags =
				    (ignore_kakrc ? ServerFlags::IgnoreKakrc
				                  : ServerFlags::None) |
				    (parser.get_switch("d") ? ServerFlags::Daemon
				                            : ServerFlags::None) |
				    (parser.get_switch("ro") ? ServerFlags::ReadOnly
				                             : ServerFlags::None) |
				    ((argc == 1 or (ignore_kakrc and argc == 2)) and isatty(0)
				         ? ServerFlags::StartupInfo
				         : ServerFlags::None);
				auto debug_flags =
				    option_from_string(Meta::Type<DebugFlags>{},
				                       parser.get_switch("debug").value_or(""));
				return run_server(session, server_init, client_init,
				                  files.empty() ? StringView{} : files[0],
				                  init_coord, flags, ui_type, debug_flags,
				                  files);
