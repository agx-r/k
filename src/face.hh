...
struct Face {
	...
	friend constexpr bool operator==(const Face& lhs,
	                                 const Face& rhs) = default;
...
inline Face merge_faces(const Face& base, const Face& face) {
	auto alpha_blend = [](Color base, Color color) {
		auto blend = [&](unsigned char Color::* field) {
			int blended =
			    (base.*field * (255 - color.a) + color.*field * color.a) / 255;
	...
	auto choose = [&](Color Face::* color, Attribute final_attr) {
		...
		if (face.*color == Color::Default)
			return base.*color;
		if ((base.*color).isRGB() and (face.*color).isRGB() and
		    (face.*color).a != 255)
	...
	return Face{choose(&Face::fg, Attribute::FinalFg),
	            ...
	            face.attributes & Attribute::FinalAttr
	                ? face.attributes |
	                      Attribute{base.attributes & Attribute::Final}
