...
template <size_t N> InplaceString<N> to_string_impl(auto val, auto format) {
	InplaceString<N> res;
...
template <size_t N> InplaceString<N> to_string_impl(auto val) {
	return to_string_impl<N>(val, 10);
}

InplaceString<15> to_string(int val) { return to_string_impl<15>(val); }

InplaceString<15> to_string(unsigned val) { return to_string_impl<15>(val); }

InplaceString<23> to_string(long int val) { return to_string_impl<23>(val); }
...
InplaceString<23> to_string(Hex val) { return to_string_impl<23>(val.val, 16); }
...
void format_impl(StringView fmt, ArrayView<const StringView> params,
	...
	for (auto it = fmt.begin(), end = fmt.end(); it != end;) {
		...
		if (opening == end) {
			...
			const int index = opening + 1 == format
			                      ? implicitIndex
			                      : str_to_int({opening + 1, format});
			...
			if (format != closing) {
				...
				for (ColumnCount width = str_to_int({format, closing}),
				                 len = params[index].column_length();
				     width > len; --width)
					append(padding);
			}
...
void format_with(FunctionRef<void(StringView)> append, StringView fmt,
                 ArrayView<const StringView> params) {
