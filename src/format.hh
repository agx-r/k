...
template <size_t N> struct InplaceString {
...
template <typename T>
    requires(not std::is_convertible_v<T, StringView>)
...
template <typename... Types> String format(StringView fmt, Types&&... params) {
...
void format_with(FunctionRef<void(StringView)> append, StringView fmt,
                 Types&&... params) {
	return format_with(append, fmt,
	                   ArrayView<const StringView>{detail::format_param(
	                       std::forward<Types>(params))...});
}
