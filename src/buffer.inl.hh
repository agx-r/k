...
inline ByteCount Buffer::distance(ArrayView<const StringDataPtr> lines,
...
inline ConstArrayView<Buffer::Change>
Buffer::changes_since(size_t timestamp) const {
...
inline BufferIterator::BufferIterator(const StringDataPtr* lines,
                                      LineCount line_count,
                                      BufferCoord coord) noexcept
    : m_lines{lines},
      m_line{coord.line < line_count ? m_lines[(size_t)coord.line]->strview()
                                     : StringView{}},
      m_line_count{line_count}, m_coord{coord} {}

inline bool
BufferIterator::operator==(const BufferIterator& iterator) const noexcept {
...
inline bool
BufferIterator::operator==(const BufferCoord& coord) const noexcept {
...
inline const char& BufferIterator::operator[](size_t n) const noexcept {
	auto coord =
	    Buffer::advance({m_lines, (size_t)(int)m_line_count}, m_coord, n);
...
inline size_t BufferIterator::operator-(const BufferIterator& iterator) const {
	kak_assert(m_lines == iterator.m_lines);
	return (size_t)Buffer::distance({m_lines, (size_t)(int)m_line_count},
	                                iterator.m_coord, m_coord);
}
...
inline BufferIterator BufferIterator::operator+(ByteCount size) const {
	...
	return {
	    ...
	    Buffer::advance({m_lines, (size_t)(int)m_line_count}, m_coord, size)};
}
...
inline BufferIterator& BufferIterator::operator++() {
	if (++m_coord.column == m_line.length()) {
		m_line = ((size_t)++m_coord.line < m_line_count)
		             ? m_lines[(size_t)m_coord.line]->strview()
		             : StringView{};
		m_coord.column = 0;
	}
	return *this;
