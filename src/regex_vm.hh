...
struct CharacterClass {
	...
	bool matches(Codepoint cp) const {
		...
		return (ctypes != CharacterType::None and is_ctype(ctypes, cp)) !=
		       negative;
	}
};
...
struct CompiledRegex : UseMemoryDomain<MemoryDomain::Regex> {
	...
	union Param {
		using bool_t = uint8_t; // on ppc bool can be 4 bytes, which breaks
		                        // packing of this union
...
template <typename It> struct SentinelType {
	using Type = It;
};

template <typename It>
    requires requires { typename It::Sentinel; }
struct SentinelType<It> {
...
public:
	...
	bool exec(const Iterator& begin, const Iterator& end,
		...
		const ExecConfig config{
		    Sentinel{forward ? begin : end}, Sentinel{forward ? end : begin},
		    Sentinel{subject_begin}, Sentinel{subject_end}, flags};
...
private:
	...
	template <bool copy> int16_t new_saves(Iterator* pos, uint32_t valid_mask) {
		...
		auto* new_pos =
		    reinterpret_cast<Iterator*>(operator new(count * sizeof(Iterator)));
	...
	[[gnu::always_inline]]
	void step_current_thread(const Iterator& pos, Codepoint cp,
	                         uint16_t current_step, const ExecConfig& config) {
		...
		while (true) {
			...
			case CompiledRegex::Match:
				if ((pos != config.end and not(mode & RegexMode::Search)) or
				    (config.flags & RegexExecFlags::NotInitialNull and
				     pos == config.begin))
					return failed();
			...
			case CompiledRegex::Literal:
				if (pos != config.end and
				    param.literal.codepoint ==
				        (param.literal.ignore_case ? to_lower(cp) : cp))
					return consumed();
				return failed();
			...
			case CompiledRegex::CharRange:
				if (auto actual_cp =
				        (param.range.ignore_case ? to_lower(cp) : cp);
				    pos != config.end and
				    (actual_cp >= param.range.min and
				     actual_cp <= param.range.max) != param.range.negative)
					return consumed();
				return failed();
			...
			case CompiledRegex::CharClass:
				if (pos != config.end and
				    m_program.character_classes[param.character_class_index]
				        .matches(cp))
					return consumed();
				return failed();
			...
			case CompiledRegex::Save:
				...
				else if (auto& saves = m_saves[thread.saves];
				         saves.refcount > 1) {
					--saves.refcount;
			...
			case CompiledRegex::LineAssertion:
				if (not(param.line_start ? is_line_start(pos, config)
				                         : is_line_end(pos, config)))
					return failed();
				break;
			case CompiledRegex::SubjectAssertion:
				if (pos != (param.subject_begin ? config.subject_begin
				                                : config.subject_end))
					return failed();
				break;
			case CompiledRegex::WordBoundary:
				if (is_word_boundary(pos, config) !=
				    param.word_boundary_positive)
					return failed();
				break;
			case CompiledRegex::LookAround:
				if (lookaround(param.lookaround, pos, config) !=
				    param.lookaround.positive)
					return failed();
				break;
			}
		}
	...
	void exec_program(const Iterator& start, const ExecConfig& config,
	                  auto&& idle_func) {
		...
		if (start_desc) {
			...
			if (next_start == config.end or // Non null start_desc means we
			                                // consume at least one char
			    (not(mode & RegexMode::Search) and
			     not start_desc->map[static_cast<unsigned char>(
			         forward ? *start : *std::prev(start))]))
				return;
		}

		const auto insts = forward
		                       ? ArrayView(m_program.instructions)
		                             .subrange(0, m_program.first_backward_inst)
		                       : ArrayView(m_program.instructions)
		                             .subrange(m_program.first_backward_inst);
		m_threads.push_current({insts.begin(), -1});

		uint16_t current_step = -1;
		uint8_t idle_count =
		    0; // Run idle loop every 256 * 65536 == 16M codepoints
		...
		while (pos != config.end) {
			...
			if ((mode & RegexMode::Search) and not m_found_match) {
				if (start_desc) {
					if (pos == next_start)
						next_start =
						    find_next_start(next, config.end, *start_desc);
					if (m_threads.next_is_empty())
						next = next_start;
				}
				if (not start_desc or next == next_start)
					m_threads.push_next({insts.begin(), -1});
			} else if (m_threads.next_is_empty() or
			           (m_found_match and (mode & RegexMode::AnyMatch)))
				return;
	...
	static Iterator find_next_start(const Iterator& start, const Sentinel& end,
		...
		if (char start_byte = start_desc.start_byte) {
			while (pos != end) {
				if constexpr (forward) {
					if (*pos == start_byte)
						return utf8::advance(pos, start,
						                     -CharCount(start_desc.offset));
					++pos;
				} else {
		...
		while (pos != end) {
			static_assert(StartDesc::count <= 256,
			              "start desc should be ascii only");
			if constexpr (forward) {
	...
	static bool is_word_boundary(const Iterator& pos,
	                             const ExecConfig& config) {
		if (pos == config.subject_begin)
	...
	// This stores both the current thread stack and next thread stack in a ring
	// buffer current threads go from m_current to m_next_begin (pushing
	// decrements m_current) next thrads go from m_next_begin to m_next_end
	// (pushing increments m_next_end)
	struct DualThreadStack {
		...
		void push_current(Thread thread) {
			m_data[decrement(m_current)] = thread;
			grow_ifn(true);
		}
		[[gnu::always_inline]]
		Thread pop_current() {
			return m_data[post_increment(m_current)];
		}

		[[gnu::always_inline]]
		void push_next(Thread thread) {
			m_data[post_increment(m_next_end)] = thread;
			grow_ifn(false);
		}
		[[gnu::always_inline]]
		Thread pop_next() {
			return m_data[decrement(m_next_end)];
		}
		...
		void ensure_initial_capacity() {
			...
			static_assert(initial_capacity >= 4 and
			              std::has_single_bit(initial_capacity));
		...
		void grow(bool pushed_current) {
			...
			// copy the whole data to avoid having to consider either stack
			// wrapping around
			std::rotate_copy(old_data, old_data + m_current,
			                 old_data + capacity, new_data);
