...
public:
	...
	Client(UniquePtr<UserInterface>&& ui, UniquePtr<Window>&& window,
	       SelectionList selections, int pid, EnvVarMap env_vars, String name,
	...
	void info_show(DisplayLine title, DisplayLineList content,
	               BufferCoord anchor, InfoStyle style);
	void info_show(StringView title, StringView content, BufferCoord anchor,
	               InfoStyle style);
	void info_hide(bool even_modal = false);
	bool info_pending() const { return m_ui_pending & PendingUI::InfoShow; };
	bool status_line_pending() const {
		return m_ui_pending & PendingUI::StatusLine;
	};

	void print_status(DisplayLine prompt, DisplayLine content,
	...
	void change_buffer(Buffer& buffer,
	                   Optional<FunctionRef<void()>> set_selection);
...
private:
	...
	enum class PendingClear { None = 0, Info = 0b01, StatusLine = 0b10 };
	friend constexpr bool with_bit_ops(Meta::Type<PendingClear>) {
		return true;
	}
...
enum class Autoreload { Yes, No, Ask };

constexpr auto enum_desc(Meta::Type<Autoreload>) {
	return make_array<EnumDesc<Autoreload>>({{Autoreload::Yes, "yes"},
	                                         {Autoreload::No, "no"},
	                                         {Autoreload::Ask, "ask"},
	                                         {Autoreload::Yes, "true"},
	                                         {Autoreload::No, "false"}});
