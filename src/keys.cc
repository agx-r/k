...
static Key canonicalize_ifn(Key key) {
	...
	if (key.modifiers & Key::Modifiers::Shift) {
		if (is_basic_alpha(key.key)) {
			...
			throw key_parse_error(format("Shift modifier only works on special "
			                             "keys and lowercase ASCII, not '{}'",
			                             key.key));
		}
	}
...
Key::MouseButton str_to_button(StringView str) {
	if (str == "left")
		return Key::MouseButton::Left;
	if (str == "middle")
		return Key::MouseButton::Middle;
	if (str == "right")
		return Key::MouseButton::Right;
...
String to_string(Key key) {
	...
	if (key.modifiers & Key::Modifiers::MousePos)
		...
		res = format("mouse:press:{}:{}.{}", key.mouse_button(), coord.line,
		             coord.column);
	else if (key.modifiers & Key::Modifiers::MouseRelease)
		res = format("mouse:release:{}:{}.{}", key.mouse_button(), coord.line,
		             coord.column);
	else if (key.modifiers & Key::Modifiers::Scroll)
		res = format("scroll:{}:{}.{}", key.scroll_amount(), coord.line,
		             coord.column);
...
UnitTest test_keys{[]() {
	KeyList keys{
	    {Key::Space},   {'c'},     {Key::Up},  alt('j'),  ctrl('r'),
	    shift(Key::Up), ctrl('['), ctrl('\\'), ctrl(']'), ctrl('_'),
	};
	String keys_as_str;
	for (auto& key : keys)
