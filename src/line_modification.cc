...
Vector<LineModification> compute_line_modifications(const Buffer& buffer,
	...
	for (auto& buf_change : buffer.changes_since(timestamp)) {
		...
		auto pos =
		    std::upper_bound(res.begin(), res.end(), change.new_line,
		...
		if (pos != res.begin()) {
			...
			if (change.new_line <= prev.new_line + prev.num_added) {
				...
				pos->num_removed +=
				    change.num_removed - removed_from_previously_added_by_pos;
				pos->num_added +=
...
void LineRangeSet::update(ConstArrayView<LineModification> modifs) {
	...
	for (auto it = begin(); it != end(); ++it) {
		auto modif_beg =
		    std::lower_bound(modifs.begin(), modifs.end(), it->begin,
		                     [](const LineModification& c, const LineCount& l) {
			                     return c.old_line + c.num_removed < l;
		                     });
		auto modif_end =
		    std::upper_bound(modifs.begin(), modifs.end(), it->end,
		                     [](const LineCount& l, const LineModification& c) {
			                     return l < c.old_line;
		                     });
...
void LineRangeSet::add_range(LineRange range,
                             FunctionRef<void(LineRange)> on_new_range) {
...
UnitTest test_line_modifications{[]() {
...

	{
		...
		kak_assert(modifs.size() == 1 and
		           modifs[0] == LineModification{1, 1, 1, 0});
	}

	{
		...
		auto modifs = compute_line_modifications(buffer, ts);
		kak_assert(modifs.size() == 1 and
		           modifs[0] == LineModification{2, 2, 0, 1});
	}

	{
		...
		auto modifs = compute_line_modifications(buffer, ts);
		kak_assert(modifs.size() == 1 and
		           modifs[0] == LineModification{0, 0, 2, 2});
	}

	{
		Buffer buffer(
		    "test", Buffer::Flags::None,
		    make_lines("line 1\n", "line 2\n", "line 3\n", "line 4\n"));
		...
		buffer.erase({0, 0}, {1, 0});
		{
			auto modifs = compute_line_modifications(buffer, ts);
			kak_assert(modifs.size() == 1 and
			           modifs[0] == LineModification{0, 0, 4, 3});
		}
...

		{
			auto modifs = compute_line_modifications(buffer, ts);
			kak_assert(modifs.size() == 1 and
			           modifs[0] == LineModification{0, 0, 4, 4});
		}
	}

	{
		...
		auto modifs = compute_line_modifications(buffer, ts);
		kak_assert(modifs.size() == 1 and
		           modifs[0] == LineModification{0, 0, 1, 1});
	}
}};
...
UnitTest test_line_range_set{[] {
	...
	}
	{
		...
		kak_assert((ranges.view() == ConstArrayView<LineRange>{
		                                 {0, 1}, {2, 7}, {12, 17}, {22, 27}}));
	}
}};
