...
Context::Context(InputHandler& input_handler, SelectionList selections,
    ...
    : m_flags(flags), m_input_handler{&input_handler},
...
FaceRegistry& Context::faces(bool allow_local) const {
	return scope(allow_local).faces();
}
...
void Context::print_status(DisplayLine prompt, DisplayLine content,
                           ColumnCount cursor_pos, StatusStyle style) const {
	if (has_client())
...
Context::SelectionHistory::SelectionHistory(Context& context)
    : m_context(context) {}
...
SelectionList& Context::SelectionHistory::selections(bool update) {
	...
	auto& sels =
	    m_staging ? m_staging->selections : current_history_node().selections;
	if (update)
		sels.update();
	return sels;
}
...
void Context::SelectionHistory::end_edition() {
	...
	if (m_history_id != HistoryId::Invalid and
	    current_history_node().selections == m_staging->selections) {
		// No change, except maybe the index of the main selection.
		// Update timestamp to potentially improve interaction with content
		// undo.
...
template <Direction direction> void Context::SelectionHistory::undo() {
	...
	do {
		...
		if (next == HistoryId::Invalid)
			throw runtime_error(backward ? "no selection change to undo"
		...
		if (&destination_buffer == &m_context.buffer())
			...
			m_context.change_buffer(destination_buffer,
			                        {std::move(select_next)});
...
void Context::SelectionHistory::forget_buffer(Buffer& buffer) {
	...
	auto new_id = [&new_ids](HistoryId old_id) -> HistoryId {
		return old_id == HistoryId::Invalid ? HistoryId::Invalid
		                                    : new_ids[(size_t)old_id];
	};

	m_history.erase(remove_if(m_history,
...
void Context::forget_buffer(Buffer& buffer) {
	...
	if (&this->buffer() == &buffer) {
		...
		change_buffer(last_buffer
		                  ? *last_buffer
		                  : BufferManager::instance().get_first_buffer());
	}
...
Buffer* Context::last_buffer() const {
	...
	auto next_buffer =
	    find_if(jump_list.subrange(m_jump_list.current_index() - 1), predicate);
	...
	return previous_buffer != jump_list.rend() ? &previous_buffer->buffer()
	                                           : nullptr;
}
...
template <Direction direction> void Context::undo_selection_change() {
...
SelectionList& Context::selections_write_only() { return selections(false); }
...
void Context::set_name(String name) {
	...
	hooks().run_hook(Hook::ClientRenamed, format("{}:{}", old_name, m_name),
	                 *this);
}
...
ScopedSelectionEdition::ScopedSelectionEdition(Context& context)
    : m_context{context},
      m_valid{not(m_context.flags() & Context::Flags::Draft) and
              context.has_buffer()} {
	if (m_valid)
		m_context.m_selection_history.begin_edition();
}

ScopedSelectionEdition::ScopedSelectionEdition(ScopedSelectionEdition&& other)
    : m_context{other.m_context}, m_valid{other.m_valid} {
	other.m_valid = false;
}

ScopedSelectionEdition::~ScopedSelectionEdition() {
	if (m_valid)
