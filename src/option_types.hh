...
String option_to_string(const T& value, Quoting)
    requires std::is_same_v<decltype(option_to_string(std::declval<T>())),
                            String>
{
...
inline String option_to_string(Codepoint opt, Quoting quoting) {
	return quoter(quoting)(to_string(opt));
}
...
Vector<String> option_to_strings(const Vector<T, domain>& opt) {
	return opt | transform([](const T& t) {
		       return option_to_string(t, Quoting::Raw);
	       }) |
...
String option_to_string(const Vector<T, domain>& opt, Quoting quoting) {
	return join(opt | transform([=](const T& t) {
		            return option_to_string(t, quoting);
	            }),
	            ' ', false);
}
...
template <typename T, MemoryDomain domain>
bool option_remove_from_strings(Vector<T, domain>& opt,
                                ConstArrayView<String> strs) {
	bool did_remove = false;
	for (auto&& val : strs | transform([](auto&& s) {
		                  return option_from_string(Meta::Type<T>{}, s);
	                  })) {
...
template <typename Key, typename Value, MemoryDomain domain>
bool option_remove_from_strings(HashMap<Key, Value, domain>& opt,
                                ConstArrayView<String> strs) {
	struct error : runtime_error {
		error(size_t) : runtime_error{"map option expects key=value"} {}
	};
	...
	for (auto&& str : strs) {
		...
		if (auto it = opt.find(key_value[0]);
		    it != opt.end() and
		    (key_value[1].empty() or key_value[1] == it->value)) {
			opt.remove(it->key);
			changed = true;
		}
	}
...
template <typename Key, typename Value, MemoryDomain domain>
HashMap<Key, Value, domain>
option_from_strings(Meta::Type<HashMap<Key, Value, domain>>,
...
String option_to_string_impl(Quoting quoting, const std::tuple<Types...>& opt,
                             std::index_sequence<I...>) {
	return quoter(quoting)(
	    join(make_array({option_to_string(std::get<I>(opt), Quoting::Raw)...}),
	         tuple_separator));
}

template <typename... Types>
String option_to_string(const std::tuple<Types...>& opt, Quoting quoting) {
	return option_to_string_impl(quoting, opt,
...
inline String
option_to_string(const StronglyTypedNumber<RealType, ValueType>& opt) {
...
struct WorstMatch {
	template <typename T> WorstMatch(T&&) {}
};
...
template <typename Coord>
    requires std::is_base_of_v<
        LineAndColumn<Coord, decltype(Coord::line), decltype(Coord::column)>,
        Coord>
...
template <DescribedEnum Flags>
    requires WithBitOps<Flags>
...
template <DescribedEnum Enum>
    requires(not WithBitOps<Enum>)
...
Enum option_from_string(Meta::Type<Enum>, StringView str) {
	constexpr auto desc = enum_desc(Meta::Type<Enum>{});
	auto it =
	    find_if(desc, [str](const EnumDesc<Enum>& d) { return d.name == str; });
	if (it == desc.end())
...
template <typename P, typename T>
inline bool option_remove_from_strings(PrefixedList<P, T>& opt,
                                       ConstArrayView<String> str) {
