...
public:
	...
	DisplayAtom(const Buffer& buffer, BufferRange range, String str,
	            Face face = {})
	    : face(face), m_type(ReplacedRange), m_buffer(&buffer), m_range{range},
	      m_text{std::move(str)} {}

	DisplayAtom(String str, Face face)
	    : face(face), m_type(Text), m_text(std::move(str)) {}

	explicit DisplayAtom(String str) : DisplayAtom(std::move(str), Face{}) {}
...
public:
	...
	template <typename It> iterator insert(iterator pos, It beg, It end) {
		auto has_buffer_range = [](const DisplayAtom& atom) {
			return atom.has_buffer_range();
		};
		auto had_range = any_of(*this, has_buffer_range);
		if (auto first = std::find_if(beg, end, has_buffer_range);
		    first != end) {
			auto& last =
			    *std::find_if(std::reverse_iterator(end),
			                  std::reverse_iterator(first), has_buffer_range);
			m_range.begin = had_range
			                    ? std::min(m_range.begin, (*first).begin())
			                    : (*first).begin();
			m_range.end =
			    had_range ? std::max(m_range.end, last.end()) : last.end();
		}
	...
	// the line is less that col_count character
	bool trim_from(ColumnCount first_col, ColumnCount front,
...
DisplayLine
parse_display_line(StringView line, const FaceRegistry& faces,
