...
public:
	...
	template <Direction direction> void undo_selection_change();
	...
	Scope* local_scope() const {
		return m_local_scopes.empty() ? nullptr : m_local_scopes.back();
	}
	...
	template <typename Func> void set_last_select(Func&& last_select) {
...
private:
	...
	public:
		...
		template <Direction direction> void undo();
		void forget_buffer(Buffer& buffer);
	...
	private:
		...
		struct HistoryNode {
			HistoryNode(SelectionList selections, HistoryId parent)
			    : selections(selections), parent(parent) {}
		...
		HistoryNode& history_node(HistoryId id) {
		...
		HistoryNode& current_history_node() {
			kak_assert((size_t)m_history_id < m_history.size());
