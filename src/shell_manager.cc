...
ShellManager::ShellManager(ConstArrayView<EnvVarDesc> builtin_env_vars)
	...
	// have access to the kak command regardless of if the user installed it
	{
		...
		auto new_path = format("{}../libexec/kak:{}",
		                       split_path(get_kak_binary_path()).first, path);
...
Shell spawn_shell(const char* shell, StringView cmdline,
	...
	auto make_pipe = []() -> Array<UniqueFd, 2> {
		...
		throw runtime_error(
		    format("unable to create pipe, errno: {}", ::strerror(errno)));
	};

	auto stdin_pipe =
	    open_stdin ? make_pipe()
	               : Array{UniqueFd{open("/dev/null", O_RDONLY)}, UniqueFd{}};
	auto stdout_pipe = make_pipe();
	auto stderr_pipe = make_pipe();
	if (pid_t pid = vfork())
		return {pid, std::move(stdin_pipe[1]), std::move(stdout_pipe[0]),
		        std::move(stderr_pipe[0])};
	...
	char buffer[1024];
	write(STDERR_FILENO,
	      format_to(buffer, "execve failed: {}\n", strerror(errno)));
	_exit(-1);
	return {-1, {}, {}, {}};
}

template <typename GetValue>
Vector<String> generate_env(StringView cmdline, ConstArrayView<String> params,
                            const Context& context, GetValue&& get_value) {
	...
	auto add_matches = [&](StringView s) {
		for (auto&& match : RegexIterator{s.begin(), s.end(), re}) {
			...
			try {
				...
				Quoting quoting =
				    match[1].matched ? Quoting::Shell : Quoting::Raw;
				env.push_back(format("kak_{}{}={}", quoted, name,
				                     get_value(name, quoting)));
			} catch (runtime_error&) {
			}
		}
	};
...
FDWatcher make_pipe_writer(UniqueFd& fd,
                           const FunctionRef<StringView()>& generator) {
	int flags = fcntl((int)fd, F_GETFL, 0);
	fcntl((int)fd, F_SETFL, flags | O_NONBLOCK);
	return {
	    (int)fd, FdEvents::Write, EventMode::Urgent,
...
struct CommandFifos {
	      ...
	      command_watcher([&] {
		      if (mkdtemp(base_dir.data()) == nullptr or
			      ...
			      throw runtime_error(format(
			          "unable to create command/response fifos, errno: {}",
			          ::strerror(errno)));
		      ...
		      return make_reader(fd, command, [&, fd](bool graceful) {
			      if (not graceful) {
				      write_to_debug_buffer(
				          format("error reading from command fifo '{}'",
				                 strerror(errno)));
				      return;
			      }
	...
	String command_fifo_path() const {
		return format("{}/command-fifo", base_dir);
	}
	String response_fifo_path() const {
		return format("{}/response-fifo", base_dir);
	}
};
...
std::pair<String, int>
ShellManager::eval(StringView cmdline, const Context& context,
                   FunctionRef<StringView()> input_generator, Flags flags,
	...
	if (debug_flags & DebugFlags::Shell)
		write_to_debug_buffer(
		    format("shell:\n{}\n----\nargs: {}\n----\n", cmdline,
		           join(shell_context.params | transform(shell_quote), ' ')));

	auto start_time = profile ? Clock::now() : Clock::time_point{};

	Optional<CommandFifos> command_fifos;

	auto kak_env = generate_env(
	    cmdline, shell_context.params, context,
	    [&](StringView name, Quoting quoting) {
		    if (name == "command_fifo" or name == "response_fifo") {
			    if (not command_fifos)
				    command_fifos.emplace(const_cast<Context&>(context),
				                          shell_context);
			    return name == "command_fifo"
			               ? command_fifos->command_fifo_path()
			               : command_fifos->response_fifo_path();
		    }

		    if (auto it = shell_context.env_vars.find(name);
		        it != shell_context.env_vars.end())
			    return it->value;
		    return join(get_val(name, context) | transform(quoter(quoting)),
		                ' ', false);
	    });

	auto spawn_time = profile ? Clock::now() : Clock::time_point{};
	auto shell = spawn_shell(m_shell.c_str(), cmdline, shell_context.params,
	                         kak_env, true);
	auto wait_time = Clock::now();

	String stdout_contents, stderr_contents;
	auto stdout_reader = make_reader((int)shell.out, stdout_contents,
	                                 [&](bool) { shell.out.close(); });
	auto stderr_reader = make_reader((int)shell.err, stderr_contents,
	                                 [&](bool) { shell.err.close(); });
	...
	auto restore_mask =
	    OnScopeEnd([&] { sigprocmask(SIG_SETMASK, &orig_mask, nullptr); });
	...
	while (not terminated or shell.in or
		...
		try {
			...
			write_to_debug_buffer(
			    format("error while waiting for shell: {}", error.what()));
			failed = true;
		}
		if (not terminated)
			terminated =
			    waitpid((int)shell.pid, &status, WNOHANG) == (int)shell.pid;
	}
...
Shell ShellManager::spawn(StringView cmdline, const Context& context,
	...
	return spawn_shell(m_shell.c_str(), cmdline, shell_context.params, kak_env,
	                   open_stdin);
}

Vector<String> ShellManager::get_val(StringView name,
