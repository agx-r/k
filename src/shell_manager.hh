...
public:
	...
	enum class Flags { None = 0, WaitForStdout = 1 };
	...
	std::pair<String, int> eval(StringView cmdline, const Context& context,
		...
		return eval(
		    cmdline, context,
		    [in]() mutable { return std::exchange(in, StringView{}); }, flags,
		    shell_context);
	}

	Shell spawn(StringView cmdline, const Context& context, bool open_stdin,
	            const ShellContext& shell_context = {});
