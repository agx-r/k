...
InsertCompletion complete_word(const SelectionList& sels,
	...
	for (int i = 0; i < sels.size(); ++i) {
		int len = 0;
		auto is_short_enough_word = [&](Codepoint c) {
			return len++ < WordSplitter::max_word_len and is_word_pred(c);
		};

		Utf8It end{buffer.iterator_at(sels[i].cursor()), buffer};
		Utf8It begin = end - 1;
		if (not skip_while_reverse(begin, buffer.begin(),
		                           is_short_enough_word) and
	...
	if (other_buffers) {
		for (const auto& buf : BufferManager::instance()) {
			if (buf.get() == &buffer or buf->flags() & Buffer::Flags::Debug)
				continue;
			for (auto& m :
			     get_word_db(*buf).find_matching(prefix) |
			         // filter out words that are not considered words for the
			         // current buffer
			         filter([&](auto& rm) {
				         auto&& c = rm.candidate();
	...
	auto limit = [](StringView s, ColumnCount l) {
		return s.column_length() <= l
		           ? s.str()
		           : "…" + s.substr(s.column_length() - (l + 1));
	};
	...
	for_n_best(
	    ...
	    [&](RankedMatchAndBuffer& m) {
		    if (not candidates.empty() and
		        candidates.back().completion == m.candidate())
			    return false;
		    ...
		    if (other_buffers and m.buffer) {
			    ...
			    menu_entry.push_back(
			        {limit(m.buffer->display_name(), 20), faces["MenuInfo"]});
		    } else
			    menu_entry.push_back({m.candidate().str(), {}});
...
InsertCompletion complete_option(const SelectionList& sels,
	...
	for_n_best(
	    ...
	    [&](RankedMatchAndInfo& m) {
		    if (not candidates.empty() and
		        candidates.back().completion == m.candidate() and
		        candidates.back().on_select == m.on_select)
			    return false;
...
InsertCompletion complete_line(const SelectionList& sels,
	...
	StringView prefix = trim_leading_whitespaces(
	...
	auto add_candidates = [&](const Buffer& buf) {
		for (LineCount l = 0_line; l < buf.line_count(); ++l) {
			...
			StringView candidate = trim_leading_whitespaces(
			    line.substr(0_byte, line.length() - 1));
	...
	if (other_buffers) {
		for (const auto& buf : BufferManager::instance()) {
			if (buf.get() != &buffer and
			    not(buf->flags() & Buffer::Flags::Debug))
				add_candidates(*buf);
		}
	}
	...
	candidates.erase(std::unique(candidates.begin(), candidates.end()),
	                 candidates.end());
	return {std::move(candidates), replace_begin, cursor_pos,
	        buffer.timestamp()};
}
...
InsertCompleter::InsertCompleter(Context& context)
    : m_context(context),
      // local scopes might go away before completion ends, make sure to
      // register on a long lived one
...
void InsertCompleter::select(int index, bool relative,
                             FunctionRef<void(Key)> record_key) {
	m_enabled = true;
	if (not setup_ifn())
		return;
	...
	for (auto& sel : selections) {
		...
		if (pos.coord().column >= prefix_len and
		    (pos + suffix_len) != buffer.end() and
		    std::equal(ref.begin(), ref.end(), pos - prefix_len))
			ranges.push_back(
			    {(pos - prefix_len).coord(), (pos + suffix_len).coord()});
	}
...
bool InsertCompleter::has_candidate_selected() const {
	return m_completions.is_valid() and m_current_candidate >= 0 and
...
void InsertCompleter::reset() {
	...
	if (m_context.has_client() and has_candidate_selected()) {
		...
		hook_param = join(m_inserted_ranges | filter([](auto&& r) {
			                  return not r.empty();
		                  }) | transform([&](auto&& r) {
			                  return selection_to_string(
	...
	if (m_context.has_client()) {
		...
		m_context.hooks().run_hook(Hook::InsertCompletionHide, hook_param,
		                           m_context);
	}
}
		...
		for (auto& completer : completers) {
			...
			if (completer.mode == InsertCompleterDesc::Word and
			    *completer.param == "buffer" and
				...
				return true;
			if (completer.mode == InsertCompleterDesc::Word and
			    *completer.param == "all" and try_complete(complete_word<true>))
				return true;
			if (completer.mode == InsertCompleterDesc::Line and
			    *completer.param == "buffer" and
				...
				return true;
			if (completer.mode == InsertCompleterDesc::Line and
			    *completer.param == "all" and try_complete(complete_line<true>))
				return true;
		}
		return false;
	}
