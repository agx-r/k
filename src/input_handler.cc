...
public:
	...
	void handle_key(Key key) {
		RefPtr<InputMode> keep_alive{this};
		on_key(key);
	}
...
void InputMode::paste(StringView content) {
	try {
		...
		context().selections().for_each(
		    [&buffer, content = std::move(content), linewise](size_t index,
		                                                      Selection& sel) {
			    auto& min = sel.min();
			    auto& max = sel.max();
			    BufferRange range = buffer.insert(
			        paste_pos(buffer, min, max, PasteMode::Insert, linewise),
			        content);
			    min = range.begin;
			    max = range.end > range.begin ? buffer.char_prev(range.end)
			                                  : range.begin;
		    },
		    false);
...
std::chrono::milliseconds get_idle_timeout(const Context& context) {
	return std::chrono::milliseconds{
	    context.options()["idle_timeout"].get<int>()};
}

std::chrono::milliseconds get_fs_check_timeout(const Context& context) {
	return std::chrono::milliseconds{
	    context.options()["fs_check_timeout"].get<int>()};
}
...
struct MouseHandler {
	bool handle_key(Key key, Context& context) {
		...
		constexpr auto modifiers = Key::Modifiers::Control |
		                           Key::Modifiers::Alt | Key::Modifiers::Shift |
		                           Key::Modifiers::MouseButtonMask | ~mask;
		...
		case Key::Modifiers::Scroll:
			scroll_window(context, key.scroll_amount(),
			              m_dragging ? OnHiddenCursor::MoveCursor
...
constexpr StringView register_doc = "Special registers:\n"
...
public:
	...
	void on_enabled(bool from_pop) override {
		...
		if (not(context().flags() & Context::Flags::Draft)) {
			...
			m_fs_check_timer.set_next_date(Clock::now() +
			                               get_fs_check_timeout(context()));
			m_idle_timer.set_next_date(Clock::now() +
			                           get_idle_timeout(context()));
		}
	...
	void on_key(Key key) override {
		...
		if (m_mouse_handler.handle_key(key, context())) {
			...
			if (new_val > std::numeric_limits<int>::max())
				context().print_status(
				    {"parameter overflowed", context().faces()["Error"]});
			else
			...
			on_next_key_with_autoinfo(
			    context(), "register", KeymapMode::None,
			...
			// Hack to parse keys sent by terminals using the 8th bit to mark
			// the meta key. In normal mode, give priority to a potential
			// alt-key than the accentuated character.
			...
			if (auto command = get_normal_command(key)) {
				...
				if (autoinfo & AutoInfo::Normal and context().has_client())
					context().client().info_show(to_string(key),
					                             command->docstring.str(), {},
					                             InfoStyle::Prompt);
	...
	ModeInfo mode_info() const override {
		...
		int hidden_count = 0;
		for (auto& sel : sels)
			hidden_count += context().has_window() and
			                not context().window().display_coord(sel.cursor());
		...
		if (sels.size() == 1) {
			...
			atoms.emplace_back(
			    format("{} sels ({})", sels.size(), sels.main_index() + 1),
			    info_face);
			if (hidden_count)
				atoms.emplace_back(format(" ({} hidden)", hidden_count),
				                   info_face);
		}
...
public:
	...
	void handle_key(Key key) {
		auto erase_move = [this](auto&& move_func) {
			...
			m_clipboard =
			    m_line.substr(m_cursor_pos, old_pos - m_cursor_pos).str();
		...
		if (key == Key::Left or key == ctrl('b')) {
		...
		else if (key == Key::Backspace or key == shift(Key::Backspace) or
		         key == ctrl('h')) {
			if (m_cursor_pos != 0) {
			...
			m_line = m_line.substr(0, m_cursor_pos) + m_clipboard +
			         m_line.substr(m_cursor_pos);
	...
	void insert(StringView str) { insert_from(m_cursor_pos, str); }

	void insert_from(CharCount start, StringView str) {
		kak_assert(start <= m_cursor_pos);
		m_line = m_line.substr(0, start) + str + m_line.substr(m_cursor_pos);
	...
	DisplayLine build_display_line(ColumnCount in_width) {
		...
		if (m_cursor_pos == str.char_length())
			return DisplayLine{
			    {{str.str(), line_face}, {" "_str, cursor_face}}};
		else
			return DisplayLine(
			    {{str.substr(0, m_cursor_pos).str(), line_face},
...
public:
	Prompt(InputHandler& input_handler, StringView prompt, String initstr,
	       String emptystr, Face face, PromptFlags flags, char history_register,
	       PromptCompleter completer, PromptCallback callback)
	    : InputMode(input_handler), m_callback(std::move(callback)),
	      m_completer(std::move(completer)), m_prompt(prompt.str()),
	      m_prompt_face(face), m_empty_text{std::move(emptystr)},
	      ...
	      m_current_history{-1},
	      m_auto_complete{
	          context().options()["autocomplete"].get<AutoComplete>() &
	          AutoComplete::Prompt},
	      m_idle_timer{TimePoint::max(),
	                   context().flags() & Context::Flags::Draft
	                       ? Timer::Callback{}
	                       : [this](Timer&) {
		                         RefPtr<InputMode> keep_alive{
		                             this}; // hook or m_callback could trigger
		                                    // pop_mode()
		                         if (m_auto_complete and
		                             m_refresh_completion_pending)
			                         refresh_completions();
		                         if (m_line_changed) {
			                         m_callback(m_line_editor.line(),
			                                    PromptEvent::Change, context());
			                         m_line_changed = false;
		                         }
		                         context().hooks().run_hook(Hook::PromptIdle,
		                                                    "", context());
	                         }} {
	...
	void on_key(Key key) override {
		...
		auto can_auto_insert_completion = [&] {
			...
			const bool text_entered =
			    m_completions.start !=
			    line.byte_count_to(m_line_editor.cursor_pos());
			const bool at_end =
			    line.byte_count_to(m_line_editor.cursor_pos()) == line.length();
			return (m_completions.flags & Completions::Flags::Menu) and
			       has_completions and not completion_selected and at_end and
			       (not(m_completions.flags & Completions::Flags::NoEmpty) or
			        text_entered);
		};
		...
		if (key == Key::Return) {
			if (can_auto_insert_completion()) {
				...
				m_line_editor.insert_from(
				    line.char_count_to(m_completions.start), completion);
			}
			...
			return;
		           ...
		           ((key == Key::Backspace or key == shift(Key::Backspace) or
		             key == ctrl('h')) and
		            line.empty())) {
			history_push(line);
			context().print_status(DisplayLine{});
			...
			on_next_key_with_autoinfo(
			    ...
			    [this](Key key, Context&) {
				    const bool joined =
				        (bool)(key.modifiers & Key::Modifiers::Alt);
				    const bool quoted =
				        (bool)(key.modifiers & Key::Modifiers::Control);
				    key.modifiers &=
				        ~(Key::Modifiers::Alt | Key::Modifiers::Control);

				    auto cp = key.codepoint();
				    if (not cp or key == Key::Escape)
					    return;

				    auto* quoter = Kakoune::quoter(quoted ? Quoting::Kakoune
				                                          : Quoting::Raw);
				    m_line_editor.insert(
				        joined ? join(RegisterManager::instance()[*cp].get(
				                          context()) |
				                          transform(quoter),
				                      ' ', false)
				               : quoter(context().main_sel_register_value(
				                     String{*cp})));
			...
			on_next_key_with_autoinfo(
			    context(), "raw-key", KeymapMode::None,
			    [this](Key key, Context&) {
			...
			m_current_history = std::min(static_cast<int>(history.size()) - 1,
			                             m_current_history);
			if (m_current_history == -1)
				m_prefix = line;
			auto next = find_if(
			    history.subrange(m_current_history + 1),
			    [this](StringView s) { return prefix_match(s, m_prefix); });
			if (next != history.end()) {
		...
		} else if (key == Key::Down or key == ctrl('n')) // next
		{
			...
			if (m_current_history >= 0) {
				auto next = find_if(
				    history.subrange(0, m_current_history) | reverse(),
				    [this](StringView s) { return prefix_match(s, m_prefix); });
				m_current_history = history.rend() - next - 1;
				m_line_editor.reset(next != history.rend() ? *next : m_prefix,
				                    m_empty_text);
				clear_completions();
				m_refresh_completion_pending = true;
			}
		} else if (key == Key::Tab or key == shift(Key::Tab) or
		           key.modifiers == Key::Modifiers::MenuSelect) // completion
		{
			CandidateList& candidates = m_completions.candidates;

			if (m_auto_complete and m_refresh_completion_pending)
				refresh_completions();
			if (candidates.empty()) // manual completion, we need to ask our
			...
			if (key.modifiers == Key::Modifiers::MenuSelect)
				m_current_completion =
				    clamp<int>(key.key, 0, candidates.size() - 1);
			else if (not reverse and
			...
			on_next_key_with_autoinfo(
			    context(), "explicit-completion", KeymapMode::None,
			    [this](Key key, Context&) {
				    m_explicit_completer = ArgCompleter{};

				    if (key.key == 'f')
					    use_explicit_completer([](const Context& context,
					                              StringView token) {
			...
			try {
				...
				context().print_status(
				    {error.what(), context().faces()["Error"]});
				return;
			}
		} else if (key == alt(';')) {
			...
			if ((key == Key::Space or key == shift(Key::Space)) and
			    not(m_completions.flags &
			        Completions::Flags::Quoted) and // if token is quoted, this
			                                        // space does not end it
			    can_auto_insert_completion())
				m_line_editor.insert_from(
				    line.char_count_to(m_completions.start),
		...
		if (enabled() and
		    not(context().flags() &
		        Context::Flags::Draft)) // The callback might have disabled us
			m_idle_timer.set_next_date(Clock::now() +
			                           get_idle_timeout(context()));
	}

	void refresh_ifn() override {
		bool explicit_completion_selected =
		    m_current_completion != -1 and
		    (not m_prefix_in_completions or
		     m_current_completion != m_completions.candidates.size() - 1);
		if (not enabled() or (context().flags() & Context::Flags::Draft) or
		    explicit_completion_selected)
			return;
	...
	void on_raw_key() override { m_was_interactive = true; }
...
private:
	...
	void use_explicit_completer(Completer&& completer) {
		m_explicit_completer = [completer](const Context& context,
		                                   StringView content,
			...
			if (last_token and
			    (last_token->pos + last_token->content.length() < cursor_pos))
				last_token.reset();

			auto token_start = last_token.map([&](auto&& t) { return t.pos; })
			                       .value_or(cursor_pos);
			auto token_content =
			    last_token.map([](auto&& t) -> StringView { return t.content; })
			        .value_or(StringView{});

			return Completions{token_start, cursor_pos,
			                   completer(context, token_content)};
		};
	}
	...
	void refresh_completions() {
		try {
			...
			auto& completer =
			    m_explicit_completer ? m_explicit_completer : m_completer;
			if (not completer)
				return;
			m_current_completion = -1;
			const String& line = m_line_editor.line();
			m_completions =
			    completer(context(), line,
			              line.byte_count_to(m_line_editor.cursor_pos()));
			...
			show_completions();
			const bool menu =
			    (bool)(m_completions.flags & Completions::Flags::Menu);
			if (menu)
				context().client().menu_select(0);
			auto prefix = line.substr(m_completions.start,
			                          m_completions.end - m_completions.start);
			m_prefix_in_completions =
			    not menu and not contains(m_completions.candidates, prefix);
	...
	void show_completions() {
		...
		const auto menu_style = (m_flags & PromptFlags::Search)
		                            ? MenuStyle::Search
		                            : MenuStyle::Prompt;
	...
	void display() {
		...
		auto width =
		    context().client().dimensions().column - m_prompt.column_length();
		DisplayLine display_line;
		if (not(m_flags & PromptFlags::Password))
			display_line = m_line_editor.build_display_line(width);

		auto status_style =
		    (m_flags & PromptFlags::Search)    ? StatusStyle::Search
		    : (m_flags & PromptFlags::Command) ? StatusStyle::Command
		                                       : StatusStyle::Prompt;
		context().print_status({m_prompt, m_prompt_face}, display_line,
		                       m_line_editor.cursor_display_column(),
		                       status_style);
	}
...
public:
	NextKey(InputHandler& input_handler, String name, KeymapMode keymap_mode,
	        KeyCallback callback, Timer::Callback idle_callback)
	    : InputMode(input_handler), m_name{std::move(name)},
	      m_callback(std::move(callback)), m_keymap_mode(keymap_mode),
	      m_idle_timer{Clock::now() + get_idle_timeout(context()),
		...
		if (not from_push) {
			...
			if (m_restore_cursor) {
				for (auto& sel : selections) {
					if (sel.cursor() > sel.anchor() and
					    sel.cursor() > BufferCoord{0, 0})
						sel.cursor() =
						    context().buffer().char_prev(sel.cursor());
				}
			}
		}
	}
	...
	void on_key(Key key) override {
		...
		if (m_mouse_handler.handle_key(key, context())) {
			...
			if (not main_char.empty())
				context().hooks().run_hook(Hook::InsertDelete, main_char,
				                           context());
			...
			for (auto& sel : selections) {
				...
				sel.anchor() = sel.cursor() =
				    buffer.clamp({line, buffer[line].length()});
			}
			selections.sort_and_merge_overlapping();
		} else if (auto cp = key.codepoint()) {
		...
		} else if (key == ctrl('n') or key == ctrl('p') or
		           key.modifiers == Key::Modifiers::MenuSelect) {
			drop_last_recorded_key();
			bool relative = key.modifiers != Key::Modifiers::MenuSelect;
			int index = relative ? (key == ctrl('n') ? 1 : -1) : key.key;
			m_completer.select(index, relative,
			                   [&](Key key) { record_key(key); });
			update_completions = false;
		} else if (key == ctrl('x')) {
			on_next_key_with_autoinfo(
			    context(), "explicit-completion", KeymapMode::None,
			...
			context().print_status(
			    {format("committed change #{}",
			            (size_t)context().buffer().current_history_id()),
			     context().faces()["Information"]});
		} else if (key == ctrl('v')) {
			m_completer.try_accept();
			on_next_key_with_autoinfo(
			    context(), "raw-insert", KeymapMode::None,
			    [this, transient](Key key, Context&) {
				    if (auto cp = get_raw_codepoint(key)) {
					    insert(*cp);
					    context().hooks().run_hook(Hook::InsertKey,
					                               to_string(key), context());
					    if (enabled() and not transient)
						    m_idle_timer.set_next_date(
						        Clock::now() + get_idle_timeout(context()));
				    }
			    },
			    "raw insert", "enter key to insert");
			update_completions = false;
		...
		if (moved)
			context().hooks().run_hook(Hook::InsertMove, to_string(key),
			                           context());

		if (update_completions and enabled() and
		    not transient) // Hooks might have disabled us
			m_idle_timer.set_next_date(Clock::now() +
			                           get_idle_timeout(context()));
	}
...
private:
	template <typename Type> void move(Type offset) {
		auto& selections = context().selections();
		const ColumnCount tabstop = context().options()["tabstop"].get<int>();
		for (auto& sel : selections) {
			auto cursor =
			    context().buffer().offset_coord(sel.cursor(), offset, tabstop);
			sel.anchor() = sel.cursor() = cursor;
		}
		selections.sort_and_merge_overlapping();
	}

	template <typename S> void insert(ConstArrayView<S> strings) {
		kak_assert(not m_completer.has_candidate_selected());
		context().selections().for_each(
		    [strings, &buffer = context().buffer()](size_t index,
		                                            Selection& sel) {
			    Kakoune::insert(buffer, sel, sel.cursor(),
			                    strings[std::min(strings.size() - 1, index)]);
		    },
		    false);
	}
	...
	void prepare(InsertMode mode, int count) {
		...
		case InsertMode::OpenLineBelow: {
			...
			for (auto sel : selections) {
				...
				for (int i = 0; i < count; ++i)
					new_sels.push_back(
					    BufferCoord{sel.max().line + inserted_count + i + 1});
		...
		case InsertMode::OpenLineAbove: {
			...
			for (auto sel : selections) {
				...
				for (int i = 0; i < count; ++i)
					new_sels.push_back(
					    BufferCoord{sel.min().line + inserted_count + i});
				inserted_count += count;
			}
			selections.set(std::move(new_sels),
			               selections.main_index() * count + count - 1);
...
InputHandler::InputHandler(SelectionList selections, Context::Flags flags,
                           String name)
...
void InputHandler::push_mode(InputMode* new_mode) {
	...
	context().hooks().run_hook(
	    Hook::ModeChange, format("push:{}:{}", prev_name, new_mode->name()),
	    context());
}
...
void InputHandler::pop_mode(InputMode* mode) {
	...
	context().hooks().run_hook(
	    Hook::ModeChange, format("pop:{}:{}", prev_name, current_mode().name()),
	    context());
}

void InputHandler::reset_normal_mode() {
	kak_assert(dynamic_cast<InputModes::Normal*>(m_mode_stack[0].get()) !=
	           nullptr);
...
void InputHandler::insert(InsertMode mode, int count) {
	push_mode(new InputModes::Insert(*this, mode, count,
	                                 m_handle_key_level <= 1 ? &m_last_insert
	                                                         : nullptr));
}
...
void InputHandler::repeat_last_insert() {
	...
	push_mode(new InputModes::Insert(*this, m_last_insert.mode,
	                                 m_last_insert.count, nullptr));
	for (auto& key : m_last_insert.keys)
		handle_key(key);
	kak_assert(dynamic_cast<InputModes::Normal*>(&current_mode()) != nullptr);
}

void InputHandler::paste(StringView content) { current_mode().paste(content); }

void InputHandler::prompt(StringView prompt, String initstr, String emptystr,
                          Face prompt_face, PromptFlags flags,
                          char history_register, PromptCompleter completer,
                          PromptCallback callback) {
	push_mode(new InputModes::Prompt(
	    *this, prompt, std::move(initstr), std::move(emptystr), prompt_face,
	    flags, history_register, std::move(completer), std::move(callback)));
}
...
void InputHandler::on_next_key(StringView mode_name, KeymapMode keymap_mode,
                               KeyCallback callback,
                               Timer::Callback idle_callback) {
	push_mode(new InputModes::NextKey(*this, format("next-key[{}]", mode_name),
	                                  keymap_mode, std::move(callback),
	                                  std::move(idle_callback)));
}
...
InputHandler::ScopedForceNormal::ScopedForceNormal(InputHandler& handler,
	...
	static_cast<InputModes::Normal*>(handler.m_mode_stack.back().get())
	    ->m_params = params;
}
...
static bool is_valid(Key key) {
	constexpr Key::Modifiers valid_mods =
	    (Key::Modifiers::Control | Key::Modifiers::Alt | Key::Modifiers::Shift);
...
void InputHandler::refresh_ifn() { current_mode().refresh_ifn(); }
...
bool InputHandler::is_recording() const { return m_recording_reg != 0; }
...
ModeInfo InputHandler::mode_info() const { return current_mode().mode_info(); }

bool should_show_info(AutoInfo mask, const Context& context) {
	return (context.options()["autoinfo"].get<AutoInfo>() & mask) and
	       context.has_client();
}
...
void scroll_window(Context& context, LineCount offset,
                   OnHiddenCursor on_hidden_cursor) {
	...
	if ((offset < 0 and win_pos.line == 0) or
	    (offset > 0 and win_pos.line == line_count - 1))
		return;

	const DisplayCoord max_offset{(win_dim.line - 1) / 2,
	                              (win_dim.column - 1) / 2};
	const DisplayCoord scrolloff = std::min(
	    context.options()["scrolloff"].get<DisplayCoord>(), max_offset);
	...
	if (on_hidden_cursor != OnHiddenCursor::PreserveSelections) {
		...
		auto line =
		    clamp(cursor.line + cursor_off, win_pos.line + scrolloff.line,
		          win_pos.line + win_dim.line - 1 - scrolloff.line);

		const ColumnCount tabstop = context.options()["tabstop"].get<int>();
		auto new_cursor =
		    buffer.offset_coord(cursor, line - cursor.line, tabstop);

		main_selection = {on_hidden_cursor == OnHiddenCursor::MoveCursor
		                      ? anchor
		                      : new_cursor,
		                  new_cursor};
