...
template <typename Res, typename... Args> struct FunctionVTable {
	Res (*call)(void*, Args...);
	void* (*clone)(void*);
	void (*destroy)(void*);
};
...
inline constexpr FunctionVTable<Res, Args...> unset_vtable{
    .call = [](void*, Args...) -> Res {
	    throw runtime_error("called an empty function");
    },
...
inline constexpr FunctionVTable<Res, Args...> vtable_for{
    .call = [](void* target, Args... args) -> Res {
	    return (*reinterpret_cast<Target*>(target))(
	        std::forward<Args>(args)...);
    },
    .clone = [](void* target) -> void* {
	    if constexpr (Copyable) {
		    return new Target(*reinterpret_cast<Target*>(target));
	    }
	    return nullptr;
    },
...
inline constexpr FunctionVTable<Res, Args...> vtable_for_func{
    .call = [](void* target, Args... args) -> Res {
	    return (*reinterpret_cast<Res (*)(Args...)>(target))(
	        std::forward<Args>(args)...);
    },
    .clone = [](void* target) -> void* { return target; },
    .destroy = [](void* target) {}};

template <bool Copyable, typename F> class FunctionImpl;
...
public:
	...
	FunctionImpl(Target&& target) {
		...
		if constexpr (std::is_convertible_v<EffectiveTarget,
		                                    Res (*)(Args...)>) {
			m_target =
			    reinterpret_cast<void*>(static_cast<Res (*)(Args...)>(target));
	...
	FunctionImpl(FunctionImpl&& other)
	    : m_target(other.m_target), m_vtable(other.m_vtable) {
		other.m_target = nullptr;
		other.m_vtable = &unset_vtable<Res, Args...>;
	}

	~FunctionImpl() { m_vtable->destroy(m_target); }
	...
	explicit operator bool() const {
		return m_vtable != &unset_vtable<Res, Args...>;
	}
...
template <typename F> using Function = FunctionImpl<true, F>;

template <typename F> using MoveOnlyFunction = FunctionImpl<false, F>;
