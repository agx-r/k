...
private:
	...
	using OptionMap =
	    HashMap<StringView, UniquePtr<Option>, MemoryDomain::Options>;
...
public:
	...
	String get_desc_string() const override {
		if constexpr (std::is_same_v<int, T> or std::is_same_v<bool, T> or
...
public:
	...
	Option& declare_option(StringView name, StringView docstring,
		...
		return *opts.insert({m_descs.back()->name(),
		                     make_unique_ptr<TypedCheckedOption<T, validator>>(
		                         m_global_manager, *m_descs.back(), value)});
	}

	const OptionDesc* option_desc(StringView name) const {
		auto it =
		    find_if(m_descs, [&name](const UniquePtr<const OptionDesc>& opt) {
	...
	void move_to_trash(UniquePtr<Option>&& option) {
		m_option_trash.push_back(std::move(option));
	}
