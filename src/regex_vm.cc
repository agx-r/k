...
template <RegexMode mode = RegexMode::Forward> struct Children {
	...
	struct Iterator {
		...
		Iterator(ArrayView<const ParsedRegex::Node> nodes, Index index)
		    : m_nodes{nodes},
		      m_pos(forward ? index + 1
		                    : find_prev(index, nodes[index].children_end)),
		      m_end(forward ? nodes[index].children_end : index) {}

		Iterator& operator++() {
			m_pos =
			    forward ? m_nodes[m_pos].children_end : find_prev(m_end, m_pos);
			return *this;
		}
...
struct RegexParser {
	static ParsedRegex parse(StringView re) {
		return RegexParser{re}.m_parsed_regex;
	}

private:
	RegexParser(StringView re) : m_regex{re}, m_pos{re.begin(), re} {
	...
	struct InvalidPolicy {
		Codepoint operator()(Codepoint cp) const {
			throw regex_error{"Invalid utf8 in regex"};
		}
	};
	...
	using Iterator =
	    utf8::iterator<const char*, const char*, Codepoint, int, InvalidPolicy>;
		...
		if (m_regex.end() - it >= 4 and *it++ == '(' and *it++ == '?') {
			while (true) {
				...
				case 'I':
					...
					break;
				case 's':
					m_flags |= Flags::DotMatchesNewLine;
					break;
				case 'S':
					m_flags &= ~Flags::DotMatchesNewLine;
					break;
	...
	Optional<NodeIndex> assertion() {
		...
		case '^':
			++m_pos;
			return add_node(ParsedRegex::LineStart);
		case '$':
			++m_pos;
			return add_node(ParsedRegex::LineEnd);
		case '\\':
			...
			case 'b':
				m_pos += 2;
				return add_node(ParsedRegex::WordBoundary);
			case 'B':
				m_pos += 2;
				return add_node(ParsedRegex::NotWordBoundary);
			case 'A':
				m_pos += 2;
				return add_node(ParsedRegex::SubjectBegin);
			case 'z':
				m_pos += 2;
				return add_node(ParsedRegex::SubjectEnd);
			case 'K':
				m_pos += 2;
				return add_node(ParsedRegex::ResetStart);
			}
			break;
		case '(': {
			...
			switch (*it++) {
			case '=':
				op = ParsedRegex::LookAhead;
				break;
			case '!':
				op = ParsedRegex::NegativeLookAhead;
				break;
			case '<': {
				switch (*it++) {
				case '=':
					op = ParsedRegex::LookBehind;
					break;
				case '!':
					op = ParsedRegex::NegativeLookBehind;
					break;
				default:
					return {};
				}
	...
	Optional<NodeIndex> atom() {
		...
		case '.':
			++m_pos;
			return add_node((m_flags & Flags::DotMatchesNewLine)
			                    ? ParsedRegex::AnyChar
			                    : ParsedRegex::AnyCharExceptNewLine);
		case '(': {
			...
			if (m_regex.end() - it < 2 or *it++ != '?')
				...
				if (it == m_regex.end() or *it != '>')
					parse_error(
					    "named captures should be only ascii word characters");
				capture_group = m_parsed_regex.capture_count++;
				m_parsed_regex.named_captures.push_back(
				    {{name_start, it}, capture_group});
				m_pos = Iterator{++it, m_regex};
			}
		...
		case ')':
			return {};
		default:
			if (contains(StringView{"^$.*+?[]{}"}, cp) or
			    (cp >= 0xF0000 and cp <= 0xFFFFF))
	...
	NodeIndex atom_escape() {
		...
		if (cp == 'Q') {
			...
			auto quote_end = std::search(m_pos.base(), m_regex.end(),
			                             end_mark.begin(), end_mark.end());
			while (m_pos != quote_end)
				add_node(ParsedRegex::Literal, *m_pos++);
			get_node(escaped_sequence).children_end =
			    m_parsed_regex.nodes.size();
	...
	static void normalize_ranges(
	    Vector<CharacterClass::Range, MemoryDomain::Regex>& ranges) {
		if (ranges.empty())
			return;
	...
	NodeIndex character_class() {
		...
		while (m_pos != m_regex.end() and *m_pos != ']') {
			...
			auto read_escaped_char = [this]() {
				Codepoint cp = *m_pos++;
				auto it = find_if(control_escapes,
				                  [cp](auto&& t) { return t.name == cp; });
				...
				if (cp == 'u')
					return read_hex(6);
				if (not contains("^$\\.*+?()[]{}|-",
				                 cp)) // SyntaxCharacter and -
					parse_error(
					    format("unknown character class escape '{}'", cp));
				return cp;
			};

			if (cp == '\\') {
				auto it =
				    find_if(character_class_escapes,
		...
		if (not character_class.ignore_case) {
			...
			for (const auto& [min, max] : character_class.ranges) {
				if (not contains(
				        character_class.ranges,
				        CharacterClass::Range{to_lower(min), to_lower(max)}) or
				    not contains(
				        character_class.ranges,
				        CharacterClass::Range{to_upper(min), to_upper(max)}))
		...
		if (character_class.ctypes == CharacterType::None and
		    ...
		    character_class.ranges.front().min ==
		        character_class.ranges.front().max)
			return add_node(ParsedRegex::Literal,
			                character_class.ranges.front().min, {1, 1},
			                character_class.ignore_case);

		if (character_class.ctypes != CharacterType::None and
		    not character_class.negative and character_class.ranges.empty())
			return add_node(ParsedRegex::CharType,
			                (Codepoint)character_class.ctypes);

		auto it =
		    std::find(m_parsed_regex.character_classes.begin(),
		              m_parsed_regex.character_classes.end(), character_class);
		auto class_id = it - m_parsed_regex.character_classes.begin();
		if (it == m_parsed_regex.character_classes.end())
			m_parsed_regex.character_classes.push_back(
	...
	ParsedRegex::Quantifier quantifier() {
		...
		auto read_bound = [&]() -> Optional<int16_t> {
			...
			for (auto begin = m_pos; m_pos != m_regex.end(); ++m_pos) {
				...
				if (res > max_repeat)
					parse_error(
					    format("Explicit quantifier is too big, maximum is {}",
					           max_repeat));
			}
			return res;
		};
		...
		case '*':
			++m_pos;
			return {0, ParsedRegex::Quantifier::infinite, check_greedy()};
		case '+':
			++m_pos;
			return {1, ParsedRegex::Quantifier::infinite, check_greedy()};
		case '?':
			++m_pos;
			return {0, 1, check_greedy()};
		case '{': {
			++m_pos;
	...
	NodeIndex add_node(ParsedRegex::Op op, Codepoint value = -1,
	                   ParsedRegex::Quantifier quantifier = {1, 1},
	                   bool ignore_case = false) {
		constexpr auto max_nodes = std::numeric_limits<int16_t>::max();
		const NodeIndex res = m_parsed_regex.nodes.size();
		if (res == max_nodes)
			parse_error(
			    format("regex parsed to more than {} ast nodes", max_nodes));
		const NodeIndex next = res + 1;
		m_parsed_regex.nodes.push_back(
		    {op, ignore_case or (m_flags & Flags::IgnoreCase), next, value,
		     quantifier});
		return res;
	}
	...
	void parse_error(StringView error) const {
		throw regex_error(format("regex parse error: {} at '{}<<<HERE>>>{}'",
		                         error,
	...
	void validate_lookaround(NodeIndex index) {
		...
		for (auto child_index : Children<>{m_parsed_regex, index}) {
			...
			if (child.op != ParsedRegex::Literal and
			    child.op != ParsedRegex::CharClass and
			    child.op != ParsedRegex::CharType and
			    child.op != ParsedRegex::AnyChar and
			    child.op != ParsedRegex::AnyCharExceptNewLine)
				parse_error("Lookaround can only contain literals, any chars "
				            "or character classes");
			if (child.op == ParsedRegex::Literal and
			    to_underlying(Lookaround::OpBegin) <= child.value and
			    child.value < to_underlying(Lookaround::OpEnd))
				parse_error("Lookaround does not support literals codepoint "
				            "between 0xF0000 and 0xFFFFD");
	...
	static constexpr struct CharacterClassEscape {
	    ...
	    {'d', CharacterType::Digit},
	    {'D', CharacterType::NotDigit},
	    {'w', CharacterType::Word},
	    {'W', CharacterType::NotWord},
	    {'s', CharacterType::Whitespace},
	    {'S', CharacterType::NotWhitespace},
	    {'h', CharacterType::HorizontalWhitespace},
	    {'H', CharacterType::NotHorizontalWhitespace},
	};
...
constexpr RegexParser::CharacterClassEscape
    RegexParser::character_class_escapes[];
...
struct RegexCompiler {
	...
	RegexCompiler(ParsedRegex&& parsed_regex, RegexCompileFlags flags)
		...
		kak_assert(not(flags & RegexCompileFlags::NoForward) or
		           flags & RegexCompileFlags::Backward);
		// Approximation of the number of instructions generated
		m_program.instructions.reserve(
		    (parsed_regex.nodes.size() + 1) *
		    (((flags & RegexCompileFlags::Backward) and
		      not(flags & RegexCompileFlags::NoForward))
		         ? 2
		         : 1));

		if (not(flags & RegexCompileFlags::NoForward)) {
			m_program.forward_start_desc =
			    compute_start_desc<RegexMode::Forward>();
		...
		if (flags & RegexCompileFlags::Backward) {
			...
			optimize(m_program.first_backward_inst,
			         m_program.instructions.size());
			push_inst(CompiledRegex::Match);
		} else
			m_program.first_backward_inst = -1;

		m_program.character_classes =
		    std::move(m_parsed_regex.character_classes);
...
private:
	...
	OpIndex compile_node_inner(ParsedRegex::NodeIndex index) {
		...
		const bool save =
		    (node.op == ParsedRegex::Alternation or
		     node.op == ParsedRegex::Sequence) and
		    (node.value == 0 or
		     (node.value != -1 and not(m_flags & RegexCompileFlags::NoSubs)));
		constexpr bool forward = direction == RegexMode::Forward;
		if (save)
			push_inst(
			    CompiledRegex::Save,
			    {.save_index = int16_t(node.value * 2 + (forward ? 0 : 1))});
		...
		case ParsedRegex::Literal:
			push_inst(
			    ...
			    {.literal = {.codepoint = ignore_case ? to_lower(node.value)
			                                          : node.value,
		...
		case ParsedRegex::CharClass:
			if (auto& char_class = m_parsed_regex.character_classes[node.value];
			    char_class.ranges.size() == 1 and
			    char_class.ctypes == CharacterType::None and
			    char_class.ranges[0].max <= std::numeric_limits<uint8_t>::max())
				push_inst(CompiledRegex::CharRange,
				          {.range = {.min = uint8_t(char_class.ranges[0].min),
				                     .max = uint8_t(char_class.ranges[0].max),
				                     .ignore_case = char_class.ignore_case,
				                     .negative = char_class.negative}});
			else
				push_inst(CompiledRegex::CharClass,
				          {.character_class_index = int16_t(node.value)});
			break;
		case ParsedRegex::CharType:
			push_inst(
			    CompiledRegex::CharType,
			    {.character_type = CharacterType{(unsigned char)node.value}});
			break;
		...
		case ParsedRegex::Alternation: {
			...
			for (auto child : Children<>{m_parsed_regex, index}) {
				...
				if (child != index + 1) {
					--split_pos;
					m_program.instructions[split_pos].param.split = {
					    .offset = offset(node, split_pos),
					    .prioritize_parent = true};
				}
		...
		case ParsedRegex::NegativeLookAhead:
			push_inst(
			    CompiledRegex::LookAround,
			    {.lookaround = {.index = push_lookaround<RegexMode::Forward>(
			                        index, ignore_case),
			                    .ahead = true,
		...
		case ParsedRegex::NegativeLookBehind:
			push_inst(
			    CompiledRegex::LookAround,
			    {.lookaround = {.index = push_lookaround<RegexMode::Backward>(
			                        index, ignore_case),
			                    .ahead = false,
		...
		for (auto& index : goto_inner_end_offsets)
			m_program.instructions[index].param.jump_offset =
			    offset(op_count(), index);

		if (save)
			push_inst(
			    CompiledRegex::Save,
			    {.save_index = int16_t(node.value * 2 + (forward ? 1 : 0))});
	...
	OpIndex compile_node(ParsedRegex::NodeIndex index) {
		...
		if (quantifier.allows_none()) {
			auto split_pos =
			    push_inst(CompiledRegex::Split,
			              {.split = {.offset = 0,
			                         .prioritize_parent = quantifier.greedy}});
		...
		if (quantifier.allows_infinite_repeat())
			push_inst(CompiledRegex::Split,
			          {.split = {.offset = offset(inner_pos, op_count()),
			                     .prioritize_parent = not quantifier.greedy}});
		// Write the node as an optional match for the min -> max counts
		else
			for (int i = std::max((int16_t)1, quantifier.min); // STILL UGLY !
			     i < quantifier.max; ++i) {
				auto split_pos = push_inst(
				    CompiledRegex::Split,
				    {.split = {.offset = 0,
				               .prioritize_parent = quantifier.greedy}});
				goto_ends.push_back(split_pos);
		...
		for (auto index : goto_ends)
			m_program.instructions[index].param.split.offset =
			    offset(op_count(), index);
	...
	OpIndex push_inst(CompiledRegex::Op op, CompiledRegex::Param param = {}) {
		...
		if (res >= max_instructions)
			throw regex_error(
			    format("regex compiled to more than {} instructions",
			           max_instructions));
	...
	int16_t push_lookaround(ParsedRegex::NodeIndex index, bool ignore_case) {
		...
		for (auto child : Children<direction>{m_parsed_regex, index}) {
			...
			if (character.op == ParsedRegex::Literal)
				m_program.lookarounds.push_back(static_cast<Lookaround>(
				    ignore_case ? to_lower(character.value) : character.value));
				...
				m_program.lookarounds.push_back(
				    Lookaround::AnyCharExceptNewLine);
				...
				m_program.lookarounds.push_back(static_cast<Lookaround>(
				    to_underlying(Lookaround::CharacterType) |
				    character.value));
			else
				kak_assert(false);
		}
	...
	// Mutate start_desc with informations on which Codepoint could start a
	// match. Returns true if the subsequent nodes are still relevant for
	// computing the start desc
	template <RegexMode direction>
	bool compute_start_desc(ParsedRegex::NodeIndex index,
		...
		auto add_multi_byte_utf8 = [&] {
			std::fill(start_desc.map + 0b11000000, start_desc.map + 0b11111000,
			          true);
		};
		...
		case ParsedRegex::AnyChar:
			if (start_desc.offset + node.quantifier.max <=
			    CompiledRegex::StartDesc::OffsetLimits::max()) {
		...
		case ParsedRegex::CharClass: {
			auto& character_class =
			    m_parsed_regex.character_classes[node.value];
			if (character_class.ctypes == CharacterType::None and
			    not character_class.negative and
			    not character_class.ignore_case) {
				for (auto& range : character_class.ranges) {
					const auto clamp = [](Codepoint cp) {
						return std::min(single_byte_limit, cp);
					};
					for (auto cp = clamp(range.min), end = clamp(range.max + 1);
					     cp < end; ++cp)
						start_desc.map[cp] = true;
	...
	UniquePtr<CompiledRegex::StartDesc> compute_start_desc() const {
		...
		if (std::count(std::begin(start_desc.map), std::end(start_desc.map),
		               true) == 1)
			start_desc.start_byte =
			    find(start_desc.map, true) - std::begin(start_desc.map);
	...
	void optimize(size_t begin, size_t end) {
		...
		auto is_jump = [](CompiledRegex::Op op) {
			return op >= CompiledRegex::Op::Jump and
			       op <= CompiledRegex::Op::Split;
		};
		for (auto i = begin; i < end; ++i) {
			auto& inst = m_program.instructions[i];
			if (is_jump(inst.op))
				m_program.instructions[i + inst.param.jump_offset].last_step =
				    0xffff; // tag as jump target
		}
	...
	void peephole_optimize(size_t begin, size_t end) {
		...
		auto is_assertion = [](CompiledRegex::Op op) {
			return op >= CompiledRegex::LineAssertion;
		};
...
String dump_regex(const CompiledRegex& program) {
	...
	for (auto& inst : program.instructions) {
		...
		case CompiledRegex::Literal:
			res +=
			    format("literal {}{}\n",
			           inst.param.literal.ignore_case ? "(ignore case) " : "",
			           inst.param.literal.codepoint);
			break;
		...
		case CompiledRegex::Jump:
			res += format("jump {} ({:03})\n", inst.param.jump_offset,
			              index + inst.param.jump_offset);
			break;
		...
		case CompiledRegex::LookAround: {
			...
			String str;
			for (auto it =
			         program.lookarounds.begin() + inst.param.lookaround.index;
	...
	auto dump_start_desc = [&](const CompiledRegex::StartDesc& desc,
	                           StringView name) {
		res += name + " start desc: [";
...
bool is_ctype(CharacterType ctype, Codepoint cp) {
	...
	return check(CharacterType::Word, CharacterType::NotWord,
	             [](Codepoint cp) { return is_word(cp); }) or
	       check(CharacterType::Whitespace, CharacterType::NotWhitespace,
	             is_blank) or
	       check(CharacterType::HorizontalWhitespace,
	             CharacterType::NotHorizontalWhitespace, is_horizontal_blank) or
...
struct TestVM : CompiledRegex, ThreadedRegexVM<const char*, mode> {
	...
	bool exec(StringView re, RegexExecFlags flags = RegexExecFlags::None) {
		return TestVM::ThreadedRegexVM::exec(re.begin(), re.end(), re.begin(),
		                                     re.end(), flags);
	}
...
auto test_regex = UnitTest{[] {
...

	{
		...
		kak_assert(StringView{vm.captures()[0], vm.captures()[1]} ==
		           "foobarfoo");
	...
	{
		TestVM<RegexMode::Forward | RegexMode::Search> vm{
		    R"(foobaz|foo|foobar)"};
		kak_assert(vm.exec("foobar"));
	...
	{
		TestVM<RegexMode::Backward | RegexMode::Search> vm{
		    R"((?<=f)oo(b[ae]r)?(?=baz))"};
...

	{
		...
		kak_assert(not vm.exec(str, str + 4, str, str + sizeof(str) - 1,
		                       RegexExecFlags::None));
	}
...

	{
		...
		kak_assert(vm.exec("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",
		                   RegexExecFlags::None));
	}

	{
		TestVM<RegexMode::Forward | RegexMode::Search> vm{"(.{3,4}|f)oo"};
		kak_assert(vm.forward_start_desc and
		           vm.forward_start_desc->offset == 4);
...

	{
		...
		kak_assert(vm.backward_start_desc and
		           vm.backward_start_desc->offset == 4);
		for (int c = 0; c < CompiledRegex::StartDesc::count; ++c)
			kak_assert(vm.backward_start_desc->map[c] ==
			           (c == 'f' or c == 'o'));
...

	{
		auto eq = [](const CompiledRegex::NamedCapture& lhs,
			...
			return lhs.name == rhs.name and lhs.index == rhs.index;
		};
	...
	auto check_parse_error = [](StringView re, StringView expected_error) {
		try {
			...
			kak_assert(err.what() ==
			           String{"regex parse error: "} + expected_error);
		}
	};

	check_parse_error(
	    "(?=a*)",
	    "Quantifiers cannot be used in lookarounds at '(?=a*)<<<HERE>>>'");
	check_parse_error("(?=(a))",
	                  "Lookaround can only contain literals, any chars or "
	                  "character classes at '(?=(a))<<<HERE>>>'");
	check_parse_error(
	    "(?=a|b)",
	    "Alternations cannot be used in lookarounds at '(?=a|<<<HERE>>>b)'");
}};
